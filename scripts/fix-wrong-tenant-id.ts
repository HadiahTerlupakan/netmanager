...
async function main() {
  logger.info("Memeriksa tenant saat ini...");

  const tenants = await prisma.tenant.findMany();
  logger.info("Daftar Tenant:", tenants);

  const wrongIdTenant = tenants.find(
    (t) => t.name === MAIN_TENANT_NAME && t.id !== MAIN_TENANT_ID,
  );

  if (wrongIdTenant) {
    logger.info(
      `Ditemukan tenant ${MAIN_TENANT_NAME} dengan ID salah: ${wrongIdTenant.id}. Akan diperbaiki...`,
    );
    ...
    logger.info(
      "Menerapkan ID ke seluruh record yang terkait dengan old tenant (menggunakan Raw SQL untuk update cascade otomatis karena prisma tidak support update PK)...",
    );

    try {
      await prisma.$executeRawUnsafe(
        `UPDATE "Tenant" SET id = '${MAIN_TENANT_ID}' WHERE id = '${wrongIdTenant.id}'`,
      );
      logger.info(
        "Berhasil update ID tenant di database secara force/cascade!",
      );
    } catch (e: any) {
      logger.warn(
        "Gagal update PK langsung (biasanya terkendala constrain jika tidak cascade).",
        e.message,
      );
      logger.info("Mencoba cara aman (recreate & migrate)...");

      const correctTenantExists = await prisma.tenant.findUnique({
        where: { id: MAIN_TENANT_ID },
      });
      if (!correctTenantExists) {
        await prisma.tenant.create({
          data: { id: MAIN_TENANT_ID, name: MAIN_TENANT_NAME + "_TEMP" },
        });
      }
    ...
    logger.info(
      `Tenant ${MAIN_TENANT_NAME} sudah memiliki ID yang benar: ${MAIN_TENANT_ID}`,
    );
  }
}
