...
async function seedFinancePermissions() {
  ...
  const rolesWithFinance = await prisma.$queryRaw<
  ...
  logger.info(
    "Roles with finance permission:",
    rolesWithFinance.map((r) => r.name),
  );
  ...
  for (const resource of newResources) {
    for (const action of actions) {
      ...
      if (!existing) {
        ...
        logger.info("Created:", `${resource}:${action}`);
        ...
        logger.info("Exists:", `${resource}:${action}`);
