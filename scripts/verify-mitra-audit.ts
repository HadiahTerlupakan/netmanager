...
async function verifyMitra() {
  console.log('--- MITRA MULTI-TENANT AUDIT ---');

  // 1. Check a known Mitra (from previous logs or database)
  const mitras = await prismaMitraAuth.mitra.findMany({ take: 1 });
  if (mitras.length === 0) {
    console.log('No Mitra found in database. Please seed some data first.');
    return;
  }

  const testMitra = mitras[0];
  console.log(`Testing Mitra: ${testMitra.email} (Tenant: ${testMitra.tenantId})`);
  ...
  console.log(`Login Lookup Result: ${foundMitra ? 'FOUND' : 'NOT FOUND'}`);
  ...
  try {
    ...
    console.log(`Lookup with NO context: ${result ? 'FOUND (LEAK!)' : 'NOT FOUND (SAFE/EXPECTED)'}`);
  } catch (err: any) {
    console.log(`Lookup with NO context errored: ${err.message}`);
  }
...
verifyMitra().catch(console.error);
