...
async function cleanTestData() {
  logger.info("--- Memulai Pembersihan Data Pengujian Fase 3 ---");
  ...
  logger.info(`Ditemukan ${testTenants.length} tenant pengujian.`);

  for (const tenant of testTenants) {
    logger.info(`Membersihkan data untuk tenant: ${tenant.id}...`);
    ...
    logger.info(`✓ Tenant ${tenant.id} dan datanya berhasil dihapus.`);
  }
...
cleanTestData()
  .catch((e) => {
    logger.error("Gagal membersihkan data:", e);
    process.exit(1);
  })
  .finally(async () => {
