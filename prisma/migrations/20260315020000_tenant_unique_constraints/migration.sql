...
DO $$ BEGIN
IF NOT EXISTS (SELECT 1 FROM pg_class WHERE relname = 'roles_name_tenantId_key') THEN
...
DO $$ BEGIN
IF NOT EXISTS (SELECT 1 FROM pg_class WHERE relname = 'Permission_resource_action_tenantId_key') THEN
...
DO $$ BEGIN
IF NOT EXISTS (SELECT 1 FROM pg_class WHERE relname = 'Settings_key_tenantId_key') THEN
