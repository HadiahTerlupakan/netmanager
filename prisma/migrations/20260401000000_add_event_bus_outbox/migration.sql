...
CREATE INDEX "OutboxEvent_category_idx" ON "OutboxEvent" ("category");
CREATE INDEX "OutboxEvent_aggregateId_idx" ON "OutboxEvent" ("aggregateId");
CREATE INDEX "OutboxEvent_priority_createdAt_idx" ON "OutboxEvent" ("priority", "createdAt");

-- Composite index for the main poll query (status + priority + createdAt)
CREATE INDEX "OutboxEvent_poll_idx" ON "OutboxEvent" ("status", "priority", "createdAt")
    WHERE "status" = 'PENDING';

-- Index for cleanup of old completed events
CREATE INDEX "OutboxEvent_cleanup_idx" ON "OutboxEvent" ("status", "processedAt")
