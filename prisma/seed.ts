...
async function main() {
  ...
  // ========================================================================
  logger.info("🏢 STEP 0: Creating Main Tenant...");
  ...
  logger.info(`   ✅ Tenant: ${MAIN_TENANT_NAME} (${MAIN_TENANT_ID})`);
  ...
  logger.info(`   ✅ Created ${permissions.length} standard permissions`);
    ...
    const permission = await prisma.permission.upsert({
      ...
      create: {
        ...
        name: `${action
          ...
          .join(" ")} ${resource.charAt(0).toUpperCase() + resource.slice(1)}`,
        resource,
        action,
        tenantId: MAIN_TENANT_ID,
  ...
  logger.info(
    `   ✅ Created ${granularPermissionValues.length} granular permissions`,
  );
  ...
  logger.info(`   ✅ Role: SUPER_ADMIN (${permissions.length} permissions)`);
    ...
    update: {
      ...
      permission: {
        ...
        connect: permissions
          .filter(
            (p) =>
              !(MOBILE_RESOURCES as string[]).includes(p.resource as string),
          )
          .map((p) => ({ id: p.id })),
      },
    },
  ...
  logger.info(
    `   ✅ Role: ADMIN (${permissions.length - karyawanPermissions.length} permissions)`,
  );
  ...
  logger.info(`   ✅ Role: FINANCE (${financePermissions.length} permissions)`);
  ...
  logger.info("   ✅ Department: Technical");

  await prisma.departments.upsert({
    where: {
      name_tenantId: { name: "Customer Service", tenantId: MAIN_TENANT_ID },
    },
    update: {},
    create: {
      id: randomUUID(),
  ...
  logger.info("   ✅ Department: Customer Service");
  ...
  logger.info("   ✅ Department: Operations");
  ...
  logger.info("   ✅ Department: Finance");
  ...
  logger.info("   ✅ Department: Marketing");
  ...
  logger.info("   ✅ Position: Administrator");
  ...
  logger.info("   ✅ Position: Sales Representative");
  ...
  logger.info("   ✅ Position: Finance Officer");
  ...
  logger.info("   ✅ Gudang: GDG-PUSAT (Gudang Pusat)");
  ...
  logger.info("   ✅ Gudang: GDG-JKT01 (Gudang Jakarta Selatan)");
  ...
  const settingsData = [
    ...
    {
      key: "namaPerusahaan",
      value: "PT. Network Solutions",
      description: "Nama perusahaan",
    },
    {
      key: "alamatPerusahaan",
      value: "Jl. Utama No. 1, Jakarta",
      description: "Alamat perusahaan",
    },
    {
      key: "teleponPerusahaan",
      value: "+62-21-1234567",
    ...
    },
    {
      key: "jamMasukKerja",
      value: "09:00",
      description: "Jam masuk kerja default",
    },
    {
      key: "jamKeluarKerja",
      value: "17:00",
      description: "Jam keluar kerja default",
    },
    {
      key: "toleransiTelat",
      value: "15",
    ...
    },
    {
      ...
      description: "Hari kerja (format CSV)",
    },
  ...
  logger.info(`   ✅ Created ${settingsData.length} settings entries`);
  ...
  logger.info("   ✅ User: admin@example.com (Role: SUPER_ADMIN)");
  ...
  logger.info("   ✅ User: admin2@example.com (Role: ADMIN)");
  ...
  logger.info("   ✅ User: teknisi@example.com (Role: TEKNISI)");
  ...
  logger.info("   ✅ Mitra: mitrateknisi@example.com (Type: MITRA_TEKNISI)");
  ...
  logger.info("   ✅ Mitra: mitrasales@example.com (Type: MITRA_SALES)");
  // ========================================================================
  ...
  logger.info("   ✅ Investor: investordemo (Type: INVESTOR)");
  ...
  logger.info("   ✅ Investor: investor2 (Type: INVESTOR)");
  ...
  logger.info(`   ✅ Permissions: ${permissionCount} records`);
  ...
  logger.info(`   ✅ Roles: ${roleCount} records`);
  if (roleCount === 0) {
  ...
  for (const user of users) {
    ...
    if (!user.passwordHash) {
      logger.warn(`   ⚠️  WARNING: User ${user.email} has no passwordHash`);
    }

    // Check user has role
    if (!user.roleId) {
      logger.warn(`   ⚠️  WARNING: User ${user.email} has no roleId`);
    } else if (!user.role) {
      logger.warn(`   ⚠️  WARNING: User ${user.email} has invalid roleId`);
    } else {
  ...
  logger.info(`   Departments: ${await prisma.departments.count()}`);
  logger.info(`   Sites: ${await prisma.sites.count()}`);
  logger.info(`   Positions: ${await prisma.positions.count()}`);
  logger.info(`   Gudang: ${await prisma.gudang.count()}`);
  logger.info(`   Users: ${userCount}`);
  logger.info(`   Investors: ${await prisma.investor.count()}`);
  logger.info("\n🔑 Login Credentials:");
  logger.info("   ┌────────────────────────────────────────────────────┐");
  logger.info("   │ SUPER_ADMIN: admin@example.com / admin123         │");
  logger.info("   │ ADMIN:       admin2@example.com / admin2          │");
  logger.info("   │ TEKNISI:     teknisi@example.com / tech123        │");
  logger.info("   │ SALES:       sales@example.com / sales123         │");
  logger.info("   │ FINANCE:     finance@example.com / finance123     │");

  logger.info("   │ INVESTOR:    investordemo / investor123            │");
  logger.info("   └────────────────────────────────────────────────────┘");
  logger.info("\n📝 Portal Access:");
  logger.info("   ┌────────────────────────────────────────────────────┐");
  logger.info("   │ SUPER_ADMIN:  Admin Panel ✓  Employee Panel ✓     │");
  logger.info("   │ ADMIN:        Admin Panel ✓  Employee Panel ✗     │");
  logger.info("   │ TEKNISI:      Admin Panel ✗  Employee Panel ✓     │");
  logger.info("   │ SALES:        Admin Panel ✗  Employee Panel ✓     │");
  logger.info("   │ FINANCE:      Admin Panel ✓  Employee Panel ✗     │");
  logger.info("   │ MITRA:        Admin Panel ✗  Mitra App ✓          │");
  logger.info("   │ INVESTOR:     Admin Panel ✗  Investor Portal ✓    │");
  logger.info("   └────────────────────────────────────────────────────┘");
  ...
  const existingCash = await prisma.financialAccount.findFirst({
    where: { type: "CASH", tenantId: MAIN_TENANT_ID },
  });
  if (!existingCash) {
    ...
    logger.info("   ✅ Account: Kas Operasional");
  } else {
    logger.info("   ℹ️  Account: Kas Operasional (Already exists)");
  }

  const existingBank = await prisma.financialAccount.findFirst({
    where: { type: "BANK", tenantId: MAIN_TENANT_ID },
  });
  if (!existingBank) {
    await prisma.financialAccount.create({
      data: {
    ...
    logger.info("   ✅ Account: Bank BCA");
  } else {
    logger.info("   ℹ️  Account: Bank BCA (Already exists)");
  }
  ...
  const pppBasic = await prisma.profilePPP.upsert({
    where: {
      name_tenantId: { name: "Profile-Basic", tenantId: MAIN_TENANT_ID },
    },
    update: pppBasicData,
    create: { id: randomUUID(), name: "Profile-Basic", ...pppBasicData },
  });
  logger.info("   ✅ Profile PPP: Profile-Basic");
  ...
  const pppStandard = await prisma.profilePPP.upsert({
    where: {
      name_tenantId: { name: "Profile-Standard", tenantId: MAIN_TENANT_ID },
    },
    update: pppStandardData,
    create: { id: randomUUID(), name: "Profile-Standard", ...pppStandardData },
  });
  logger.info("   ✅ Profile PPP: Profile-Standard");
  ...
  const pppPremium = await prisma.profilePPP.upsert({
    where: {
      name_tenantId: { name: "Profile-Premium", tenantId: MAIN_TENANT_ID },
    },
  ...
  logger.info("   ✅ Harga Paket: Paket Basic 10 Mbps");
  ...
  logger.info("   ✅ Harga Paket: Paket Standard 20 Mbps");
  ...
  logger.info("   ✅ Harga Paket: Paket Premium 50 Mbps");
  ...
  logger.info("   ✅ Map Settings: Default Jakarta");
  ...
  logger.info("   ✅ Payment Gateway: Midtrans Config created");
