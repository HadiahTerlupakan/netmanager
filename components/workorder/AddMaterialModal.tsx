...
export default function AddMaterialModal({
  ...
  const handleSubmit = async () => {
    ...
    if (quantity > selectedItem.totalStock) {
      toast.error(
        `Stok tidak mencukupi (Tersedia: ${selectedItem.totalStock})`,
      );
      return;
    }
    ...
    try {
      const res = await fetch(
        `/api/admin/workorders/${workOrderId}/materials`,
        {
          method: "POST",
      ...
      clientLogger.error("Error adding material:", error);
                  ...
                  {items.map((item) => (
                    ...
                    >
                      ...
                      <div className="text-right">
                        <div
                          className={`font-semibold ${item.totalStock > 0 ? "text-gray-900 dark:text-white" : "text-red-600"}`}
                        >
            ...
            <div className="bg-gray-50 dark:bg-gray-700/50 p-4 rounded-lg flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400">
                  Kode: {selectedItem?.kode} • Stok: {selectedItem?.totalStock}{" "}
                  {selectedItem?.satuan}
                </p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-6">
              <div>
                ...
                <input
                  ...
                  value={quantity}
                  onChange={(e) =>
                    setQuantity(Math.floor(parseFloat(e.target.value) || 0))
                  }
