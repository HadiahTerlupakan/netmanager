...
export default function TemplateForm({ initialData, isEdit = false }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formData.items.length === 0)
      return toast.error("Minimal harus ada 1 item tugas");

    // Validate items
    for (const item of formData.items) {
      if (!item.title)
        return toast.error("Semua item tugas harus memiliki judul");
    }
    ...
    try {
      ...
      if (res.ok) {
        toast.success(
          isEdit ? "Template berhasil diperbarui" : "Template berhasil dibuat",
        );
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
        ...
        <div className="space-y-4">
          {formData.items.map((item, index) => (
            <div
              key={index}
              className="flex gap-4 p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg border border-gray-200 dark:border-gray-600 group"
            >
