...
export interface ResponsiveTableProps<T> {
  ...
  renderMobileCard?:
    | ((item: T, columns: Column<T>[]) => React.ReactNode)
    | undefined;
...
function DefaultMobileCard<T>({
  ...
  const otherColumns = columns.filter(
    (col) => col.priority === "secondary" || col.priority === "tertiary",
  );
          ...
          <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            <thead className="bg-gray-50 dark:bg-gray-800">
              <tr>
                {showRowNumbers && (
                  <th
                    key="header-row-number"
                    className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider w-10"
                  >
                    #
                  </th>
                )}
            ...
            <tr>
              ...
              {columns.map((column) => (
                <th
                  key={String(column.key)}
                  className={`
                    px-4 py-3.5 text-[11px] font-black uppercase tracking-[0.08em] text-gray-500 dark:text-gray-300
                ...
                >
                  <div
                    className={`flex items-center gap-1 ${column.align === "center" ? "justify-center" : column.align === "right" ? "justify-end" : "justify-start"}`}
                  >
                    {column.header}
              ...
              >
                ...
                {columns.map((column) => {
                  ...
                  return (
                    <td
                      key={String(column.key)}
                      className={`px-4 py-3.5 text-sm text-gray-900 dark:text-white ${getAlignmentClass(column.align)} ${getPriorityClasses(column.priority)} ${column.className || ""}`}
                    >
                      {safeRender(value)}
                    </td>
                  );
              ...
              >
                ...
                {(() => {
                  ...
                  let startPage = Math.max(
                    1,
                    (page || 1) - Math.floor(maxPagesToShow / 2),
                  );
