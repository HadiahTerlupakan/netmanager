            ...
            datasets: [
                {
                    ...
                    data: data,
                    backgroundColor: data.map(val => val === maxDataVal ? 'rgba(79, 70, 229, 0.8)' : 'rgba(209, 213, 219, 0.5)'), // Indigo for max
                    borderRadius: 6,
                }
            ]
        },
  ...
  return (
    <div className="w-full space-y-6">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-700 p-8 text-center relative overflow-hidden">
        ...
        <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">Cek Harga Pasar</h2>
        <p className="text-gray-500 dark:text-gray-400 mb-6 max-w-xl mx-auto">
             Cari referensi harga barang secara real-time dari marketplace untuk estimasi budget yang lebih akurat.
        </p>
        ...
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
             ...
             <div className="bg-white dark:bg-gray-800 p-5 rounded-xl border border-gray-100 dark:border-gray-700 shadow-sm hover:shadow-md transition-shadow">
                <div className="flex items-center gap-3 mb-2">
                    ...
                    <p className="text-sm font-medium text-gray-500 dark:text-gray-400">Harga Terendah</p>
                </div>
                <p className="text-xl font-bold text-gray-900 dark:text-white">{formatCurrency(result.minPrice)}</p>
             </div>
             ...
             <div className="bg-linear-to-br from-indigo-500 to-purple-600 p-5 rounded-xl shadow-lg text-white transform hover:-translate-y-1 transition-transform">
                <div className="flex items-center gap-3 mb-2">
                    ...
                    <p className="text-sm font-medium text-indigo-100">Harga Wajar*</p>
                </div>
                <p className="text-2xl font-bold text-white mb-1">{formatCurrency(analytics.weightedAvg)}</p>
                <p className="text-xs text-indigo-100/80">*Rata-rata tertimbang vol. penjualan</p>
             </div>
             ...
             <div className="bg-white dark:bg-gray-800 p-5 rounded-xl border border-gray-100 dark:border-gray-700 shadow-sm hover:shadow-md transition-shadow">
                <div className="flex items-center gap-3 mb-2">
                    ...
                    <p className="text-sm font-medium text-gray-500 dark:text-gray-400">Rata-Rata Biasa</p>
                </div>
                <p className="text-xl font-bold text-gray-900 dark:text-white">{formatCurrency(result.averagePrice)}</p>
             </div>
             ...
             <div className="bg-white dark:bg-gray-800 p-5 rounded-xl border border-gray-100 dark:border-gray-700 shadow-sm hover:shadow-md transition-shadow">
                <div className="flex items-center gap-3 mb-2">
                    ...
                    <p className="text-sm font-medium text-gray-500 dark:text-gray-400">Harga Tertinggi</p>
                </div>
                <p className="text-xl font-bold text-gray-900 dark:text-white">{formatCurrency(result.maxPrice)}</p>
             </div>
          </div>
                ...
                {analytics.bestSellerItem && analytics.bestSellerItem.soldCount > 0 && (
                    <div className="bg-linear-to-br from-amber-50 to-orange-50 dark:from-amber-900/10 dark:to-orange-900/10 p-5 rounded-xl border border-amber-100 dark:border-amber-800">
                        ...
                        <div className="flex gap-4">
                             <Image width={64} height={64} src={analytics.bestSellerItem.image} alt="Best Seller" className="rounded-lg object-cover border border-white shadow-sm" />
                             <div className="flex-1 min-w-0">
                                 ...
                                 <p className="text-sm font-semibold text-orange-600 mt-1">{analytics.bestSellerItem.priceText}</p>
                                 <p className="text-xs text-gray-500 mt-0.5">{analytics.bestSellerItem.sold} Terjual • {analytics.bestSellerItem.shopName}</p>
                             </div>
                        </div>
                    </div>
                )}
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        {analytics.recommendedShops.length > 0 ? analytics.recommendedShops.map((shop: Product) => (
                             <a key={shop.id} href={shop.url} target="_blank" rel="noopener" className="flex items-center gap-3 p-3 rounded-lg border border-gray-100 dark:border-gray-700 hover:border-indigo-300 dark:hover:border-indigo-700 hover:shadow-md transition-all group">
                                <Image width={40} height={40} src={shop.image} alt={shop.shopName} className="rounded-lg object-cover bg-gray-100" />
                                <div className="flex-1 min-w-0">
                                    ...
                                    <div className="flex items-center gap-2 text-xs text-gray-500">
                                        <span className="flex items-center text-amber-500"><HiStar className="w-3 h-3 mr-0.5" /> {shop.rating}</span>
                                        <span>•</span>
                            ...
                            <p className="text-sm text-gray-500 italic">Tidak ada rekomendasi toko yang memenuhi kriteria.</p>
                        )}
                    </div>
                </div>
           </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 shadow-sm overflow-hidden">
             <div className="px-6 py-4 border-b border-gray-100 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-900/50">
                 <h3 className="font-bold text-gray-900 dark:text-white">Detail Semua Produk ({result.products.length})</h3>
             </div>
                           ...
                           <tr key={item.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors group">
                               ...
                               <td className="px-6 py-4">
                                   <div className="flex items-start gap-4">
                                       <div className="relative shrink-0">
                                           <Image width={48} height={48} src={item.image} alt={item.name} className="object-cover rounded-lg shadow-sm" />
                                       </div>
                               ...
                               <td className="px-6 py-4 whitespace-nowrap">
                                   <div className="flex flex-col">
                                       <span className="font-bold text-gray-900 dark:text-white">{item.priceText}</span>
                                       {item.discount > 0 && (
                                           <div className="flex items-center gap-1.5 mt-0.5">
                                               <span className="text-xs bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400 px-1.5 rounded font-medium">{item.discount}%</span>
                                               <span className="text-xs text-gray-400 line-through decoration-gray-400/60">{formatCurrency(item.originalPrice)}</span>
                                           </div>
                                       )}
                                   </div>
                               </td>
                               ...
                               <td className="px-6 py-4 whitespace-nowrap">
                                   <div className="flex flex-col">
                                       <div className="flex items-center gap-1.5 font-medium text-gray-900 dark:text-gray-200">
                                           <span className="truncate max-w-[140px]" title={item.shopName}>{item.shopName}</span>
                                            {(item.badge?.includes('Official') || item.badge?.includes('Power')) && (
                                                <HiCheckBadge className="w-4 h-4 text-blue-500 shrink-0" title="Official/Power Merchant" />
                                            )}
                                       </div>
          ...
          <div className="flex justify-between items-center text-xs text-gray-400 px-2">
             <span>*Harga Wajar dihitung menggunakan rata-rata tertimbang (harga x estimasi unit terjual) untuk mengurangi bias harga outlier.</span>
