...
interface SessionsTableProps {
  ...
  onForceDelete?: (username: string) => Promise<void>;
            ...
            {[1, 2, 3, 4, 5].map((i) => (
              <tr key={i}>
                ...
                <td className="px-6 py-4 text-right">
                  <div className="h-4 bg-gray-200 dark:bg-gray-700 rounded w-24 ml-auto animate-pulse" />
                </td>
                <td className="px-6 py-4 text-right">
          ...
          {sessions.map((session) => (
            ...
            >
              ...
              <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-mono text-gray-700 dark:text-gray-300">
                {formatUsage(session.downloadMB)} /{" "}
                {formatUsage(session.uploadMB)}
              </td>
              <td className="px-6 py-4 whitespace-nowrap text-right">
                <div className="inline-flex items-center gap-2">
                  <button
                    ...
                    disabled={
                      !session.username ||
                      viewingHistoryUsername === session.username
                    }
                    onClick={() => {
                  ...
                  <button
                    ...
                    disabled={
                      !session.username ||
                      resettingUsername === session.username
                    }
                    onClick={() => {
                      if (!session.username || !onResetConnection) return;
                      if (
                        !window.confirm(
                          `Reset koneksi untuk ${session.username}?`,
                        )
                      )
                        return;
                  ...
                  <button
                    type="button"
                    disabled={
                      !session.username || deletingUsername === session.username
                    }
                    onClick={() => {
                      if (!session.username || !onForceDelete) return;
                      if (
                        !window.confirm(
                          `Hapus user ${session.username} dari RADIUS? Aksi ini akan menghapus kredensial dan menutup sesi aktif.`,
                        )
                      )
                        return;
                      void onForceDelete(session.username);
                    }}
                    className="inline-flex items-center rounded-md bg-red-600 px-3 py-1.5 text-xs font-medium text-white hover:bg-red-700 disabled:cursor-not-allowed disabled:opacity-50"
                  >
                    {deletingUsername === session.username
                      ? "Menghapus..."
                      : "Hapus"}
                  </button>
                </div>
