...
export default function GudangSelector({
  selectedIds,
  onChange,
  currentSiteId,
}: GudangSelectorProps) {
  ...
  const {
    ...
    isLoading: loading,
    error: fetchError,
  } = useApi<{ gudangs?: Gudang[] }>("/api/inventory/gudang?view=all");
  const gudangs = useMemo<Gudang[]>(() => data?.gudangs ?? [], [data?.gudangs]);
  const error = fetchError ? "Gagal memuat daftar gudang" : null;
  ...
  const paginatedGudangs = filteredGudangs.slice(
    (page - 1) * itemsPerPage,
    page * itemsPerPage,
  );
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-2">
        ...
        >
          <HiFunnel
            className={`h-4 w-4 mr-1 ${showSelectedOnly ? "fill-current" : "text-gray-400"}`}
          />
          ...
          {paginatedGudangs.length === 0 ? (
            ...
            paginatedGudangs.map((gudang) => {
              ...
              const assignedToOther =
                gudang.siteId && gudang.siteId !== currentSiteId;
                  ...
                  <div className="px-4 py-3 flex items-start">
                    ...
                    <div className="ml-3 flex-1">
                      <div className="flex justify-between">
                        <p
                          className={`text-sm font-medium ${isSelected ? "text-indigo-700 dark:text-indigo-300" : "text-gray-900 dark:text-gray-100"}`}
                        >
                          {gudang.nama}
                        </p>
