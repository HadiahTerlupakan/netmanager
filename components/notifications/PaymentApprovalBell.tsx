...
export function PaymentApprovalBell() {
  ...
  const unreadCount = useMemo(() => {
    ...
    return payments.filter(
      (p) => new Date(p.createdAt).getTime() > lastReadTimestamp,
    ).length;
  ...
  useEffect(() => {
    if (payments.length > 0) {
      ...
      if (
        prevMaxDateRef.current > 0 &&
        currentMaxDate > prevMaxDateRef.current
      ) {
        try {
            ...
            {loading ? (
              ...
              <div className="divide-y divide-gray-50 dark:divide-gray-800">
                {payments.map((payment) => {
                  const isUnread =
                    lastReadTimestamp === 0 ||
                    new Date(payment.createdAt).getTime() > lastReadTimestamp;
                  return (
                    <div
                      key={payment.id}
                      ...
                      <div className="flex gap-3">
                        <div className="shrink-0 mt-0.5">
                          <HiOutlineReceiptRefund
                            className={`w-5 h-5 ${isUnread ? "text-orange-500" : "text-gray-400"}`}
                          />
                        </div>
                        <div className="flex-1 min-w-0">
                          <div className="flex flex-col gap-0.5">
                            <p
                              className={`text-sm font-semibold truncate ${isUnread ? "text-gray-900 dark:text-white" : "text-gray-500 dark:text-gray-400"}`}
                            >
                          ...
                          <p
                            className={`text-xs mt-1 font-semibold ${isUnread ? "text-gray-700 dark:text-gray-300" : "text-gray-400 dark:text-gray-500"}`}
                          >
