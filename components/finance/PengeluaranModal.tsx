...
export default function PengeluaranModal({
  ...
  onSuccess,
  pengeluaranId,
  apiEndpoint = '/api/pengeluaran',
  token,
}: PengeluaranModalProps) {
  ...
  useEffect(() => {
    if (isOpen && isEdit && pengeluaranId) {
      loadData()
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const response = await fetch(`${apiEndpoint}/${pengeluaranId}`, {
        headers,
      })
      if (!response.ok) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = isEdit ? `${apiEndpoint}/${pengeluaranId}` : apiEndpoint
      ...
      if (!response.ok) {
        throw new Error(data.error || `Gagal ${isEdit ? 'mengupdate' : 'menambah'} pengeluaran`)
      }

      onSuccess()
      onClose()
    } catch (err: any) {
      setError(err.message || `Terjadi kesalahan saat ${isEdit ? 'mengupdate' : 'menambah'} pengeluaran`)
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <Modal open={isOpen} onClose={onClose} title={isEdit ? 'Edit Pengeluaran' : 'Tambah Pengeluaran'}>
