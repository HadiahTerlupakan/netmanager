        ...
        >
          ...
          <div className="bg-gradient-to-r from-emerald-400 to-teal-500 text-white p-4">
            <div className="flex items-center justify-between mb-4">
              ...
              <button
                onClick={() => setIsOpen(false)}
                className="touch-target touch-manipulation p-2 hover:bg-white/10 active:bg-white/20 rounded-lg transition-colors md:hidden"
                  ...
                  <div key={index}>
                    {item.href ? (
                      <Link
                        ...
                        className={`touch-target touch-manipulation flex items-center gap-3 px-3 md:px-4 py-3 rounded-lg transition-all active:scale-95 ${isActive
                      ...
                      <div>
                        ...
                        <button
                          ...
                          className={`touch-target touch-manipulation w-full flex items-center justify-between gap-3 px-3 md:px-4 py-3 rounded-lg transition-all active:scale-95 ${hasActiveSubmenu
                            ? 'bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400 font-medium'
                            : 'text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800 active:bg-gray-200 dark:active:bg-gray-700'
                            }`}
                        >
                        ...
                        {item.submenu && openMenus[item.label] && (
                          <div className="ml-6 md:ml-8 mt-1 space-y-1 animate-in slide-in-from-top-2 duration-200">
                            {item.submenu.map((subItem, subIndex) => (
                              <Link
                                ...
                                className={`touch-target touch-manipulation block px-3 md:px-4 py-2.5 md:py-2 text-sm rounded-lg transition-all active:scale-95 ${pathname === subItem.href
                                  ? 'bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400 font-medium'
          ...
          {/* Footer */}
          <div className="p-3 md:p-4 border-t border-gray-200 safe-area-inset-bottom">
            <button
              onClick={handleLogout}
              className="touch-target touch-manipulation w-full flex items-center justify-center gap-3 px-3 md:px-4 py-3 rounded-lg text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 active:bg-red-100 dark:active:bg-red-900/30 transition-colors active:scale-95"
            >
