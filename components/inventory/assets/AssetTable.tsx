...
export function AssetTable() {
  ...
  const columns: Column<Asset>[] = [
    {
        ...
        render: (item) => (
            <div className="flex flex-col">
                <span className="font-medium text-gray-900 dark:text-white">{item.kodeAsset}</span>
            </div>
        )
    },
    {
    ...
    },
    {
        ...
        render: (item) => (
            <span className={`inline-flex px-2 py-1 text-xs font-medium rounded-full ${
                ...
                item.status === 'INSTALLED' ? 'bg-blue-100 text-blue-800' :
  ...
  const renderActions = (item: Asset) => (
      <div className="flex gap-2">
          <Link href={`/admin/inventory/assets/${item.id}`} className="p-2 text-blue-600 hover:bg-blue-50 rounded">
              <FiEye />
          </Link>
