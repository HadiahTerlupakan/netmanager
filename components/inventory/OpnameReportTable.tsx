...
export function OpnameReportTable({
  ...
  const akurasiStok =
    totalItems > 0
      ? Math.round(
          (opnameList.filter((item) => item.selisih === 0).length /
            totalItems) *
            100,
        )
      : 100;
  const totalBaik = opnameList.reduce((sum, item) => sum + item.kondisiBaik, 0);
  const totalRusak = opnameList.reduce(
    (sum, item) => sum + item.kondisiRusak,
    0,
  );
  ...
  const handleDelete = async (id: string) => {
    ...
    if (
      !confirm("Apakah Anda yakin ingin menghapus record stock opname ini?")
    ) {
      return;
    }
  ...
  const handleExportCSV = () => {
    ...
    link.setAttribute(
      "download",
      `stock_opname_${new Date().toISOString().split("T")[0]}.csv`,
    );
      ...
      render: (item) => {
        ...
        return (
          <span className="text-sm text-gray-900 dark:text-white">
            {item.alasanSelisih
              ? alasanLabels[item.alasanSelisih] || item.alasanSelisih
              : "-"}
          </span>
        );
      },
  ...
  const renderActions = (opname: StockOpnameRecord) => (
    <div className="flex justify-center space-x-2">
      ...
      <Button
        onClick={() => {
          clientLogger.info("Delete clicked for ID:", opname.id);
