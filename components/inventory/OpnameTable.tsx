...
export function OpnameTable({
  ...
  const fetchOpnameList = useCallback(async () => {
    ...
    try {
      ...
      const response = await getWithAuth(
        `/api/inventory/opname/list?${params}`,
      );
      if (!response.ok) {
      ...
      clientLogger.error("Error fetching opname list:", error);
  ...
  const handleDelete = async (id: string) => {
    if (
      !confirm(
        "Apakah Anda yakin ingin menghapus record stock opname ini? Stok akan dikembalikan ke nilai sistem sebelum opname.",
      )
    ) {
      return;
    }
    ...
    try {
      ...
      clientLogger.error("Error deleting stock opname:", error);
      alert(
        error instanceof Error ? error.message : "Gagal menghapus stock opname",
      );
    } finally {
      setDeletingId(null);
    }
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setFilters({ ...filters, barangId: e.target.value })
              }
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setFilters({ ...filters, gudangId: e.target.value })
              }
              className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white"
            >
          ...
          <div className="md:col-span-2 flex items-end">
            <Button
              onClick={() =>
                setFilters({ barangId: "", gudangId: "", search: "" })
              }
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow overflow-hidden">
        ...
        {error ? (
          <div className="text-center py-8">
            ...
            <Button onClick={fetchOpnameList} className="mt-2">
              Retry
            </Button>
          </div>
        ) : (
