  ...
  const fetchCalculatedData = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `/api/inventory/opname/calculate?gudangId=${gudangId}`,
      );
      if (!response.ok) {
      ...
      clientLogger.error("Error fetching calculated data:", error);
      setError(
  ...
  const handleRecordOpname = async (e: React.FormEvent) => {
    ...
    try {
      const promises = itemsToRecord.map(async (item) => {
        ...
        const alasanLabel =
          ALASAN_SELISIH_OPTIONS.find((o) => o.value === item.alasanSelisih)
            ?.label || "";
      ...
      setSuccess(
        `Stock opname berhasil dicatat untuk ${itemsToRecord.length} item!`,
      );
      ...
      clientLogger.error("Error recording stock opname:", error);
          ...
          {selectedGudang && (
            <div className="bg-blue-50 dark:bg-blue-900/20 rounded-lg p-4">
              <p className="text-sm font-medium text-blue-900 dark:text-blue-300 flex items-center gap-2">
                <FiBarChart2 className="w-4 h-4" /> Input Stock Opname - Stok
                Fisik
              </p>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
            ...
            <div className="mt-4 p-4 bg-yellow-50 dark:bg-yellow-900/20 rounded-lg">
              <p className="text-sm text-yellow-800 dark:text-yellow-200">
                <strong>Instruksi:</strong> Bandingkan stok sistem dengan stok
                fisik yang Anda hitung. Input jumlah stok fisik yang sebenarnya
                dan breakdown kondisi aktual di lapangan.
              </p>
            </div>
          </div>
        )}
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow overflow-hidden">
            <div className="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400">
                Bandingkan stok sistem dengan hasil hitungan fisik di{" "}
                {selectedGudang?.nama}
              </p>
            </div>
                ...
                <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                  {calculatedData.map((item) => {
                    ...
                    return (
                      <tr
                        key={item.barangId}
                        className={`${needsRecording ? "bg-yellow-50 dark:bg-yellow-900/10" : ""}`}
                      >
            ...
            <div className="px-6 py-4 bg-gray-50 dark:bg-gray-700 border-t border-gray-200 dark:border-gray-700">
              <div className="flex items-center justify-between">
                <div className="text-sm text-gray-500 dark:text-gray-400">
                  Total barang: {calculatedData.length} | Item dengan perbedaan:{" "}
                  {itemsWithDiscrepancies.length}
                </div>
                <div className="text-sm">
                  <span
                    className={`font-medium ${itemsWithDiscrepancies.length > 0 ? "text-indigo-600" : "text-green-600"}`}
                  >
                    {itemsWithDiscrepancies.length > 0
                      ? `${itemsWithDiscrepancies.length} item perlu dicatat`
                      : "Tidak ada perbedaan"}
                  </span>
                </div>
              </div>
        ...
        {gudangId && !fetching && calculatedData.length > 0 && (
          <div className="flex justify-end space-x-3 pt-6 border-t border-gray-200 dark:border-gray-700">
            ...
            >
              {loading
                ? "Mencatat..."
                : `Catat Stock Opname (${itemsWithDiscrepancies.length} item dengan selisih)`}
            </Button>
          </div>
        )}
      </form>
