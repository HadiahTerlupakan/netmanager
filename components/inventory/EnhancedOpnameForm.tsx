...
export function EnhancedOpnameForm({
  onClose,
  onSuccess,
  defaultGudangId,
    ...
    async function fetchBarangList() {
      if (gudangId) {
        ...
        try {
          ...
          clientLogger.error("Error fetching barang list:", error);
  ...
  const validateOpnameData = (barangId: string): string[] => {
    ...
    if (totalKondisi !== stokFisik) {
      errors.push(
        `Total kondisi (${totalKondisi}) harus sama dengan stok fisik (${stokFisik})`,
      );
    }
    ...
    if (
      data.kelembaban &&
      (parseFloat(data.kelembaban) < 0 || parseFloat(data.kelembaban) > 100)
    ) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const promises = Object.entries(opnameData).map(
        async ([barangId, data]) => {
          ...
          const response = await fetch("/api/inventory/opname", {
            ...
            body: JSON.stringify({
              ...
              suhuPenyimpanan: data.suhuPenyimpanan
                ? parseFloat(data.suhuPenyimpanan)
                : undefined,
              kelembaban: data.kelembaban
                ? parseFloat(data.kelembaban)
                : undefined,
          ...
          if (!response.ok) {
            ...
            throw new Error(
              responseData.error || "Gagal mencatat stock opname",
            );
          }
      ...
      clientLogger.error("Error submitting stock opname:", error);
  ...
  const hasChanges = barangList.some((barang) => {
    const currentStok =
      barang.stockPerGudang?.find(
        (s: { gudangId: string; stok: number }) => s.gudangId === gudangId,
      )?.stok || 0;
                      ...
                      >
                        ...
                        <td className="px-2 py-3 text-center">
                          <div className="flex flex-col items-center space-y-1">
                            <span
                              className={`font-bold text-sm ${newStok > 0 ? "text-blue-600" : "text-red-600"}`}
                            >
                              {newStok}
                            </span>
                            {diff !== 0 && (
            ...
            <div className="px-6 py-4 border-t border-gray-200 dark:border-gray-700">
              <div className="flex items-center justify-between">
                ...
                <div className="text-sm">
                  ...
                  <span
                    className={`font-medium ${hasChanges ? "text-indigo-600" : "text-green-600"}`}
                  >
