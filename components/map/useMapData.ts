...
import { calculateMapDistance, generateMapId } from '@/components/map/map-utils'
...
export function useMapData({ showToast }: UseMapDataParams) {
  ...
  const [statistics, setStatistics] = useState<MapStatistics>(DEFAULT_STATISTICS)
  ...
  const updateNodePosition = useCallback(async (nodeId: string, position: [number, number]) => {
    try {
  ...
  const saveNode = useCallback(async (data: Partial<MappingNode>, editingNodeId?: string) => {
    try {
      const res = await fetch(editingNodeId ? `/api/map/nodes/${editingNodeId}` : '/api/map/nodes', {
  ...
  const saveFiberLine = useCallback(async (fiberFormData: FiberFormData | null, formData: SaveFiberLineFormData) => {
    if (!fiberFormData) {
      return false
    }

    try {
      const sourceNode = nodes.find((node) => node.nodeId === fiberFormData.source)
      const targetNode = nodes.find((node) => node.nodeId === fiberFormData.target)
  ...
  const confirmDelete = useCallback(async (deleteConfirmation: DeleteConfirmation) => {
  ...
  const saveSettings = useCallback(async (data: Partial<MapSettings>) => {
  ...
  const resetMap = useCallback(async (password: string) => {
