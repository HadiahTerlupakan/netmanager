...
type ToolbarTool = 'server' | 'odc' | 'odp' | 'ont' | 'pole' | 'joinbox' | 'fiber'
    ...
    <>
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-1 bg-gray-100 dark:bg-gray-700 rounded-lg p-1">
              <Button variant="ghost" onClick={() => onTabChange('map')}
              ...
              <Button variant="ghost" onClick={() => onTabChange('list')}
              ...
              <Button variant="ghost" onClick={() => onTabChange('settings')}
          ...
          <div className="flex items-center gap-2">
            <div className="relative z-20">
              <Button onClick={onToggleSearchDropdown} className={toolButtonClass(showSearchDropdown)}>
                  ...
                  {searchQuery && (
                    <div className="max-h-64 overflow-y-auto border-t border-gray-200 dark:border-gray-700">
                      {filteredNodes.slice(0, 10).map((node) => (
                        ...
                        >
                          <span className="w-3 h-3 rounded-full" style={{ backgroundColor: getNodeColor(node.type) }} />
            ...
            <Button onClick={() => onToolbarClick('server')} className={toolButtonClass(serverActionMode !== 'idle')}>
              <HiServer className="w-4 h-4" />
              Server
            </Button>
            <Button onClick={() => onToolbarClick('odc')} className={toolButtonClass(odcActionMode !== 'idle')}>
              <HiCube className="w-4 h-4" />
              ODC
            </Button>
            <Button onClick={() => onToolbarClick('odp')} className={toolButtonClass(odpActionMode !== 'idle')}>
              <HiSquare3Stack3D className="w-4 h-4" />
              ODP
            </Button>
            <Button onClick={() => onToolbarClick('ont')} className={toolButtonClass(ontActionMode !== 'idle')}>
              <HiCpuChip className="w-4 h-4" />
              ONT
            </Button>
            <Button onClick={() => onToolbarClick('pole')} className={toolButtonClass(poleActionMode !== 'idle')}>
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 3v18M8 6h8M8 10h8M8 14h8" />
              </svg>
              Pole
            </Button>
            <Button onClick={() => onToolbarClick('joinbox')} className={toolButtonClass(joinboxActionMode !== 'idle')}>
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <Button onClick={() => onToolbarClick('fiber')} className={toolButtonClass(fiberLineMode !== 'idle')}>
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {isAnyModeActive && (
        <div className="bg-blue-500 text-white px-4 py-2 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            {hasPendingTempPosition && (
              <Button onClick={onSaveActiveTempPosition}
                className="px-3 py-1 bg-green-500 hover:bg-green-600 rounded text-sm font-medium"
              >
                Save
              </Button>
            )}
            <Button onClick={onCancelActiveMode} className="p-1 hover:bg-blue-600 rounded">
