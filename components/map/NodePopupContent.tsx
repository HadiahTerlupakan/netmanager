...
function getNodeBadge(type: string) {
  return NODE_BADGES[type] || { label: type.toUpperCase(), color: 'bg-gray-500' }
}
...
export function NodePopupContent({
  ...
  const showConnectedTo = connectedToNodes.length > 0 && (isServerOrOlt(node.type) || node.type === 'odc')
  ...
  return (
    <div className="min-w-[240px] max-w-[280px]">
      ...
      <Button variant="secondary" size="sm" onClick={onCopyInfo} className="mb-3">
        ...
        <div className="border-t border-gray-200 pt-3 mb-3">
          ...
          <div className="grid grid-cols-2 gap-2 text-xs">
            <div>
              ...
              <span className="font-medium text-gray-900">{node.attenuationIn !== null && node.attenuationIn !== undefined ? `${node.attenuationIn} dBm` : '-'}</span>
            </div>
            <div>
              <span className="text-gray-500 block">Output Redaman:</span>
              <span className="font-medium text-gray-900">{node.attenuationOut !== null && node.attenuationOut !== undefined ? `${node.attenuationOut} dBm` : '-'}</span>
            </div>
        ...
        <div className="border-t border-gray-200 pt-3 mb-3">
          ...
          <div className="space-y-1">
            {connectedFromNodes.map((connectedNode) => (
              <div key={connectedNode.nodeId} className="flex items-center gap-2 text-sm">
                <span
                  className="w-2 h-2 rounded-full"
                  style={{ backgroundColor: NODE_COLORS[connectedNode.type] || '#6b7280' }}
                />
      ...
      {showConnectedTo && (
        <div className="border-t border-gray-200 pt-3 mb-3">
          <p className="text-sm font-medium text-gray-700 mb-1">
            {isServerOrOlt(node.type) ? 'Connected to ODCs:' : 'Connected to ODPs:'}
          </p>
          <div className="space-y-1">
