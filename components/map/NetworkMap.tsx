...
export default function NetworkMap() {
  const { data: settingsData, fetchData: fetchSettings } = useApiResponse<MapSettings>();
  const { data: nodesData, fetchData: fetchNodes } = useApiResponse<MappingNode[]>();
  const { data: edgesData, fetchData: fetchEdges } = useApiResponse<MappingEdge[]>();
  ...
  const centerPosition: [number, number] = config?.centerLat && config?.centerLng
    ...
    <div className="relative w-full h-[calc(100vh-64px)] z-0">
      ...
      >
        ...
        {edges.map((edge) => {
             ...
             if (!sourceNode?.latitude || !sourceNode?.longitude || !targetNode?.latitude || !targetNode?.longitude) return null;
                    ...
                    <Popup>
                        <div className="min-w-[200px]">
                            ...
                            <div className="grid grid-cols-2 gap-2 text-sm">
                                ...
                                <span className="font-mono bg-gray-100 px-1 rounded">{node.type.toUpperCase()}</span>
