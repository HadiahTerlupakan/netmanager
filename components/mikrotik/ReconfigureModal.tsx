...
export default function ReconfigureModal({ open, onClose, onSuccess }: ReconfigureModalProps) {
        ...
        <div className="space-y-4">
            ...
            <div className="max-h-[300px] overflow-y-auto border rounded-lg divide-y divide-gray-100 dark:divide-gray-800">
                {loading ? (
                    ...
                    <div className="p-4 text-center text-sm text-gray-500">Tidak ada router aktif</div>
                ) : (
                    routers.map(router => (
                        <label key={router.id} className="flex items-center p-3 hover:bg-gray-50 dark:hover:bg-gray-800/50 cursor-pointer">
                            <input 
                                type="checkbox"
                            ...
                            <div className="ml-3 flex-1">
                                <div className="flex items-center justify-between">
                                    <span className="text-sm font-medium text-gray-900 dark:text-gray-100">{router.name}</span>
                                ...
                                <span className="text-xs text-gray-500">{router.ipAddress}</span>
                            </div>
                        </label>
                    ))
                )}
            ...
            <div className="bg-yellow-50 dark:bg-yellow-900/20 p-3 rounded-md flex gap-2 text-yellow-700 dark:text-yellow-400 text-xs">
                ...
                <p>
                    Proses ini akan mengirim ulang konfigurasi RADIUS, Firewall Filter, IP Pool, dan Profile PPP ke router yang dipilih. 
                    Koneksi user <strong>tidak akan terputus</strong>, namun router mungkin akan mengalami spike CPU sesaat.
                </p>
            </div>
        </div>
    )
    ...
    const renderProcessing = () => (
        <div className="flex flex-col items-center justify-center py-12">
            ...
            <h3 className="text-lg font-semibold text-gray-900 dark:text-white">Sedang Memproses...</h3>
            <p className="text-sm text-gray-500 mt-2">Mohon tunggu, sedang mengirim konfigurasi ke router.</p>
        </div>
    )
        ...
        return (
            <div className="space-y-4">
                <div className={`p-4 rounded-lg flex items-center gap-3 ${
                    ...
                    {successCount === results.length ? <HiCheck className="w-6 h-6" /> : <HiExclamationTriangle className="w-6 h-6" />}
                    <div>
                        <p className="font-semibold">Proses Selesai</p>
                        <p className="text-sm">Berhasil: {successCount}, Gagal: {results.length - successCount}</p>
                    </div>
                </div>
    ...
    return (
        <Modal
            isOpen={open}
            onClose={onClose}
            title={step === 'result' ? 'Hasil Reconfigurasi' : 'Reconfigurasi Mikrotik'}
            size="lg"
        >
            <div className="p-6">
