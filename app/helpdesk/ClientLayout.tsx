...
const navigation = [
    ...
    { name: 'Tickets', href: '/helpdesk/tickets', icon: HiOutlineTicket },
]

// Allowed departments for Helpdesk access
const ALLOWED_DEPARTMENTS = ['IT', 'Support', 'Teknisi', 'Technical Support']

function HelpdeskContent({ children }: { children: React.ReactNode }) {
    ...
    // Render special pages without layout (no auth needed)
    if (pathname === '/helpdesk/login' || pathname === '/helpdesk/access-denied') {
        return <>{children}</>
    }

    // Access Control: Check if user has access to Helpdesk
    useEffect(() => {
        ...
        if (status === 'unauthenticated') {
            ...
            // Check if employee is from allowed department
            if (!departmentName || !ALLOWED_DEPARTMENTS.includes(departmentName)) {
                console.log('[HELPDESK] Access denied for department:', departmentName)
                router.push('/helpdesk/access-denied')
            }
        }
            ...
            <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
                ...
                <div className="lg:hidden bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 sticky top-0 z-20 px-4 py-3 flex items-center justify-between shadow-sm">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="font-bold text-gray-900 dark:text-white">Helpdesk</span>
                    </div>
                    <button
                    ...
                    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                        <div className="flex items-center justify-between h-16">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <h1 className="text-lg font-bold text-gray-900 dark:text-white">Helpdesk Portal</h1>
                                    <p className="text-xs text-gray-500 dark:text-gray-400">{employee?.fullName}</p>
                                </div>
                            </div>
                            <button
                                onClick={handleSignOut}
...
export default function ClientLayout({ children }: { children: React.ReactNode }) {
    return (
        <SessionProvider>
            <HelpdeskContent>{children}</HelpdeskContent>
        </SessionProvider>
    )
}
