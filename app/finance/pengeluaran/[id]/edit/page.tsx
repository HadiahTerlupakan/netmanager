...
export default function FinancePengeluaranEditPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const loadData = async (pengeluaranId: string) => {
    try {
      ...
      const response = await fetch(`/api/finance/pengeluaran/${pengeluaranId}`, {
        headers: {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const token = localStorage.getItem('finance_token')
      const response = await fetch(`/api/finance/pengeluaran/${id}`, {
        method: 'PUT',
        headers: {
        ...
        <div className="px-4 py-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <h1 className="text-xl font-bold">Edit Pengeluaran</h1>
            </div>
            <Link
