...
// Quick action definitions with feature requirements
const quickActionDefinitions = [
    { name: 'Absensi', href: '/employee/attendance', icon: HiOutlineClock, feature: 'EMPLOYEE.ABSENSI', colorClass: 'indigo' },
    { name: 'Ajukan Cuti', href: '/employee/leaves', icon: HiOutlineCalendar, feature: 'EMPLOYEE.CUTI', colorClass: 'purple' },
    { name: 'Slip Gaji', href: '/employee/payslips', icon: HiOutlineBanknotes, feature: 'EMPLOYEE.PAYSLIPS', colorClass: 'green' },
    { name: 'Profil Saya', href: '/employee/profile', icon: HiOutlineUser, feature: 'EMPLOYEE.PROFILE', colorClass: 'blue' },
    { name: 'Gudang', href: '/employee/inventory', icon: HiOutlineCube, feature: 'EMPLOYEE.INVENTORY', colorClass: 'orange' },
    { name: 'Work Orders', href: '/employee/workorders', icon: HiOutlineWrench, feature: 'EMPLOYEE.WORKORDERS', colorClass: 'red' },
]
...
export default function EmployeeDashboard() {
    ...
    const filteredQuickActions = useMemo(() => {
        return quickActionDefinitions.filter(action => {
    ...
    const canViewAttendance = hasFeature('EMPLOYEE.ABSENSI')
    const canViewLeave = hasFeature('EMPLOYEE.CUTI')
    const canViewPayslip = hasFeature('EMPLOYEE.PAYSLIPS')
        ...
        <div className="space-y-6 sm:space-y-8 fade-in">
            ...
            <div className="relative overflow-hidden rounded-3xl shadow-2xl">
                ...
                <div className="relative p-6 sm:p-8 lg:p-10 text-white">
                    ...
                    {canViewAttendance && stats && stats.attendance.thisMonth > 0 && (
                ...
                <>
                    ...
                    {filteredQuickActions.length > 0 && (
                        <div className="bg-white/90 dark:bg-gray-800/90 backdrop-blur-sm rounded-2xl shadow-lg border border-gray-100 dark:border-gray-700 p-6 sm:p-8">
                            ...
                            <div className={`grid gap-4 ${filteredQuickActions.length <= 4 ? `grid-cols-2 lg:grid-cols-${filteredQuickActions.length}` : 'grid-cols-2 lg:grid-cols-4'}`}>
                                {filteredQuickActions.map((action) => {
                                    const colorMap: Record<string, { gradient: string, hover: string }> = {
                                        indigo: { gradient: 'from-indigo-500 to-indigo-600', hover: 'hover:border-indigo-500' },
                                        purple: { gradient: 'from-purple-500 to-purple-600', hover: 'hover:border-purple-500' },
                                        green: { gradient: 'from-green-500 to-green-600', hover: 'hover:border-green-500' },
                                        blue: { gradient: 'from-blue-500 to-blue-600', hover: 'hover:border-blue-500' },
                                        orange: { gradient: 'from-orange-500 to-orange-600', hover: 'hover:border-orange-500' },
                                        red: { gradient: 'from-red-500 to-red-600', hover: 'hover:border-red-500' },
                                    }
                                    const colors = colorMap[action.colorClass] || colorMap.indigo

                                    return (
                                        <Link
                                            key={action.href}
                                            href={action.href}
                                            className={`group flex flex-col items-center gap-4 p-6 sm:p-5 border-2 border-gray-200 dark:border-gray-700 rounded-2xl ${colors.hover} hover:shadow-lg transition-all ripple touch-manipulation min-h-[130px] sm:min-h-[140px] bg-gradient-to-br from-white to-gray-50 dark:from-gray-800 dark:to-gray-800/50`}
                                        >
                                            <div className={`w-14 h-14 flex items-center justify-center rounded-2xl bg-gradient-to-br ${colors.gradient} shadow-lg group-hover:scale-110 transition-transform`}>
                                                <action.icon className="w-7 h-7 text-white" />
                                            </div>
                                            <span className="text-sm sm:text-base font-semibold text-gray-900 dark:text-white text-center">{action.name}</span>
                                        </Link>
                                    )
                                })}
                            </div>
