...
import { HiOutlineArrowRightOnRectangle, HiOutlineUserCircle, HiOutlineExclamationCircle } from 'react-icons/hi2'
import InvestorBottomNav from '../components/InvestorBottomNav'
import toast from 'react-hot-toast'

export default function InvestorProfile() {
    const router = useRouter()
    const [user, setUser] = useState<{ username?: string; namaLengkap?: string } | null>(null)
    ...
    return (
        <div className="flex-1 flex flex-col min-h-screen bg-gray-50 dark:bg-black pb-24 overflow-y-auto">
            ...
            <div className="bg-gradient-to-b from-indigo-700 to-indigo-900 pb-16 pt-12 px-6 rounded-b-[40px] shadow-lg relative flex flex-col items-center">
                ...
                <h1 className="text-2xl font-black text-white text-center">{user?.namaLengkap || 'Investor'}</h1>
                <p className="text-indigo-200 text-sm font-medium mt-1">Akun Investor</p>
            </div>
            ...
            <div className="px-5 -mt-6 relative z-10 space-y-4">
                <div className="bg-white dark:bg-neutral-900 rounded-3xl p-6 shadow-xl shadow-gray-200/40 dark:shadow-none border border-gray-100 dark:border-neutral-800">
                    <h3 className="text-sm font-bold text-gray-900 dark:text-white mb-5 uppercase tracking-wider">Informasi Akun</h3>
                    ...
                    <div className="space-y-4">
                        <div className="flex flex-col border-b border-gray-50 dark:border-neutral-800 pb-3">
                            ...
                            <span className="text-base font-bold text-gray-900 dark:text-white">{user?.username || '-'}</span>
                        </div>
                        <div className="flex flex-col border-b border-gray-50 dark:border-neutral-800 pb-3">
                            <span className="text-xs text-gray-500 font-medium">Nama Lengkap</span>
                            <span className="text-base font-bold text-gray-900 dark:text-white">{user?.namaLengkap || '-'}</span>
                        </div>
                        <div className="flex flex-col">
                            <span className="text-xs text-gray-500 font-medium">Status</span>
                            <span className="text-sm font-black text-green-500 bg-green-50 dark:bg-green-900/20 px-3 py-1 rounded-full w-max mt-1">Aktif</span>
                        </div>
                    </div>
                </div>
                ...
                <div className="bg-white dark:bg-neutral-900 rounded-3xl p-2 shadow-xl shadow-gray-200/40 dark:shadow-none border border-gray-100 dark:border-neutral-800">
                    ...
                    >
                        ...
                        <div className="text-left flex-1">
                            <h4 className="text-sm font-bold text-gray-900 dark:text-white group-hover:text-red-600 transition-colors">Keluar Aplikasi</h4>
                            <p className="text-[10px] sm:text-xs text-gray-500">Akhiri sesi Anda saat ini</p>
                        </div>
                    </button>
                </div>
                ...
                <div className="flex gap-2 items-center justify-center pt-8 text-gray-400">
                    ...
                    <span className="text-xs font-medium">Hubungi Admin jika ada kesalahan data</span>
                </div>
            </div>
