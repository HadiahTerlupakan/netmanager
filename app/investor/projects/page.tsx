      ...
      <div className="p-4 space-y-4">
        {projects.map((item) => {
          const p = item as {
            ...
            totalActualRevenue: string | number;
            totalActualOpex?: string | number;
          ...
          const getStatusUI = (status: string) => {
            switch (status) {
              ...
              case "SELESAI":
                return {
                  color:
                    "text-white bg-gray-800 dark:bg-white dark:text-gray-900",
                  icon: HiOutlineCheckCircle,
                };
              default:
                return {
                ...
                <div className="flex justify-between items-start mb-4">
                  <div className="pr-4">
                    ...
                    <p className="text-xs text-gray-500 flex items-center gap-1">
                      ...
                      {p.siteName || "Lokasi Global"}
                    </p>
                  </div>
                  <div
                    className={`px-2 py-1 rounded-lg text-[10px] font-black uppercase tracking-wider flex items-center gap-1 ${ui.color}`}
                  >
                ...
                <div className="grid grid-cols-2 gap-4 mt-6 pt-4 border-t border-gray-50 dark:border-neutral-800">
                  <div>
                    ...
                    <p className="text-sm font-black text-gray-900 dark:text-white">
                      {formatCurrency(Number(p.investmentAmount))}
                    </p>
                  </div>
                  <div>
                    <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1">
                    ...
                    <p className="text-sm font-black text-blue-600 dark:text-blue-400">
                      {formatCurrency(
                        Math.max(
                          0,
                          Number(p.totalActualRevenue) -
                            Number(p.totalActualOpex || 0),
                        ) *
                          (Number(p.profitSharePercent) / 100),
                      )}
                    </p>
                  </div>
                </div>
        ...
        {projects.length === 0 && !isLoading && (
          <div className="text-center py-20 px-6">
            ...
            <p className="text-sm text-gray-500 leading-relaxed">
              Anda belum terlibat dalam pendanaan proyek apapun saat ini.
            </p>
          </div>
        )}
      </div>
