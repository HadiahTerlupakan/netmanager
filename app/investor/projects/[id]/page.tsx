...
import { HiOutlineArrowLeft, HiOutlineBriefcase, HiOutlineChartBar, HiOutlineCurrencyDollar, HiOutlineUsers } from 'react-icons/hi2'
...
export default function InvestorProjectDetail() {
    ...
    if (!project) {
        return (
            <div className="flex-1 flex flex-col min-h-screen bg-gray-50 dark:bg-black items-center justify-center p-6 text-center">
                ...
                <h3 className="text-lg font-bold text-gray-900 dark:text-white">Proyek Tidak Ditemukan</h3>
                <button onClick={() => router.back()} className="mt-4 px-6 py-2 bg-gray-200 dark:bg-neutral-800 rounded-full text-sm font-bold">Kembali</button>
            </div>
        )
    }
    ...
    const achievements = project.actualAchievements as Record<string, unknown>[] || []
    achievements.forEach((ach) => {
        const netActual = Math.max(0, Number(ach.achievedRevenue) - Number(ach.opex || 0))
        totalActualProfitShare += netActual * (Number(project.profitSharePercent) / 100)
    })

    // Calculate average actual profit per month (if there are achievements)
    const averageActualProfitShare = achievements.length > 0 ? totalActualProfitShare / achievements.length : 0
            ...
            <div className="p-4 space-y-6">
                ...
                <div className="grid grid-cols-2 gap-3">
                    <div className="bg-gradient-to-br from-blue-600 to-indigo-700 p-4 rounded-3xl text-white shadow-xl shadow-blue-500/20 relative overflow-hidden">
                        ...
                        <p className="text-[10px] font-bold text-blue-100 uppercase tracking-wider mb-1 relative z-10">Nilai Investasi</p>
                        <p className="text-base font-black relative z-10">{formatCurrency(Number(project.investmentAmount))}</p>
                    </div>
                    <div className="bg-white dark:bg-neutral-900 border border-gray-100 dark:border-neutral-800 p-4 rounded-3xl shadow-sm">
                        <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1">Bagi Hasil</p>
                        <p className="text-xl font-black text-gray-900 dark:text-white">{String(project.profitSharePercent || 0)}%</p>
                    </div>
                </div>
                ...
                <div className="bg-white dark:bg-neutral-900 rounded-3xl p-5 shadow-sm border border-gray-100 dark:border-neutral-800 line-clamp-2">
                    <div className="flex items-center gap-3 mb-4">
                        ...
                        <h3 className="text-sm font-bold text-gray-900 dark:text-white">Performa Keuangan</h3>
                    </div>
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="pt-4 border-t border-gray-50 dark:border-neutral-800">
                            <div className="flex justify-between text-xs mb-1">
                                <span className="text-green-600 dark:text-green-400 font-bold uppercase tracking-wide">Total Profit Aktual (Akumulasi)</span>
                                <span className="font-black text-green-600 dark:text-green-400">{formatCurrency(totalActualProfitShare)}</span>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="bg-white dark:bg-neutral-900 rounded-3xl p-5 shadow-sm border border-gray-100 dark:border-neutral-800 line-clamp-2">
                    <div className="flex items-center justify-between mb-5">
                        <div className="flex items-center gap-3">
                            ...
                            <h3 className="text-sm font-bold text-gray-900 dark:text-white">Pelanggan Proyek</h3>
                        </div>
                        <div className="text-right">
                            <p className="text-[10px] font-bold text-gray-400 uppercase">Target</p>
                            <p className="text-sm font-black text-gray-900 dark:text-white">{String(project.targetSubscribers || 0)}</p>
                        </div>
                    </div>
                    ...
                    <div className="space-y-4">
                        <div className="grid grid-cols-2 gap-4">
                            <div className="p-3 bg-gray-50 dark:bg-neutral-800 rounded-xl">
                                <p className="text-[10px] text-gray-500 font-bold uppercase mb-1">Aktif & Lunas</p>
                                <p className="text-lg font-black text-green-600 dark:text-green-400">{subs?.paying || 0} <span className="text-xs text-gray-400 font-medium">PLG</span></p>
                            </div>
                            <div className="p-3 bg-gray-50 dark:bg-neutral-800 rounded-xl">
                                <p className="text-[10px] text-gray-500 font-bold uppercase mb-1">Subscribers Total</p>
                                <p className="text-lg font-black text-gray-900 dark:text-white">{subs?.total || 0} <span className="text-xs text-gray-400 font-medium">PLG</span></p>
                            </div>
                        </div>

                        <div className="pt-2">
                            <div className="flex justify-between text-xs font-bold mb-2">
                                <span className="text-gray-500">Kesehatan Tagihan (Payment Ratio)</span>
                ...
                {/* History Record */}
                <div className="pt-4">
                    <h3 className="text-sm font-bold text-gray-900 dark:text-white mb-4 px-2">Riwayat Pencapaian Bulanan</h3>
                    {(!achievements || achievements.length === 0) ? (
                        <div className="text-center py-10 bg-gray-50 dark:bg-neutral-900 rounded-3xl border border-gray-100 dark:border-neutral-800">
                            <p className="text-sm text-gray-500">Belum ada pencapaian bulanan tercatat.</p>
                        </div>
                    ) : (
                        <div className="space-y-3">
                            {achievements.map((ach) => {
                                const netActual = Math.max(0, Number(ach.achievedRevenue) - Number(ach.opex || 0))
                                const investorShare = netActual * (Number(project.profitSharePercent) / 100)
                                return (
                                    <div key={String(ach.id)} className="bg-white dark:bg-neutral-900 p-4 rounded-2xl shadow-sm border border-gray-100 dark:border-neutral-800 flex justify-between items-center">
                                        <div>
                                            <p className="text-xs font-bold text-gray-500 mb-1">Bulan {String(ach.month)} - {String(ach.year)}</p>
                                            <p className="text-sm font-black text-gray-900 dark:text-white">Rev: {formatCurrency(Number(ach.achievedRevenue))}</p>
                                        </div>
                                        <div className="text-right">
                                            <p className="text-[10px] font-bold text-gray-400 uppercase mb-1">Hak Anda</p>
                                            <p className="text-sm font-black text-green-600 dark:text-green-400">+{formatCurrency(investorShare)}</p>
                                        </div>
                                    </div>
                                )
                            })}
