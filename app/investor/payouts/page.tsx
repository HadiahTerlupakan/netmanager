      ...
      <main className="max-w-md mx-auto p-4 sm:p-6 space-y-4">
        {payouts.length > 0 ? (
          payouts.map((p) => (
            <div
              key={p.id}
              className="bg-white dark:bg-neutral-900 rounded-2xl p-4 sm:p-5 border border-gray-100 dark:border-neutral-800 shadow-sm relative overflow-hidden group"
            >
              <div className="flex justify-between items-start mb-3">
                ...
                <span
                  className={`px-2.5 py-1 text-[10px] font-bold uppercase rounded-full tracking-wider ${p.status === "COMPLETED" ? "bg-green-100 text-green-700 dark:bg-green-900/40 dark:text-green-400" : "bg-yellow-100 text-yellow-700 dark:bg-yellow-900/40 dark:text-yellow-400"}`}
                >
                  {p.status}
                </span>
              </div>
              ...
              <div className="grid grid-cols-2 gap-y-3 gap-x-4 pt-3 border-t border-gray-100 dark:border-neutral-800">
                <div>
                  <span className="block text-[10px] uppercase tracking-wider text-gray-400 font-medium mb-0.5">
                    Tanggal
                  </span>
          ...
          <div className="text-center py-12 px-4 bg-white dark:bg-neutral-900 rounded-2xl border border-gray-100 dark:border-neutral-800 shadow-sm">
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 max-w-xs mx-auto">
              Saat ini belum ada riwayat pembayaran profit yang tercatat di akun
              Anda.
            </p>
          </div>
        )}
