        ...
        <div className="sticky top-0 z-50 flex items-center bg-white dark:bg-[#1C2630] p-4 pb-2 justify-between border-b border-gray-100 dark:border-gray-800">
          ...
          <div className="flex w-12 items-center justify-end">
            <Button variant="ghost" size="icon" onClick={loadTickets}>
              <MdRefresh
                className={`text-[#111418] dark:text-white text-[24px] ${loading ? "animate-spin" : ""}`}
              />
            </Button>
          </div>
        </div>
              ...
              {tickets.map((ticket) => (
                ...
                >
                  <div className="flex items-start gap-3">
                    <div
                      className={`w-1.5 h-full min-h-[60px] rounded-full ${getPriorityColor(ticket.priority)}`}
                    />
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span
                          className={`text-[10px] font-medium px-2 py-0.5 rounded-full ${getStatusColor(ticket.status)}`}
                        >
