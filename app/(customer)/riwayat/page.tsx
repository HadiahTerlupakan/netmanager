                    ...
                    {Object.entries(groupedInvoices).length === 0 ? (
                        ...
                        Object.entries(groupedInvoices).map(([month, items]) => (
                            <div key={month} className="pt-4">
                                <h3 className="text-[#617589] dark:text-gray-400 text-sm font-semibold uppercase tracking-wider px-4 pb-2">{month}</h3>
                                {items.map((invoice, idx) => {
                                    const config = getStatusConfig(invoice.status)
                                    const Icon = config.icon

                                    return (
                                        <div key={invoice.id} className={`bg-white dark:bg-[#1C2630] ${idx === 0 ? 'border-t' : ''} border-b border-gray-100 dark:border-gray-800/50`}>
                                            <div className="flex gap-4 px-4 py-4 justify-between items-center cursor-pointer hover:bg-gray-50 dark:hover:bg-[#252e38] transition-colors">
                                                <div className="flex items-start gap-4">
                                                    <div className={`${config.iconColor} ${config.iconBg} flex items-center justify-center rounded-xl shrink-0 size-12`}>
                                                        <Icon className="text-[24px]" />
                                                    </div>
                                                    <div className="flex flex-1 flex-col justify-center gap-0.5">
                                                        <p className="text-[#111418] dark:text-white text-base font-semibold leading-tight">{invoice.items[0]?.description || 'Tagihan Layanan'}</p>
                                                        <p className="text-[#617589] dark:text-gray-400 text-xs font-normal leading-normal">#{invoice.invoiceNumber}</p>
                                                        <p className="text-[#617589] dark:text-gray-400 text-xs font-normal leading-normal mt-0.5">{formatDate(invoice.issueDate)}</p>
                                                    </div>
                                                </div>
                                                <div className="flex flex-col items-end gap-1">
                                                    <p className="text-[#111418] dark:text-white text-base font-bold leading-normal">{formatCurrency(invoice.totalAmount)}</p>
                                                    <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${config.badgeBg} ${config.badgeText}`}>
                                                        {config.label}
                                                    </span>
                                                </div>
                                            </div>
                ...
                <nav className="sticky bottom-0 z-50 bg-white dark:bg-[#1C2630] border-t border-gray-100 dark:border-gray-800 pb-safe">
                    <div className="flex justify-around items-center h-16 px-2">
                        <Link href="/dashboard" className="flex flex-col items-center justify-center w-full h-full gap-1 group">
                            ...
                            <span className="text-[10px] font-medium text-[#617589] dark:text-gray-400 group-hover:text-[#0d9488] transition-colors">Beranda</span>
                        </Link>
                        <Link href="/paket" className="flex flex-col items-center justify-center w-full h-full gap-1 group">
                            <MdDataUsage className="text-[#617589] dark:text-gray-400 group-hover:text-[#0d9488] transition-colors text-[24px]" />
                            <span className="text-[10px] font-medium text-[#617589] dark:text-gray-400 group-hover:text-[#0d9488] transition-colors">Paket</span>
                        </Link>
                        <Link href="/riwayat" className="flex flex-col items-center justify-center w-full h-full gap-1">
                            <MdHistory className="text-[#0d9488] text-[24px]" />
                            <span className="text-[10px] font-medium text-[#0d9488]">Riwayat</span>
                        </Link>
                        <Link href="/profil" className="flex flex-col items-center justify-center w-full h-full gap-1 group">
                            <MdPerson className="text-[#617589] dark:text-gray-400 group-hover:text-[#0d9488] transition-colors text-[24px]" />
                            <span className="text-[10px] font-medium text-[#617589] dark:text-gray-400 group-hover:text-[#0d9488] transition-colors">Akun</span>
                        </Link>
                    </div>
