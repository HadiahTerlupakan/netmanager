...
export default function CustomerInvoicesPage() {
    ...
    const selectedBankDetails = useMemo(() => {
        if (activePayment?.method === 'BANK_TRANSFER' && activePayment?.accountId && paymentMethods.length > 0) {
    ...
    const handlePayment = async () => {
        ...
        try {
            ...
            if (result.success) {
                ...
                if (result.data?.paymentUrl && !selectedMethodCode.startsWith('MANUAL_') && selectedMethodCode !== 'MOOTA_MANUAL') {
                ...
                <div className="flex-1 overflow-y-auto pb-24">
                            ...
                            <div className="bg-orange-50 dark:bg-orange-950/30 border border-orange-200 dark:border-orange-500/30 rounded-xl p-5 shadow-sm">
                                <div className="flex justify-between items-start mb-4">
                                    ...
                                    {activePayment.expiresAt && (
                                        <div className="flex items-center gap-1.5 px-2.5 py-1 bg-white dark:bg-[#1a2632] text-orange-600 dark:text-orange-400 rounded border border-orange-200 dark:border-orange-500/20 shadow-sm text-sm">
                                            <MdAccessTime />
                                            <CountdownTimer expiresAt={activePayment.expiresAt} onExpire={fetchInvoices} />
                                        </div>
                                    )}
                                </div>
                                ...
                                {activePayment.method === 'BANK_TRANSFER' && selectedBankDetails ? (
                                    <div className="bg-white dark:bg-[#1a2632] rounded-lg p-4 border border-gray-100 dark:border-gray-800 shadow-sm mb-4">
                                        <p className="text-xs text-gray-500 dark:text-gray-400 mb-1 font-medium">Transfer Bank Manual ({selectedBankDetails.bankName}):</p>
                                        <div className="flex justify-between items-center mb-3">
                                            <div>
                                                <p className="font-bold text-[#111418] dark:text-white text-lg">{selectedBankDetails.accountNumber}</p>
                                                <p className="text-xs text-gray-600 dark:text-gray-400">a.n {selectedBankDetails.accountName}</p>
                                            </div>
                                            <Button variant="outline" size="sm" onClick={() => copyToClipboard(selectedBankDetails.accountNumber, 'Nomor Rekening')}>
                                                <MdContentCopy /> Salin
                                            </Button>
                                        </div>
                                        <div className="border-t border-dashed border-gray-200 dark:border-gray-700 py-2"></div>
                                        <p className="text-xs text-gray-500 dark:text-gray-400 mb-1 mt-1 font-medium">Nominal Transfer:</p>
                                        <div className="flex justify-between items-center">
                                            <div>
                                                <p className="font-bold text-[#0d9488] text-xl">{formatCurrency(activePayment.amount)}</p>
                                            </div>
                                            <Button variant="outline" size="sm" onClick={() => copyToClipboard(activePayment.amount.toString(), 'Nominal')}>
                                                <MdContentCopy /> Salin
                                            </Button>
                                        </div>
                                        ...
                                        {activePayment.receiptUrl ? (
                                            <div className="mt-4 p-3 bg-teal-50 dark:bg-teal-900/30 border border-teal-200 dark:border-teal-800 rounded-lg flex flex-col items-center">
                                                ...
                                                <p className="font-bold text-teal-800 dark:text-teal-300 text-sm">Menunggu Verifikasi Admin</p>
                                            ...
                                            <div className="mt-4 flex flex-col items-center">
                                                ...
                                                >
                                                    {uploadingReceipt ? 'Mengunggah...' : 'Unggah Bukti Transfer'}
                                                </Button>
                                            </div>
                                        )}
                                    </div>
                                    ...
                                    <div className="bg-white dark:bg-[#1a2632] rounded-lg p-4 border border-gray-100 dark:border-gray-800 shadow-sm mb-4">
                                        <p className="text-xs text-center text-gray-500 dark:text-gray-400 mb-2 font-medium">Total Pembayaran</p>
                                        <p className="font-bold text-center text-[#0d9488] text-2xl">{formatCurrency(activePayment.amount)}</p>
                    ...
                    <div className="w-full overflow-x-auto hide-scrollbar pb-2">
                        <div className="flex gap-3 px-4 min-w-max">
                            ...
                            <Button variant="ghost" size="sm" onClick={() => setFilter('PAID')}
                                className={`rounded-full ${filter === 'PAID'
                                    ? 'bg-[#111418] dark:bg-white text-white dark:text-[#111418] hover:bg-[#111418] dark:hover:bg-white'
                                    : 'bg-white dark:bg-[#1a2632] border border-gray-200 dark:border-gray-700'
                                    }`}>Lunas</Button>
                            <Button variant="ghost" size="sm" onClick={() => setFilter('UNPAID')}
                    ...
                    <div className="flex flex-col mt-2">
                        {filteredInvoices.length === 0 ? (
                            ...
                            filteredInvoices.map((invoice) => {
                                ...
                                const dateObj = new Date(invoice.issueDate || invoice.createdAt || new Date())
                                    ...
                                    <div className="grid grid-cols-1 gap-2">
                                        ...
                                        {paymentMethods.map(method => (
                                            <div
                                                ...
                                                className={`flex items-center justify-between p-3 rounded-xl border-2 cursor-pointer transition-all ${selectedMethodCode === method.code
                                            ...
                                            >
                                                <div className="flex flex-col">
                                                    ...
                                                    <span className="text-[10px] text-gray-500 uppercase">{method.group}</span>
                                                </div>
                                                <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center ${selectedMethodCode === method.code ? 'border-[#0d9488]' : 'border-gray-300'
                                                    }`}>
                                                    {selectedMethodCode === method.code && <div className="w-2 h-2 rounded-full bg-[#0d9488]" />}
                                                </div>
                                            </div>
                                        ))}
                                    </div>
