...
export async function GET() {
  try {
    ...
    const siteIds = [
      ...new Set(
        rabInvestors.map((ri) => ri.rabProject.siteId).filter(Boolean),
      ),
    ] as string[];
    const mixRadiusOwners = [
      ...new Set(investorSites.flatMap((is) => is.owners || [])),
    ];
    ...
    const projects = rabInvestors.map((ri) => {
      ...
      const invSite = p.mixRadiusInvestorSiteId
        ? investorSiteMap.get(p.mixRadiusInvestorSiteId)
        : null;
      ...
      if (p.siteId && !p.mixRadiusInvestorSiteId) {
        ...
        totalActualRevenue += payingInternal.reduce(
        ...
        totalActualRevenue += activeMixRadius.reduce(
