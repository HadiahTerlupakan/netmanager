...
export const GET = createHandler({ auth: true }, async (req, ctx) => {
  ...
  const baseUrl = activeConfig?.apiUrl || process.env.MIXRADIUS_URL || 'https://sblnet.topsetting.com:973'
  const username = activeConfig?.username || process.env.MIXRADIUS_USERNAME || 'rudihartono'
  const password = activeConfig?.password || process.env.MIXRADIUS_PASSWORD || 'rudihartono12#'
