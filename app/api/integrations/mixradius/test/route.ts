...
export const GET = createHandler({ auth: true }, async (req, ctx) => {
  ...
  const baseUrl =
    ...
    "https://sblnet.topsetting.com:973";
  const username =
    activeConfig?.username || process.env.MIXRADIUS_USERNAME || "rudihartono";
  const password =
  ...
  try {
    ...
    logs.push(
      `Login response location: ${loginResponse.headers.get("location")}`,
    );
  } catch (e: unknown) {
    const message = e instanceof Error ? e.message : "Terjadi kesalahan";
  ...
  const links = pageHtml.match(
    /<a[^>]+href="([^"]*)"[^>]*>([^<]*(?:active|online|session)[^<]*)<\/a>/gi,
  );
  if (links) {
    ...
    if (hrefs) {
      logs.push(
        `Found ${hrefs.length} hrefs. Sample: ${hrefs.slice(0, 10).join(", ")}`,
      );
    }
  }
