...
export async function GET(
  ...
  try {
    ...
    const result = await onuControl.getTrafficStats(id, session.user.tenantId);

    if (!result.success) {
      return apiSuccess({ stats: null, error: result.error });
    }
    return apiSuccess(result.data);
  } catch (error) {
    logger.error("Error getting traffic stats:", error);
    const msg =
      error instanceof Error ? error.message : "Gagal membaca traffic stats";
