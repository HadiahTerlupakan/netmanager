  ...
  try {
    ...
    Object.keys(validation.data).forEach((key) => {
      if (validation.data[key as keyof typeof validation.data] !== undefined) {
        dataToUpdate[key] = validation.data[
    ...
    return ApiErrors.internalError(
      prismaError.message || "Gagal memperbarui bandwidth",
    );
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    if (bandwidth.hargaPaket && bandwidth.hargaPaket.length > 0) {
      const paketNames = bandwidth.hargaPaket
        .slice(0, 3)
        .map((p) => p.name)
        .join(", ");
      const moreCount =
        bandwidth.hargaPaket.length > 3
          ? ` dan ${bandwidth.hargaPaket.length - 3} lainnya`
          : "";
      return apiError(
    ...
    return ApiErrors.internalError(
      prismaError.message || "Gagal menghapus bandwidth",
    );
  }
}
