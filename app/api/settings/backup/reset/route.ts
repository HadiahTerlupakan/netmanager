...
export const POST = createHandler({ auth: true }, async (req, ctx) => {
  ...
  if (!isMainTenant(user.tenantId)) {
    return ApiErrors.forbidden(
      "Hanya tenant utama yang bisa mengakses fitur reset database",
    );
  }

  if (!(await hasPermission("backup_database:delete", user))) {
    return ApiErrors.forbidden(
      "Anda tidak memiliki permission untuk reset database",
    );
  }
  ...
  if (
    ...
    body.confirmationText !== RESET_CONFIRMATION_TEXT
  ) {
    return ApiErrors.badRequest(
      `Konfirmasi tidak valid. Harus tepat: ${RESET_CONFIRMATION_TEXT}`,
    );
  }
  ...
  try {
    ...
    logger.error("Error resetting settings backup data:", error);
