...
export const GET = createHandler({ auth: true }, async (_req, ctx) => {
  ...
  if (user.permissions?.includes("workorders:site_only") && !isSuper) {
    if (workOrder.siteId !== user.siteId) {
      return ApiErrors.forbidden(
        "Anda hanya bisa mengakses work order di Site Anda",
      );
    }
  }
  ...
  if (user.permissions?.includes("workorders:department_only") && !isSuper) {
    if (workOrder.departmentId !== user.departmentId) {
      return ApiErrors.forbidden(
        "Anda hanya bisa mengakses work order di Departemen Anda",
      );
    }
  }
...
export const PATCH = createHandler({ auth: true }, async (req, ctx) => {
  ...
  if (user.permissions?.includes("workorders:site_only") && !isSuper) {
    if (existingWO.siteId !== user.siteId) {
      return ApiErrors.forbidden(
        "Anda hanya bisa mengupdate work order di Site Anda",
      );
    }
  }
  ...
  if (user.permissions?.includes("workorders:department_only") && !isSuper) {
    if (existingWO.departmentId !== user.departmentId) {
      return ApiErrors.forbidden(
        "Anda hanya bisa mengupdate work order di Departemen Anda",
      );
    }
  }
...
export const DELETE = createHandler({ auth: true }, async (req, ctx) => {
  ...
  if (user.permissions?.includes("workorders:site_only") && !isSuper) {
    if (existingWO.siteId !== user.siteId) {
      return ApiErrors.forbidden(
        "Anda hanya bisa menghapus work order di Site Anda",
      );
    }
  }
  ...
  if (user.permissions?.includes("workorders:department_only") && !isSuper) {
    if (existingWO.departmentId !== user.departmentId) {
      return ApiErrors.forbidden(
        "Anda hanya bisa menghapus work order di Departemen Anda",
      );
    }
  }
  ...
  if (isPermanent) {
    ...
    if (!deleteResult.success) {
      ...
      return apiError(
        deleteResult.error || "Gagal menghapus work order",
        ErrorCodes.INTERNAL_ERROR,
        { status: 500 },
      );
    }
    ...
    return apiSuccess(null, {
      message: "Work order berhasil dihapus permanen",
    });
  }

  // For cancel, we use updateStatus
  const cancelResult = await workOrderService.updateStatus(
    id,
    "CANCELLED",
    user,
    reason,
  ...
  if (!cancelResult.success) {
    ...
    return apiError(
      cancelResult.error || "Gagal membatalkan work order",
      ErrorCodes.INTERNAL_ERROR,
      { status: 500 },
    );
  }
