...
export const POST = createHandler(
  ...
  async (req, ctx) => {
    ...
    if (!(await hasPermission("salary:create"))) {
      return ApiErrors.forbidden(
        "Anda tidak memiliki akses untuk membuat gaji",
      );
    }

    const { action, userId, month, year, departmentId, employeeType } =
      ctx.validated;
    const sessionUser = ctx.session!.user;
    ...
    if (isSiteOnly && !sessionUser.isSuperAdmin) {
      if (siteId && siteId !== sessionUser.siteId) {
        return ApiErrors.forbidden(
          "Anda hanya diperbolehkan mengolah data gaji di site Anda sendiri",
        );
      }
      siteId = sessionUser.siteId as string;
    }
    ...
    if (action === "calculate-single" && userId) {
      ...
      const result = await service.calculateSingle(
        userId,
        month,
        year,
        sessionUserId,
      ...
      if (!result.success) {
        return apiError(
          result.error || "Gagal menghitung gaji",
