...
import { getWhatsAppSettings, testWhatsAppSettings, updateWhatsAppSettings, type WhatsAppSettingsUpdatePayload } from '@/modules/settings'
import { z } from 'zod'
...
export const POST = createHandler<z.infer<typeof WHATSAPP_TEST_SCHEMA>>({
    ...
    if (!await hasPermission('whatsapp:update')) {
        return ApiErrors.forbidden('Anda tidak memiliki akses untuk mengirim pesan percobaan WhatsApp')
    }

    const { phone } = ctx.validated
    const result = await testWhatsAppSettings(phone)
