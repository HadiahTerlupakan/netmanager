...
/**
 * GET /api/admin/tax/export/pph21 - Export rekap PPh 21 bulanan ke CSV
 */
export const GET = createHandler({ auth: true }, async (_req, ctx) => {
  ...
  const csv = await service.exportPph21Csv(tenantId, year, month);

  return new NextResponse(csv, {
    status: 200,
    headers: {
      "Content-Type": "text/csv; charset=utf-8",
      "Content-Disposition": `attachment; filename="rekap-pph21-${year}-${String(month).padStart(2, "0")}.csv"`,
    },
  });
});
