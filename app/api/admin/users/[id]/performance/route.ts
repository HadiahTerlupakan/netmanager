...
export const GET = createHandler(
  ...
  async (req, ctx) => {
    ...
    attendanceStats.forEach((stat) => {
      ...
      else if (status === "ABSENT" || status === "DAY_OFF")
    ...
    if (workingHourMode === "FLEXIBLE") {
      ...
      rangeAttendance.forEach((att) => {
        if (att.checkOut) {
          const duration =
            (att.checkOut.getTime() - att.checkIn.getTime()) / (1000 * 60);
    ...
    const workOrders = {
      ...
      completionRate:
        leadTotal + supportTotal > 0
          ? Math.round(
              ((leadCompleted + supportCompleted) /
                (leadTotal + supportTotal)) *
                100,
            )
          : 0,
