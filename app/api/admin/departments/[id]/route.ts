...
export const GET = createHandler({ auth: true }, async (req, ctx) => {
  if (!(await hasPermission("department:read"))) {
    return ApiErrors.forbidden(
      "Anda tidak memiliki akses untuk melihat departemen",
    );
  }
...
export const PATCH = createHandler({ auth: true }, async (req, ctx) => {
  if (!(await hasPermission("department:update"))) {
    return ApiErrors.forbidden(
      "Anda tidak memiliki akses untuk mengubah departemen",
    );
  }
...
export const DELETE = createHandler({ auth: true }, async (req, ctx) => {
  if (!(await hasPermission("department:delete"))) {
    return ApiErrors.forbidden(
      "Anda tidak memiliki akses untuk menghapus departemen",
    );
  }
  ...
  if (!result.success) {
    ...
    if (result.code === "HAS_USERS" || result.code === "HAS_WORKORDERS") {
      return ApiErrors.conflict(
        "Departemen masih memiliki karyawan atau work order aktif",
      );
    }
    return ApiErrors.internalError(result.error);
  }
