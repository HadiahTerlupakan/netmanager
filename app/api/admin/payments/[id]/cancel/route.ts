...
export async function POST(
    ...
    try {
        ...
        if (payment.gatewayStatus !== 'PAID') {
            return NextResponse.json({ success: false, error: 'Only PAID payments can be cancelled' }, { status: 400 })
        }
        ...
        if (invoice.status === 'PAID' && (newStatus === 'SENT' || newStatus === 'PARTIAL_PAID')) {
