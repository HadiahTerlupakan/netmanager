...
export const GET = createHandler(
  ...
  async (req, ctx) => {
    ...
    const { isRestricted, siteIds } = checkSiteRestriction(
      session as unknown as Parameters<typeof checkSiteRestriction>[0],
      "pelanggan",
    );
    ...
    const { data: pelanggans, total } =
      await pelangganService.getAllPelangganPaginated(filter, page, limit);
  ...
  async (req, ctx) => {
    ...
    try {
      if (fileKTP && fileKTP.size > 0) {
        if (!(await validateFileSignature(fileKTP, ALLOWED_TYPES)))
          return apiError("File KTP tidak valid", "VALIDATION_ERROR", {
            status: 400,
          });
        fileKTPPath = isImageFile(fileKTP)
          ? await convertAndSaveImage(fileKTP, pelangganUploadDir, "ktp")
          : await saveFile(
              fileKTP,
              pelangganUploadDir,
              `ktp${path.extname(fileKTP.name) || ".pdf"}`,
            );
      }
      if (fileRumahSekitar && fileRumahSekitar.size > 0) {
        if (!(await validateFileSignature(fileRumahSekitar, ALLOWED_TYPES)))
          return apiError("File Rumah tidak valid", "VALIDATION_ERROR", {
            status: 400,
          });
        fileRumahSekitarPath = isImageFile(fileRumahSekitar)
          ? await convertAndSaveImage(
              fileRumahSekitar,
              pelangganUploadDir,
              "rumah",
            )
          : await saveFile(
              fileRumahSekitar,
              pelangganUploadDir,
              `rumah${path.extname(fileRumahSekitar.name) || ".pdf"}`,
            );
      }
      if (fileBAST && fileBAST.size > 0) {
        if (!(await validateFileSignature(fileBAST, ALLOWED_TYPES)))
          return apiError("File BAST tidak valid", "VALIDATION_ERROR", {
            status: 400,
          });
        fileBASTPath = isImageFile(fileBAST)
          ? await convertAndSaveImage(fileBAST, pelangganUploadDir, "bast")
          : await saveFile(
              fileBAST,
              pelangganUploadDir,
              `bast${path.extname(fileBAST.name) || ".pdf"}`,
            );
      }
    } catch (e) {
    ...
    } as Parameters<typeof pelangganService.createPelanggan>[0]);
