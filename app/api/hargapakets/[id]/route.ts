...
export const GET = createHandler({ auth: true }, async (req, ctx) => {
  ...
  try {
    ...
    logger.error("[HargaPaket GET Error]:", err);
    ...
    return ApiErrors.internalError(
      err?.message || "Gagal mengambil data harga paket",
    );
  }
});
...
export const PUT = createHandler({ auth: true }, async (req, ctx) => {
  if (!(await hasPermission("harga:update"))) {
    return ApiErrors.forbidden(
      "Anda tidak memiliki akses untuk memperbarui harga paket",
    );
  }
  ...
  try {
    const updated = await hargaPaketService.updateHargaPaket(
      id,
      body,
      ctx.session!.user.id,
    );
    ...
    logger.error("[HargaPaket PUT Error]:", err);
    ...
    return ApiErrors.internalError(
      err?.message || "Gagal memperbarui harga paket",
    );
  }
});
...
export const DELETE = createHandler({ auth: true }, async (req, ctx) => {
  if (!(await hasPermission("harga:delete"))) {
    return ApiErrors.forbidden(
      "Anda tidak memiliki akses untuk menghapus harga paket",
    );
  }
  ...
  try {
    ...
    const err = error as Error & { code?: string };
    logger.error("[HargaPaket DELETE Error]:", err);
    ...
    if (
      ...
      err.message.includes("masih digunakan")
    ) {
      return apiError(err.message, ErrorCodes.CONFLICT, { status: 409 });
    }

    return ApiErrors.internalError(
      err?.message || "Gagal menghapus harga paket",
    );
  }
});
