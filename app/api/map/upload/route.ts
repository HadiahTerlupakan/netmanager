...
export const POST = createHandler({
  ...
  if (!file.type.startsWith('image/')) {
    return apiError('File must be an image', 'VALIDATION_ERROR', { status: 400 });
  }

  const MAX_SIZE = 5 * 1024 * 1024; // 5MB
  if (file.size > MAX_SIZE) {
    return apiError('Photo size max 5MB', 'VALIDATION_ERROR', { status: 400 });
  }
  ...
  try {
    ...
    console.error('Map node photo upload error:', error);
    return apiError('Failed to upload photo', 'INTERNAL_ERROR', { status: 500 });
  }
});
