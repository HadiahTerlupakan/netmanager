...
export default function IdCardClient({ mitra }: { mitra: MitraData }) {
    ...
    return (
        <div className={`w-full max-w-lg mx-auto flex flex-col items-center relative ${isMobileApp ? 'h-screen overflow-hidden bg-slate-900 pb-0' : ''}`}>
            {/* Action Bar - Hidden on print and mobile mode */}
            {!isMobileApp && (
                <>
                    <div className="w-full flex justify-between items-center mb-8 print:hidden">
                        <Link href="/admin/mitra" className="text-gray-500 hover:text-gray-900 flex items-center gap-2 bg-white px-4 py-2 rounded-lg shadow-sm border border-gray-200">
                            <HiArrowLeft /> Kembali
                        </Link>
                        <button
                    ...
                    <div className="text-center mb-6 print:hidden">
                        <p className="text-sm text-gray-500 max-w-md">
                            Gunakan cetak ukuran asli atau <strong>ID-1 (85.6mm x 53.98mm)</strong>. Pastikan opsi <em>&apos;Print Background Graphics&apos;</em> diaktifkan pada pengaturan browser printer Anda.
                        </p>
                    </div>
                </>
            )}
            ...
            >
                ...
                >
                    {/* Header Pattern */}
                    <div className={`absolute top-0 left-0 right-0 h-[45px] w-full flex items-center justify-between px-4 ${isSales ? 'bg-gradient-to-r from-purple-700 to-purple-500' : 'bg-gradient-to-r from-indigo-700 to-blue-500'}`} style={{ printColorAdjust: 'exact', WebkitPrintColorAdjust: 'exact' }}>
                        ...
                        <div className="text-right">
                            ...
                            <div className={`inline-block mt-1 px-1.5 py-[2px] rounded-sm text-[5.5px] font-bold tracking-widest uppercase ${isSales ? 'bg-purple-900/50 text-white' : 'bg-blue-900/50 text-white'}`}>
                                MITRA RESMI
                            </div>
                        </div>
                    </div>

                    <div className="absolute top-[45px] left-0 right-0 bottom-[12px] px-4 py-3 bg-white flex" style={{ printColorAdjust: 'exact', WebkitPrintColorAdjust: 'exact', backgroundColor: 'white' }}>

                        {/* Left Column: Photo */}
                        <div className="w-[70px] flex flex-col items-center z-10">
                            <div className="w-[70px] h-[90px] bg-slate-50 rounded shadow-sm overflow-hidden border border-slate-200" style={{ printColorAdjust: 'exact', WebkitPrintColorAdjust: 'exact' }}>
                                {mitra.fotoDiri ? (
                                    <>
                                        {/* eslint-disable-next-line @next/next/no-img-element */}
                                        <img src={mitra.fotoDiri} alt="Foto Profil" className="w-full h-full object-cover" />
                                    </>
                                ) : (
                                    <div className="w-full h-full flex flex-col items-center justify-center text-slate-300 bg-slate-50">
                                        <svg className="w-8 h-8 mb-1" fill="currentColor" viewBox="0 0 24 24"><path d="M24 20.993V24H0v-2.996A14.977 14.977 0 0112.004 15c4.904 0 9.26 2.354 11.996 5.993zM16.002 8.999a4 4 0 11-8 0 4 4 0 018 0z" /></svg>
                                        <span className="text-[5px] font-bold tracking-widest text-slate-400">NON FOTO</span>
                                    </div>
                                )}
                            </div>
                        </div>
                        ...
                        <div className="ml-4 flex-1 flex flex-col pt-1 z-10 border-none">
                            <div className="mb-[6px]">
                                <h2 className={`font-black text-slate-900 leading-[1.1] uppercase tracking-tight truncate w-[200px] ${mitra.name && mitra.name.length > 20 ? 'text-[11px]' : 'text-[13px]'}`}>
                                    {mitra.name}
                                </h2>
                                <p className={`text-[7px] font-bold mt-[2px] uppercase tracking-widest ${isSales ? 'text-purple-600' : 'text-blue-600'}`}>
                            ...
                            <div className="space-y-[6px] mt-1 flex flex-col overflow-hidden max-w-[200px]">
                                <div>
                                    <p className="text-[5px] font-bold text-slate-400 uppercase tracking-widest leading-none mb-0.5">Mitra ID</p>
                                ...
                                {mitra.nik && (
                                    <div>
                                        <p className="text-[5px] font-bold text-slate-400 uppercase tracking-widest leading-none mb-0.5">NIK / KTP</p>
                                        <p className="text-[8.5px] font-bold text-slate-800 leading-none">{mitra.nik}</p>
                                    </div>
                                )}
                                <div>
                                    <p className="text-[5px] font-bold text-slate-400 uppercase tracking-widest leading-none mb-0.5">Branch / Site</p>
                                    <p className="text-[8.5px] font-bold text-slate-800 leading-none truncate w-full max-w-[170px]">{mitra.sites?.name || 'Kantor Pusat'}</p>
                                </div>
                            </div>
                        </div>

                        {/* Watermark Logo absolute positioned behind the text */}
                        <div className="absolute right-[-10px] bottom-[-20px] opacity-[0.03] text-slate-900 pointer-events-none z-0 overflow-hidden" style={{ printColorAdjust: 'exact', WebkitPrintColorAdjust: 'exact' }}>
                            <svg className="w-40 h-40" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-1 14.5v-9l6 4.5-6 4.5z" /></svg>
                        </div>

                        {/* QR Code on Bottom Right */}
                        <div className="absolute right-4 bottom-3 flex flex-col items-center z-20">
                            <div className="bg-white p-[2px] border border-slate-200 rounded-sm shadow-sm" style={{ printColorAdjust: 'exact', WebkitPrintColorAdjust: 'exact' }}>
                                <QRCode
                    ...
                    {/* Footer Bar */}
                    <div className={`absolute bottom-0 left-0 right-0 h-[12px] w-full flex items-center justify-between px-4 border-t ${isSales ? 'bg-purple-50/50 border-purple-100/50' : 'bg-slate-50 border-slate-200'}`} style={{ printColorAdjust: 'exact', WebkitPrintColorAdjust: 'exact' }}>
                        <p className="text-[4.5px] font-bold tracking-widest text-slate-400 uppercase m-0 p-0 leading-none">
                            Property of NetManager
                        </p>
                        <p className={`text-[4.5px] font-bold tracking-widest uppercase m-0 p-0 leading-none ${isSales ? 'text-purple-600' : 'text-blue-600'}`}>
                            Terdaftar: {format(new Date(mitra.createdAt), 'dd MMM yyyy', { locale: idLocale })}
                        </p>
                    </div>
                </div>
            </div>
            ...
            {isMobileApp && (
                <div className="absolute bottom-10 left-0 right-0 flex justify-center w-full px-6 z-50">
                    ...
                    >
                        ...
                        <span className="text-base">{isDownloading ? 'Menyiapkan ID Card...' : 'Simpan ke Galeri / HP'}</span>
                    </button>
                </div>
            )}
            <style jsx global>{`
                @media print {
                    ...
                    #__next, main { margin: 0; padding: 0; }
                }
            `}</style>
        </div>
    )
