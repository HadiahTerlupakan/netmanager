...
import type { WorkOrderDetail, MaterialDetailData, TimelineItem } from './types'
...
export function ClientComponent() {
    ...
    const handleDeleteAttachment = async (attachmentId: string) => {
        ...
        try {
            const res = await fetch(`/api/admin/workorders/${workOrderId}/attachments/${attachmentId}`, {
                method: 'DELETE',
            })
    ...
    const timelineLogItems = timelineItems.filter(item => item.type !== 'comment')

    const discussionItems = timelineItems.filter(item =>
        item.type === 'comment' || item.type === 'attachment'
    )
