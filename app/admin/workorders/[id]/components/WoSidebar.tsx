...
export function WoSidebar({
  ...
  const showCompletionReport = ["COMPLETED", "VERIFIED", "CLOSED"].includes(
    workOrder.status,
  );
  const customerInfo = getWorkOrderCustomerInfo(workOrder);
  const sourceBadge = CUSTOMER_SOURCE_BADGE[customerInfo.source];
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
        <h3 className="font-semibold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
          ...
          Customer Info
          <span
            className={`text-xs font-normal px-2 py-0.5 rounded-full ${sourceBadge.className}`}
          >
            {sourceBadge.label}
          </span>
        </h3>
          ...
          <div className="space-y-4">
            ...
            <div>
              <h4 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                Foto Dokumentasi ({completionAttachments.length})
              </h4>
            ...
            <div className="pt-2 border-t border-gray-100 dark:border-gray-700 flex justify-between text-xs text-gray-500 dark:text-gray-400">
              ...
              <span>
                {workOrder.completedAt
                  ? format(new Date(workOrder.completedAt), "dd MMM HH:mm", {
                      locale: localeId,
                    })
                  : "-"}
              </span>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
        ...
        {workOrder.assignments &&
          workOrder.assignments.filter(
            (a: { role: string }) => a.role === "PARTNER",
          ).length > 0 && (
            <div className="text-sm mt-3">
