...
import type { Bandwidth, MikroTikRouter, ProfilePppFormData } from '@/app/admin/paket/profileppp/lib/profilePppTypes'
...
type ProfilePppFormModalProps = {
  ...
  onFieldChange: <K extends keyof ProfilePppFormData>(field: K, value: ProfilePppFormData[K]) => void
}
  ...
  return (
    ...
    >
      <form onSubmit={onSubmit} className="space-y-4">
        <div>
          <label htmlFor="profileppp-name" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label htmlFor="profileppp-local-address" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
            ...
            <input
              ...
              onChange={(event) => onFieldChange('localAddress', event.target.value)}
              className="w-full rounded-md border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 px-3 py-2 text-sm"
          ...
          <div>
            <label htmlFor="profileppp-remote-address" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label htmlFor="profileppp-ip-range-start" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
            ...
            <input
              ...
              onChange={(event) => onFieldChange('ipRangeStart', event.target.value)}
              className="w-full rounded-md border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 px-3 py-2 text-sm"
          ...
          <div>
            <label htmlFor="profileppp-ip-range-end" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
            ...
            <input
              ...
              required={formData.poolMode === 'RADIUS'}
              onChange={(event) => onFieldChange('ipRangeEnd', event.target.value)}
              className="w-full rounded-md border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 px-3 py-2 text-sm"
        ...
        {pppConnectionMode === 'RADIUS' && (
          <div>
            <label htmlFor="profileppp-pool-mode" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
              Mode IP Pool <span className="text-red-500">*</span>
            </label>
            <select
              id="profileppp-pool-mode"
              value={formData.poolMode}
              onChange={(event) => onFieldChange('poolMode', event.target.value as 'MIKROTIK' | 'RADIUS')}
              className="w-full rounded-md border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 px-3 py-2 text-sm"
            >
        ...
        <div>
          <label htmlFor="profileppp-dns" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">DNS Server</label>
          <input
            id="profileppp-dns"
            type="text"
        ...
        <div>
          <label htmlFor="profileppp-router" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Target MikroTik Router</label>
          <select
            id="profileppp-router"
            value={formData.mikroTikRouterId}
            onChange={(event) => onFieldChange('mikroTikRouterId', event.target.value)}
            className="w-full rounded-md border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 px-3 py-2 text-sm"
          >
            ...
            {mikroTikRouters
              ...
              .map((router) => (
                <option key={router.id} value={router.id}>{router.name} ({router.ipAddress})</option>
              ))}
          </select>
          <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">Pilih router MikroTik sebagai target untuk profile ini</p>
        </div>

        {pppConnectionMode === 'MIKROTIK_API' && (
          <div>
            <label htmlFor="profileppp-bandwidth" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Bandwidth (untuk Rate Limit)</label>
            <select
              id="profileppp-bandwidth"
              value={formData.bandwidthId}
              onChange={(event) => onFieldChange('bandwidthId', event.target.value)}
              className="w-full rounded-md border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 px-3 py-2 text-sm"
            >
              <option value="">-- Pilih Bandwidth (Opsional) --</option>
              {bandwidths.map((bandwidth) => (
                <option key={bandwidth.id} value={bandwidth.id}>{bandwidth.name} ({bandwidth.maxLimitDownload}/{bandwidth.maxLimitUpload})</option>
              ))}
            </select>
            <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">Pilih Bandwidth untuk rate limit. Jika tidak dipilih, rate limit akan diambil dari HargaPaket yang terkait.</p>
          </div>
        )}

        <div>
          <label htmlFor="profileppp-description" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Deskripsi</label>
          <textarea
        ...
        <div>
          <label htmlFor="profileppp-status" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Status</label>
          <select
            id="profileppp-status"
            value={formData.status}
            onChange={(event) => onFieldChange('status', event.target.value as 'AKTIF' | 'NONAKTIF' | 'MAINTENANCE')}
            className="w-full rounded-md border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 px-3 py-2 text-sm"
          >
