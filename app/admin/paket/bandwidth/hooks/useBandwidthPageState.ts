...
import { buildBandwidthPayload, createInitialBandwidthFormData, mapBandwidthToFormData } from '@/app/admin/paket/bandwidth/lib/bandwidthFormatting'
import type { Bandwidth, BandwidthFormData } from '@/app/admin/paket/bandwidth/lib/bandwidthTypes'
...
export function useBandwidthPageState() {
  ...
  const [formData, setFormData] = useState<BandwidthFormData>(createInitialBandwidthFormData())
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      const url = editingBandwidth ? `/api/bandwidths/${editingBandwidth.id}` : '/api/bandwidths'
  ...
  const handleFormChange = <K extends keyof BandwidthFormData>(field: K, value: BandwidthFormData[K]) => {
