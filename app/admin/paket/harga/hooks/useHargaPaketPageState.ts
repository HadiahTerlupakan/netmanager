...
import { createInitialHargaFormData, mapHargaPaketToFormData } from '@/app/admin/paket/harga/lib/hargaPricing'
import type { Bandwidth, HargaFormData, HargaPaket, ProfilePPP, Site } from '@/app/admin/paket/harga/lib/hargaTypes'
...
export function useHargaPaketPageState() {
  ...
  const [formData, setFormData] = useState<HargaFormData>(createInitialHargaFormData())
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      const url = editingPaket ? `/api/hargapakets/${editingPaket.id}` : '/api/hargapakets'
  ...
  const handleFormChange = <K extends keyof HargaFormData>(field: K, value: HargaFormData[K]) => {
