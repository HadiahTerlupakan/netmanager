...
import type {
  SortDirection,
  SortField,
} from "@/app/admin/network/acs/devices/DevicesClient";
...
const isOnline = (lastInform: string | null) => {
  ...
  return Date.now() - new Date(lastInform).getTime() <= 10 * 60_000;
};
        ...
        <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
          <thead className="bg-[#f8fafc] dark:bg-gray-900/50">
            <tr>
              <th
                scope="col"
                className="px-6 py-4 text-left text-[11px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider"
              >
                STATUS
              </th>
              <th
                scope="col"
                className="px-6 py-4 text-left text-[11px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider cursor-pointer select-none hover:text-gray-700 dark:hover:text-gray-200"
              ...
              <th
                scope="col"
                className="px-6 py-4 text-center text-[11px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider"
              >
                WIFI (SSID)
              </th>
              <th
                scope="col"
                className="px-6 py-4 text-center text-[11px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider cursor-pointer select-none hover:text-gray-700 dark:hover:text-gray-200"
                onClick={() => onSort("rxpower")}
              >
                  ...
                  >
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span
                        className={
                          ...
                          (online
                            ? "bg-[#dcfce7] dark:bg-green-900/30 text-[#166534] dark:text-green-400"
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center">
                        <div
                          className={
                            "flex-shrink-0 h-10 w-10 rounded-full flex items-center justify-center text-white font-bold " +
                            (online
                              ? "bg-[#3b5fe5] dark:bg-blue-600"
                        ...
                        <div className="ml-4">
                          ...
                          <div className="text-[12px] text-gray-500 dark:text-gray-400 mt-0.5">
                            {device.productClass || "Unknown Model"}
                          </div>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      {device.pppoe ? (
                        <Link
                          href={`/admin/pelanggan?search=${encodeURIComponent(device.pppoe)}`}
                      ...
                      <div className="text-[12px] text-gray-500 dark:text-gray-400 mt-0.5 font-mono">
                        {device.ipAddress || "-"}
                      </div>
                    </td>
                    ...
                    <td className="px-6 py-4 text-center">
                      <span
                        className={
                          "inline-flex items-center px-2.5 py-0.5 rounded text-[12px] font-bold " +
                          getRxPowerColor(device.rxpower)
                        }
                      >
                    ...
                    <td className="px-6 py-4 text-center">
                      <div className="text-[12px] font-medium text-gray-700 dark:text-gray-300">
                        {device.lastInform
                          ? formatDistanceToNow(new Date(device.lastInform), {
                              addSuffix: true,
                            })
                          : "-"}
                      </div>
        ...
        <div className="px-6 py-4 border-t border-gray-100 dark:border-gray-700 bg-white dark:bg-gray-800 flex items-center justify-between">
          <p className="text-[13px] text-gray-500 dark:text-gray-400">
            ...
            <span className="font-bold text-gray-900 dark:text-white">
              {startIndex + 1}-
              {Math.min(startIndex + itemsPerPage, filteredDevicesLength)}
            </span>{" "}
            dari{" "}
            <span className="font-bold text-gray-900 dark:text-white">
              {filteredDevicesLength}
