...
import { MikrotikRouterForm, type MikrotikRouterFormData } from '@/app/admin/network/mikrotik/components/mikrotikRouterForm'
...
export function ClientComponent() {
  ...
  const [formData, setFormData] = useState<MikrotikRouterFormData & { autoConfigure: boolean }>({
    name: '',
    ipAddress: '',
  ...
  const { isTesting, showTestModal, testResult, runTest, closeTestModal } = useMikrotikConnectionTest({
    formData,
  })
  ...
  const handleFormChange = (field: keyof MikrotikRouterFormData, value: string | number) => {
    setFormData((prev) => ({
      ...prev,
      [field]: value,
    }))
  ...
  return (
    <div className="space-y-5">
      ...
      <MikrotikRouterForm
        ...
        submitDisabledTitle={!testPassed ? 'Silakan test koneksi terlebih dahulu dan pastikan test berhasil' : undefined}
