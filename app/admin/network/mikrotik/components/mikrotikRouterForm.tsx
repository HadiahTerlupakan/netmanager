...
type MikrotikRouterFormProps = {
  ...
  onChange: (field: keyof MikrotikRouterFormData, value: string | number) => void
...
export function MikrotikRouterForm({
  ...
  return (
    <form onSubmit={onSubmit} className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6 space-y-6">
      <div>
        <label htmlFor="mikrotik-name" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">! Nama Router</label>
        <input
          id="mikrotik-name"
          type="text"
          value={formData.name}
      ...
      <div>
        <label htmlFor="mikrotik-timezone" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">! Zona Waktu</label>
        <select
      ...
      <div>
        <label htmlFor="mikrotik-ip-address" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">! IP Router</label>
        <input
      ...
      <div>
        <label htmlFor="mikrotik-api-port" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">! Port API</label>
        <input
          id="mikrotik-api-port"
          type="number"
          value={formData.apiPort}
          onChange={(event) => onChange('apiPort', parseInt(event.target.value, 10) || 8728)}
          required
          placeholder="8728"
          min="1"
          max="65535"
      ...
      <div>
        <label htmlFor="mikrotik-api-username" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">! Username API</label>
        <input
      ...
      <div>
        <label htmlFor="mikrotik-api-password" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">! Password API</label>
        <input
      ...
      <div>
        <label htmlFor="mikrotik-isolir-url" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">URL Info Isolir (Optional)</label>
        <input
          ...
          className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-900 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500"
        />
        <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">PATH URL LENGKAP TANPA HTTP:// ATAU HTTPS://</p>
        <ul className="mt-2 text-xs text-gray-600 dark:text-gray-400 space-y-1 list-disc list-inside">
          ...
          <li>
            BACA PANDUAN DI GRUP FACEBOOK <span className="text-blue-600 dark:text-blue-400">MIXRADIUS</span>
          </li>
        </ul>
      </div>

      <div>
        <label htmlFor="mikrotik-description" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Deskripsi</label>
        <textarea
      ...
      <div className="flex items-center gap-3 pt-4">
        ...
        >
          <span className="text-white">{isSubmitting ? 'Menyimpan...' : submitLabel}</span>
        </button>
        <button
          type="button"
          onClick={onTestClick}
        ...
        <Link href={cancelHref} className="text-sm text-blue-600 dark:text-blue-400 hover:underline">
          Batal
        </Link>
      </div>
    </form>
