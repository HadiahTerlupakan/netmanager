...
export async function fetchMikrotikFormSettings(): Promise<{
  ...
  try {
    ...
    const generalSettingsData = (await generalSettingsRes.json()) as GeneralSettingsResponse
  ...
  try {
    ...
    const radiusDefaultsData = (await radiusDefaultsRes.json()) as RadiusDefaultsResponse
    radiusDefaults = {
      authPort: getNumberOrFallback(radiusDefaultsData.authPort, defaultRadiusDefaults.authPort),
      accountingPort: getNumberOrFallback(radiusDefaultsData.accountingPort, defaultRadiusDefaults.accountingPort),
      radiusSecret: getStringOrFallback(radiusDefaultsData.radiusSecret, defaultRadiusDefaults.radiusSecret),
    }
  } catch (error) {
    console.error('Failed to load RADIUS defaults:', error)
  }
...
export function validateMikrotikConnectionInput(formData: MikrotikFormConnectionData): string | null {
