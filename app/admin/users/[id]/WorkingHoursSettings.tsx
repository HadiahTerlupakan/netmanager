...
export default function WorkingHoursSettings({
  ...
  const [mode, setMode] = useState<WorkingHourMode>(
    (initialData.workingHourMode as WorkingHourMode) || WorkingHourMode.FIXED,
  );
  const [geofencePolicy, setGeofencePolicy] =
    useState<AttendanceGeofencePolicy>(
      (initialData.attendanceGeofencePolicy as AttendanceGeofencePolicy) ||
        AttendanceGeofencePolicy.WARN,
    );
  const [isAttendanceRequired, setIsAttendanceRequired] = useState(
    initialData.isAttendanceRequired ?? true,
  );
  const [startTime, setStartTime] = useState(
    initialData.startWorkTime || "09:00",
  );
  ...
  const [targetHours, setTargetHours] = useState(
    initialData.flexibleTargetHour || 8,
  );
    ...
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400">
              Konfigurasi jadwal kerja pengguna untuk sistem absensi
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          >
            <HiOutlineBriefcase
              className={`w-8 h-8 ${mode === WorkingHourMode.FIXED ? "text-blue-600" : "text-gray-400"}`}
            />
            <div className="text-center">
              <span
                className={`block font-semibold ${mode === WorkingHourMode.FIXED ? "text-blue-700 dark:text-blue-300" : "text-gray-700 dark:text-gray-300"}`}
              >
                Jam Kerja Tetap
              </span>
          ...
          >
            <HiOutlineCalendar
              className={`w-8 h-8 ${mode === WorkingHourMode.SHIFT ? "text-purple-600" : "text-gray-400"}`}
            />
            <div className="text-center">
              <span
                className={`block font-semibold ${mode === WorkingHourMode.SHIFT ? "text-purple-700 dark:text-purple-300" : "text-gray-700 dark:text-gray-300"}`}
              >
                Jam Kerja Shift
              </span>
          ...
          >
            <HiOutlineSun
              className={`w-8 h-8 ${mode === WorkingHourMode.FLEXIBLE ? "text-green-600" : "text-gray-400"}`}
            />
            <div className="text-center">
              <span
                className={`block font-semibold ${mode === WorkingHourMode.FLEXIBLE ? "text-green-700 dark:text-green-300" : "text-gray-700 dark:text-gray-300"}`}
              >
                Jam Kerja Fleksibel
              </span>
              ...
              {loadingShifts ? (
                ...
                <div className="p-4 bg-yellow-50 dark:bg-yellow-900/10 rounded-lg border border-yellow-100 dark:border-yellow-900">
                  <p className="text-sm text-yellow-700 dark:text-yellow-400">
                    Belum ada shift yang tersedia. Silakan buat shift di menu
                    Kehadiran → Manajemen Shift.
                  </p>
                </div>
              ) : (
                <select
              ...
              {selectedShiftId &&
                shifts.find((s) => s.id === selectedShiftId) && (
                  <p className="text-sm text-purple-600 dark:text-purple-400 mt-2">
                    Jadwal:{" "}
                    {shifts.find((s) => s.id === selectedShiftId)?.startTime} -{" "}
                    {shifts.find((s) => s.id === selectedShiftId)?.endTime}
                  </p>
                )}
            </div>
          </div>
          ...
          <div className="space-y-4 animate-fadeIn">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-2">
                Pengguna bebas melakukan check-in kapan saja, namun diharapkan
                memenuhi target jam kerja harian.
              </p>
            </div>
          </div>
        )}
          ...
          <div className="space-y-4 animate-fadeIn border-t border-gray-100 dark:border-gray-700 pt-6">
            <div>
              ...
              <p className="text-xs text-gray-500 mt-2">
                Pilih hari di mana pengguna ini diharapkan masuk kerja. Hari
                yang tidak dipilih akan dianggap sebagai hari libur (Off).
              </p>
            </div>
          </div>
        )}
        ...
        <div className="space-y-4 border-t border-gray-200 dark:border-gray-800 pt-6">
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">
              Kebijakan Geofence Absensi
            </label>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              <button
                type="button"
                onClick={() =>
                  setGeofencePolicy(AttendanceGeofencePolicy.STRICT)
                }
                className={`p-4 border rounded-xl text-left transition-all ${
                  geofencePolicy === AttendanceGeofencePolicy.STRICT
              ...
              >
                <div className="font-semibold text-gray-900 dark:text-white">
                  Wajib dalam area site
                </div>
                <div className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                  Tolak absensi jika berada di luar geofence.
                </div>
              </button>
              <button
                type="button"
                ...
                className={`p-4 border rounded-xl text-left transition-all ${
                  geofencePolicy === AttendanceGeofencePolicy.WARN
              ...
              >
                ...
                <div className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                  Izinkan absensi di luar area, tapi tampilkan peringatan.
                </div>
              </button>
              <button
                type="button"
                onClick={() =>
                  setGeofencePolicy(AttendanceGeofencePolicy.DISABLED)
                }
                className={`p-4 border rounded-xl text-left transition-all ${
                  geofencePolicy === AttendanceGeofencePolicy.DISABLED
              ...
              >
                ...
                <div className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                  Cocok untuk WFH atau peran remote penuh.
                </div>
              </button>
            </div>
          </div>
        ...
        <div className="pt-6 border-t border-gray-100 dark:border-gray-700">
          <div className="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-900/40 rounded-xl border border-gray-100 dark:border-gray-700">
            <div className="flex items-center gap-3">
              <div
                className={`p-2 rounded-lg ${isAttendanceRequired ? "bg-green-100 dark:bg-green-900/30" : "bg-gray-200 dark:bg-gray-800"}`}
              >
                <HiOutlineCheckCircle
                  className={`w-5 h-5 ${isAttendanceRequired ? "text-green-600 dark:text-green-400" : "text-gray-500"}`}
                />
              </div>
              <div>
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400">
                  Tentukan apakah pengguna ini harus melakukan absensi harian
                </p>
              </div>
            </div>
          ...
          {!isAttendanceRequired && (
            <div className="mt-3 p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-100 dark:border-blue-800">
              <p className="text-xs text-blue-700 dark:text-blue-300">
                <strong>Info:</strong> Pengguna yang tidak wajib absen (seperti
                Direktur) tidak akan muncul sebagai &quot;Alpha&quot; di laporan
                jika tidak melakukan check-in.
              </p>
            </div>
          )}
        </div>
