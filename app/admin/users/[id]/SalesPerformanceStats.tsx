...
export default function SalesPerformanceStats({ userId }: { userId: string }) {
    ...
    useEffect(() => {
        const fetchStats = async () => {
            setLoading(true)
            try {
                const res = await fetch(`/api/admin/users/${userId}/sales-performance?period=${period}`)
                if (res.ok) {
    ...
    return (
        <div className="space-y-6">
                ...
                <div className="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-sm border border-gray-200 dark:border-gray-700">
                    ...
                    <div className="grid grid-cols-2 gap-3 mb-4">
                        <div className="bg-emerald-50 dark:bg-emerald-900/20 p-3 rounded-lg text-center">
                            <p className="text-xs text-emerald-600 dark:text-emerald-300 uppercase font-bold">Disetujui</p>
                            <p className="text-2xl font-bold text-emerald-700 dark:text-emerald-200">{data.canvasing.approved}</p>
                        </div>
                        <div className="bg-gray-50 dark:bg-gray-700/50 p-3 rounded-lg text-center">
                            <p className="text-xs text-gray-500 uppercase font-bold">Total</p>
                            <p className="text-2xl font-bold text-gray-700 dark:text-gray-200">{data.canvasing.total}</p>
                        </div>
                    </div>
                ...
                <div className="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-sm border border-gray-200 dark:border-gray-700">
                    ...
                    <div className="space-y-2 text-sm">
                        <div className="flex justify-between items-center p-2 bg-yellow-50 dark:bg-yellow-900/20 rounded-lg">
                            ...
                            <span className="font-bold">{data.points.approved} claim ({data.points.approvedValue} pts)</span>
                        </div>
                        <div className="flex justify-between items-center p-2 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                            ...
                            <span className="font-bold">{data.points.pending} claim ({data.points.pendingValue} pts)</span>
                        </div>
                        <div className="flex justify-between items-center p-2 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                            <span className="flex items-center gap-2 text-gray-600 dark:text-gray-400">
                ...
                <div className="bg-linear-to-br from-emerald-500 to-teal-600 rounded-xl p-6 shadow-sm text-white">
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="flex justify-between items-center">
                            ...
                            <span className="text-2xl font-bold">{data.totalPointsAllTime}</span>
                        </div>
            ...
            {data.recentActivity.length > 0 && (
                <div className="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-sm border border-gray-200 dark:border-gray-700">
                    <h3 className="font-semibold text-gray-900 dark:text-white mb-4">Aktivitas Terbaru</h3>
                            ...
                            <div key={activity.id} className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                                ...
                                <div className="flex items-center gap-3">
                                    ...
                                    <span className="text-xs text-gray-400">
                                        {format(new Date(activity.createdAt), 'dd MMM', { locale: idLocale })}
                                    </span>
                                </div>
                            </div>
                        ))}
