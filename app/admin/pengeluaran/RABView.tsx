...
export default function RABView({
  ...
  const handleApproveRevision = async (revision: RABRevisionRecord) => {
    ...
    try {
      const response = await fetch(
        `/api/finance/rab-projects/${data.id}/revisions/${revision.id}/approve`,
        { method: "POST" },
      );
  ...
  const handleConfirmRejectRevision = async () => {
    ...
    try {
      const response = await fetch(
        `/api/finance/rab-projects/${data.id}/revisions/${rejectingRevision.id}/reject`,
        {
          method: "POST",
  ...
  const handleSendReminder = async () => {
    ...
    try {
      const res = await fetch(`/api/finance/rab-projects/${data.id}/reminder`, {
        method: "POST",
      });
      const json = await res.json();
                ...
                onClick={async () => {
                  ...
                  try {
                    const res = await fetch(
                      `/api/finance/rab-projects/${data.id}/approve`,
                      { method: "POST" },
                    );
                    const json = await res.json();
                    if (res.ok) {
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <p className="text-xs text-gray-500 font-semibold mb-1 flex items-center gap-1">
                    <HiOutlineBuildingOffice className="w-4 h-4" /> Site
                  </p>
                  <p className="text-sm font-medium text-gray-900 dark:text-gray-100">
                    {data.site?.name || "-"}
                  </p>
