...
export default function RABRevisionSummaryCards({
  summary,
}: RABRevisionSummaryCardsProps) {
  const cards = [
    {
      title: "Budget Awal",
      ...
      <div className="flex flex-col gap-3 rounded-xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-700 dark:bg-slate-800 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="mt-2 text-sm text-slate-600 dark:text-slate-300">
            Selisih dihitung dari baseline revisi final terhadap realisasi
            pengeluaran yang sudah tercatat.
          </p>
        </div>
        <div
          className={`inline-flex items-center rounded-full border px-3 py-1.5 text-sm font-semibold ${LABEL_STYLES[summary.label]}`}
        >
          {summary.label}
        </div>
      </div>

      {summary.unmappedRealization &&
        Number(summary.unmappedRealization) > 0 && (
          <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800 shadow-sm dark:border-amber-900/60 dark:bg-amber-950/30 dark:text-amber-300">
            Realisasi tanpa mapping item:{" "}
            {formatCurrency(Number(summary.unmappedRealization))}
          </div>
        )}
    </div>
  );
