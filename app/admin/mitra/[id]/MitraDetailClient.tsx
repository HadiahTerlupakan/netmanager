...
export default function MitraDetailClient() {
    ...
    const fetchFaceLogs = useCallback(async (page: number) => {
        try {
            setFaceLogsLoading(true)
            const res = await fetch(`/api/admin/mitra/${params.id}/face-verifications?page=${page}&limit=10`)
            const data = await res.json()
            if (res.ok && data.success) {
    ...
    const handleRequestFaceVerification = async () => {
        if (!mitra) return
        if (!confirm('Apakah Anda yakin ingin mewajibkan mitra ini untuk verifikasi wajah?')) return
    ...
    if (!mitra) {
        return (
            <div className="p-8 text-center text-gray-500">
                ...
                <button onClick={() => router.back()} className="text-indigo-600 mt-4 hover:underline">Kembali</button>
            </div>
        )
    }
    ...
    return (
        <div className="p-4 md:p-6 max-w-7xl mx-auto space-y-6">
            {/* Photo Preview Modal */}
            {previewPhoto && (
                <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in" onClick={() => setPreviewPhoto(null)}>
                    <div className="relative max-w-2xl max-h-[85vh]" onClick={e => e.stopPropagation()}>
                        {/* eslint-disable-next-line @next/next/no-img-element */}
                        <img src={previewPhoto} alt="Preview" className="max-w-full max-h-[85vh] rounded-2xl shadow-2xl object-contain" />
                        <button onClick={() => setPreviewPhoto(null)} className="absolute -top-3 -right-3 w-8 h-8 bg-white dark:bg-gray-800 rounded-full shadow-lg flex items-center justify-center text-gray-500 hover:text-red-500 transition-colors text-sm font-bold">✕</button>
                    </div>
                </div>
            )}
            ...
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                    <button onClick={() => router.push('/admin/mitra')} className="p-2 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-xl text-gray-400 hover:text-gray-700 dark:hover:text-white transition-all">
                        <HiOutlineArrowLeft className="w-5 h-5" />
                    </button>
                    <div>
                        <h1 className="text-lg font-bold text-gray-900 dark:text-white">Detail Mitra</h1>
                        <p className="text-xs text-gray-400">Informasi lengkap dan riwayat verifikasi</p>
                    </div>
                </div>
                {/* Quick Actions */}
                <div className="flex gap-2">
                    <ActionBtn icon={<HiOutlinePencilSquare className="w-4 h-4" />} label="Edit" color="indigo" onClick={() => router.push(`/admin/mitra?edit=${mitra.id}`)} />
                    <ActionBtn
                    ...
                    />
                    <ActionBtn icon={<HiOutlineIdentification className="w-4 h-4" />} label="ID Card" color="purple" onClick={() => window.open(`/mitra-id/${mitra.id}`, '_blank')} />
                </div>
            </div>

            {/* ── Profile Header ──────────────────────────────────────── */}
            <div className={`rounded-2xl overflow-hidden shadow-sm ring-1 ring-gray-200 dark:ring-gray-800 ${isTeknisi
                ...
                <div className="px-6 py-6 flex flex-col sm:flex-row items-center sm:items-start gap-5">
                    ...
                    <div className="relative flex-shrink-0">
                        {mitra.fotoDiri ? (
                            ...
                            <div className="w-20 h-20 rounded-2xl flex items-center justify-center bg-white/20 backdrop-blur-sm border border-white/30 shadow-xl">
                                {isTeknisi ? <HiOutlineWrenchScrewdriver className="w-8 h-8 text-white/80" /> : <HiOutlineMegaphone className="w-8 h-8 text-white/80" />}
                            </div>
                        )}
                    ...
                    <div className="text-center sm:text-left">
                        <h2 className="text-xl font-bold text-white drop-shadow-sm">{mitra.name || mitra.email.split('@')[0]}</h2>
                        <p className="text-white/70 text-sm mt-0.5">{mitra.email}</p>
                        <div className="flex flex-wrap items-center gap-2 mt-3 justify-center sm:justify-start">
                            <Badge icon={isTeknisi ? <HiOutlineWrenchScrewdriver className="w-3 h-3" /> : <HiOutlineMegaphone className="w-3 h-3" />}>
                            ...
                            {mitra.requiresFaceVerification && (
                                <Badge variant="warning" icon={<HiOutlineCamera className="w-3 h-3" />}>
                    ...
                    {mitra.mitraWallet && (
                        <div className="sm:ml-auto flex-shrink-0 text-center sm:text-right bg-white/15 backdrop-blur-sm rounded-xl px-5 py-3 border border-white/20">
                            <p className="text-white/60 text-[10px] uppercase tracking-wider font-semibold">Saldo Wallet</p>
                            <p className="text-2xl font-bold text-white mt-0.5">{fmt(mitra.mitraWallet.balance)}</p>
                        </div>
                    )}
                </div>
            </div>
                ...
                <div className="lg:col-span-5 space-y-5">
                    ...
                    <Section icon={<HiOutlineIdentification />} title="Informasi Pribadi">
                        <div className="space-y-1">
                            ...
                            <DataRow label="Tempat, Tgl Lahir" value={mitra.tempatLahir ? `${mitra.tempatLahir}, ${fmtDate(mitra.tanggalLahir)}` : '-'} />
                            <DataRow label="Alamat" value={mitra.alamat || '-'} />
                            {(mitra.latitudeRumah && mitra.longitudeRumah) && (
                                <div className="pt-1">
                                    <a href={`https://maps.google.com/?q=${mitra.latitudeRumah},${mitra.longitudeRumah}`} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-[11px] text-blue-600 hover:text-blue-800 hover:underline font-medium">
                        ...
                        <div className="mt-3 pt-3 border-t border-gray-100 dark:border-gray-800 space-y-1">
                            ...
                            <DataRow label="Verif Wajah Terakhir" value={fmtDateTime(mitra.lastFaceVerification)} />
                        </div>
                    </Section>
                        ...
                        {mitra.mitraWallet ? (
                            <>
                                <div className="text-center py-3">
                                    <p className="text-[10px] text-gray-400 uppercase tracking-widest font-semibold">Total Saldo</p>
                                    <p className="text-3xl font-extrabold text-green-600 dark:text-green-400 mt-1">{fmt(mitra.mitraWallet.balance)}</p>
                                </div>
                                <div className="grid grid-cols-2 gap-3 pt-3 border-t border-gray-100 dark:border-gray-800">
                                    <MiniStat label="Total Pendapatan" value={fmt(mitra.mitraWallet.totalEarnings)} color="emerald" />
                                    <MiniStat label="Total Dicairkan" value={fmt(mitra.mitraWallet.totalWithdrawn)} color="amber" />
                                </div>
                            </>
                        ) : (
                                ...
                                <div key={doc.label} className="text-center group">
                                    {doc.url ? (
                                        ...
                                        <div className="w-full aspect-square rounded-xl border-2 border-dashed border-gray-200 dark:border-gray-700 flex items-center justify-center">
                                            <span className="text-[10px] text-gray-300 dark:text-gray-600">N/A</span>
                                        </div>
                                    )}
                                    <p className="text-[10px] text-gray-400 mt-1.5 font-medium">{doc.label}</p>
                                </div>
                            ))}
                        </div>
                    </Section>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                            ...
                            <div>
                                ...
                                <div className="space-y-2">
                                    {isTeknisi ? (
                                        <>
                                            <KomisiRow label="WO PSB / Instalasi" value={fmt(mitra.mitraRateWoPsb)} />
                                            <KomisiRow label="WO Maintenance" value={fmt(mitra.mitraRateWoMaintenance)} />
                                        </>
                                    ) : (
                                        <>
                            ...
                            <div className="space-y-5">
                                <div>
                                    ...
                                    <div className="bg-gray-50 dark:bg-gray-800/50 rounded-xl p-3.5 space-y-2 text-sm">
                                        ...
                                        <BankRow label="No. Rekening" value={mitra.bankAccountNo || '-'} mono />
                                        <BankRow label="Atas Nama" value={mitra.bankAccountName || '-'} />
                                    </div>
                                </div>
                                {isTeknisi && (
                                    <div>
                                        ...
                                        <div className="bg-gray-50 dark:bg-gray-800/50 rounded-xl p-3.5 space-y-2 text-sm">
                                            <BankRow label="Masa Garansi" value={`${mitra.garansiHari ?? 7} hari`} />
                                            <BankRow label="SLA Respon" value={`${mitra.slaGaransiJam ?? 24} jam`} />
                                            <BankRow label="Denda PSB" value={fmt(mitra.penaltyPsb)} danger />
                                            <BankRow label="Denda Maintenance" value={fmt(mitra.penaltyMaintenance)} danger />
                                        </div>
                                    </div>
                                )}
                            </div>
                    ...
                    <Card className="border-0 shadow-sm ring-1 ring-gray-200 dark:ring-gray-800 overflow-hidden">
                        <CardHeader className="flex flex-row items-center justify-between border-b border-gray-100 dark:border-gray-800 py-4 px-5">
                            <CardTitle className="text-sm font-semibold flex items-center gap-2 text-gray-700 dark:text-gray-200">
                                ...
                                History Verifikasi Wajah
                                {faceLogsTotal > 0 && (
                                    <span className="text-[10px] font-medium text-gray-400 bg-gray-100 dark:bg-gray-800 px-2 py-0.5 rounded-full">{faceLogsTotal}</span>
                                )}
                            </CardTitle>
                            {faceLogsTotal > 5 && faceLogsPage === 1 && (
                                <button onClick={() => fetchFaceLogs(1)} className="text-[11px] text-indigo-600 hover:underline font-medium">Lihat Semua</button>
                            )}
                        </CardHeader>
                        <CardContent className="p-0">
                            {faceLogsLoading ? (
                                ...
                                <div className="p-10 text-center">
                                    ...
                                    <p className="text-xs text-gray-400">Belum ada riwayat verifikasi wajah</p>
                                    <p className="text-[10px] text-gray-300 dark:text-gray-600 mt-0.5">Riwayat muncul setelah mitra melakukan face verification</p>
                                </div>
                            ) : (
                                <>
                                    <div className="divide-y divide-gray-100 dark:divide-gray-800">
                                        {faceLogs.map((log) => (
                                            <div key={log.id} className="px-5 py-3.5 flex items-center gap-4 hover:bg-gray-50/50 dark:hover:bg-gray-800/30 transition-colors">
                                                {/* eslint-disable-next-line @next/next/no-img-element */}
                                                <img
                                                    src={log.photoUrl}
                                                    alt="Verifikasi"
                                                ...
                                                <div className="flex-1 min-w-0">
                                                    <div className="flex items-center gap-1.5">
                                                        ...
                                                        <span className="text-sm font-medium text-gray-800 dark:text-gray-200">{fmtDateTime(log.createdAt)}</span>
                                                    </div>
                                                    {(log.latitude && log.longitude) && (
                                                        <a href={`https://maps.google.com/?q=${log.latitude},${log.longitude}`} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-[10px] text-blue-500 hover:underline mt-0.5">
                                    ...
                                    {faceLogsTotalPages > 1 && (
                                        <div className="px-5 py-3 border-t border-gray-100 dark:border-gray-800 flex items-center justify-between">
                                            <span className="text-[10px] text-gray-400">Hal. {faceLogsPage}/{faceLogsTotalPages}</span>
                                            <div className="flex gap-1.5">
                                                <PagBtn label="← Prev" disabled={faceLogsPage <= 1} onClick={() => fetchFaceLogs(faceLogsPage - 1)} />
                                                <PagBtn label="Next →" disabled={faceLogsPage >= faceLogsTotalPages} onClick={() => fetchFaceLogs(faceLogsPage + 1)} />
                                            </div>
                                        </div>
                                    )}
                                </>
...
function SectionLabel({ children }: { children: React.ReactNode }) {
    return <p className="text-[10px] text-gray-400 uppercase tracking-widest font-semibold mb-2">{children}</p>
}
...
function DataRow({ label, value, mono }: { label: string; value: string; mono?: boolean }) {
    return (
        <div className="flex items-baseline justify-between py-1.5">
            ...
            <span className={`text-xs text-gray-800 dark:text-gray-200 text-right ml-4 ${mono ? 'font-mono' : ''}`}>{value}</span>
        </div>
    )
}

function KomisiRow({ label, value, suffix }: { label: string; value: string; suffix?: string }) {
    return (
        <div className="flex items-center justify-between bg-gray-50 dark:bg-gray-800/50 rounded-xl px-3.5 py-2.5">
            ...
            <span className="text-xs font-semibold text-emerald-600 dark:text-emerald-400">
                {value}{suffix && <span className="text-gray-400 font-normal ml-1">{suffix}</span>}
            </span>
        </div>
    )
}

function BankRow({ label, value, mono, danger }: { label: string; value: string; mono?: boolean; danger?: boolean }) {
    return (
        <div className="flex items-center justify-between">
            <span className="text-xs text-gray-400">{label}</span>
            <span className={`text-xs font-medium ${danger ? 'text-red-500' : 'text-gray-800 dark:text-gray-200'} ${mono ? 'font-mono' : ''}`}>{value}</span>
        </div>
    )
}
...
function MiniStat({ label, value, color }: { label: string; value: string; color: string }) {
    const colorMap: Record<string, string> = {
        emerald: 'bg-emerald-50 dark:bg-emerald-900/15 text-emerald-700 dark:text-emerald-400',
        amber: 'bg-amber-50 dark:bg-amber-900/15 text-amber-700 dark:text-amber-400',
    }
    return (
...
function Badge({ children, variant, icon }: { children: React.ReactNode; variant?: 'success' | 'danger' | 'warning'; icon?: React.ReactNode }) {
    ...
    return (
        <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-[11px] font-semibold border ${styles[variant || 'default']}`}>
            {icon}{children}
        </span>
    )
}
...
function ActionBtn({ icon, label, color, onClick, disabled }: { icon: React.ReactNode; label: string; color: string; onClick: () => void; disabled?: boolean }) {
    const colorMap: Record<string, string> = {
        indigo: 'bg-indigo-50 text-indigo-600 hover:bg-indigo-100 dark:bg-indigo-900/20 dark:text-indigo-400 dark:hover:bg-indigo-900/40',
    ...
    return (
        <button onClick={onClick} disabled={disabled} className={`inline-flex items-center gap-1.5 px-3 py-2 text-xs font-medium rounded-xl transition-all ${colorMap[color] || ''} disabled:opacity-50 disabled:cursor-not-allowed`}>
...
function PagBtn({ label, disabled, onClick }: { label: string; disabled: boolean; onClick: () => void }) {
    return (
        <button onClick={onClick} disabled={disabled} className="px-2.5 py-1 text-[10px] rounded-lg bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300 hover:bg-gray-200 disabled:opacity-30 disabled:cursor-not-allowed transition-colors font-medium">
            {label}
        </button>
    )
}
...
function EmptyState({ text }: { text: string }) {
    return <p className="text-xs text-gray-300 dark:text-gray-600 text-center py-4">{text}</p>
}
