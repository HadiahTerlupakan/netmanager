...
export default function PengeluaranEditPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const loadData = async (pengeluaranId: string) => {
    try {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || 'Gagal mengupdate pengeluaran')
      }

      router.push('/admin/finance/pengeluaran')
      router.refresh()
    } catch (err: any) {
      setError(err.message || 'Terjadi kesalahan saat mengupdate pengeluaran')
    } finally {
      setSaving(false)
    }
  }
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center justify-center py-12">
          <HiArrowPath className="w-8 h-8 text-indigo-600 animate-spin" />
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-bold text-gray-900 dark:text-white">Edit Pengeluaran</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Edit data pengeluaran</p>
        </div>
        <Link
