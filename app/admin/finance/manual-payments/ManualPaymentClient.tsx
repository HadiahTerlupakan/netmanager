...
export default function ManualPaymentClient() {
    ...
    const fetchPendingPayments = async () => {
        ...
        try {
            ...
            const res = await fetch(`/api/admin/payments/pending-manual?${query.toString()}`)
            const json = await res.json()
            if (json.success) {
    ...
    const handleAction = async (action: 'APPROVE' | 'REJECT') => {
        ...
        try {
            ...
            if (json.success) {
                toast.success(`Pembayaran berhasil di-${action === 'APPROVE' ? 'setujui' : 'tolak'}`)
    ...
    return (
        <div>
            ...
            <div className="bg-white dark:bg-gray-800 p-4 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 flex flex-wrap gap-4 items-end mb-6">
                ...
                <div>
                    <label className="block text-sm font-medium mb-1 dark:text-gray-300">Pilih Site</label>
                    <select
                        value={siteId}
                        onChange={(e) => { setSiteId(e.target.value); setCurrentPage(1); }}
                ...
                <div>
                    <label className="block text-sm font-medium mb-1 dark:text-gray-300">Status Pembayaran</label>
                    <select
                        value={statusFilter}
                ...
                <Button
                    onClick={() => { setCurrentPage(1); fetchPendingPayments(); }}
            ...
            {payments.length === 0 ? (
                <div className="bg-white dark:bg-gray-800 rounded-lg p-10 text-center border border-gray-100 dark:border-gray-700">
                    <p className="text-gray-500 dark:text-gray-400">Tidak ada pengajuan pembayaran manual untuk filter ini.</p>
                </div>
            ) : (
                <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                        <div className="bg-white dark:bg-gray-800 p-4 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700">
                            <p className="text-sm text-gray-500 dark:text-gray-400">Menunggu Verifikasi</p>
                        ...
                        <div className="bg-white dark:bg-gray-800 p-4 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700">
                            ...
                            <p className="text-2xl font-bold text-green-600 dark:text-green-400">
                                {payments.filter(p => p.gatewayStatus === 'SUCCESS' || p.gatewayStatus === 'PAID').length}
                            </p>
                        </div>
                        <div className="bg-white dark:bg-gray-800 p-4 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700">
                            <p className="text-sm text-gray-500 dark:text-gray-400">Ditolak</p>
                            <p className="text-2xl font-bold text-red-600 dark:text-red-400">
                                {payments.filter(p => p.gatewayStatus === 'FAILED' || p.gatewayStatus === 'CANCELLED').length}
                            </p>
                        </div>
                        <div className="bg-white dark:bg-gray-800 p-4 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700">
                            <p className="text-sm text-gray-500 dark:text-gray-400">Nominal Disetujui</p>
                            <p className="text-2xl font-bold text-indigo-600 dark:text-indigo-400">
                                Rp {payments
                                    .filter(p => p.gatewayStatus === 'SUCCESS' || p.gatewayStatus === 'PAID')
                            ...
                            <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                <thead className="bg-gray-50 dark:bg-gray-900/50">
                                    <tr>
                                        ...
                                        <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Pelanggan & Invoice</th>
                                        <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Total Dibayar</th>
                                ...
                                <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                                    {payments.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage).map(payment => {
                                        ...
                                        const isSuccess = payment.gatewayStatus === 'SUCCESS' || payment.gatewayStatus === 'PAID';
                                        const isFailed = payment.gatewayStatus === 'FAILED' || payment.gatewayStatus === 'CANCELLED';
                                        ...
                                        return (
                                            <tr key={payment.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                                                ...
                                                <td className="px-6 py-4 whitespace-nowrap text-right text-sm">
                                                    <Button
                                                        size="sm"
                                                        className={isPending ? "bg-indigo-600 hover:bg-indigo-700 text-white" : "bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700"}
                        ...
                        {Math.ceil(payments.length / itemsPerPage) > 1 && (
                            <div className="px-6 py-3 flex flex-col sm:flex-row justify-between items-center border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 gap-3">
                                ...
                                <span className="text-sm text-gray-500 dark:text-gray-400">
                                    Halaman {currentPage} dari {Math.ceil(payments.length / itemsPerPage)} ({payments.length} Data)
                                </span>
                                <Button disabled={currentPage === Math.ceil(payments.length / itemsPerPage)}
                                    onClick={() => setCurrentPage(p => p + 1)}
                                    className="px-3 py-1 border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-gray-800 disabled:opacity-50 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700"
                                >
                                    Selanjutnya
                ...
                {selectedPayment && (
                    <div className="space-y-6">
                        <div className="bg-gray-100 dark:bg-gray-900 rounded-lg p-2 text-center overflow-hidden">
                            <p className="text-xs text-gray-500 mb-2">Bukti Transfer / Pembayaran</p>
                        ...
                        <div className="grid grid-cols-2 gap-4 text-sm">
                            <div className="bg-gray-50 dark:bg-gray-800/50 p-3 rounded-lg border border-gray-100 dark:border-gray-700">
                                <span className="block text-gray-500 text-xs">Total Dibayar</span>
                                <span className="font-bold text-lg text-gray-900 dark:text-white">Rp {Number(selectedPayment.amount).toLocaleString('id-ID')}</span>
                            </div>
                            <div className="bg-gray-50 dark:bg-gray-800/50 p-3 rounded-lg border border-gray-100 dark:border-gray-700">
                                <span className="block text-gray-500 text-xs">Tagihan Seharusnya</span>
                                <span className="font-bold text-lg text-gray-900 dark:text-white">Rp {Number(selectedPayment.invoice?.totalAmount).toLocaleString('id-ID')}</span>
                            </div>
                        </div>
