...
export default function TransferModal({
  ...
  // Default to first account when the modal opens (prevValue comparator pattern,
  // setState during render is allowed when guarded by previous-state check).
  ...
  if (isOpen !== prevOpen) {
    ...
    if (
      ...
      setFormData({ ...formData, sourceAccountId: accounts[0].id });
    }
  }
                ...
                {formData.sourceAccountId && (
                  <p className="text-xs text-gray-500 dark:text-gray-400">
                    ...
                    <span className="font-medium text-gray-700 dark:text-gray-300">
                      {formatBalance(
                        accounts.find((a) => a.id === formData.sourceAccountId)
                          ?.balance || 0,
                      )}
                    </span>
                  </p>
