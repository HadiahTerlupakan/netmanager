...
export default function TreasuryClient({ accounts }: TreasuryClientProps) {
  ...
  const getIcon = (type: string) => {
    switch (type) {
      case 'BANK': return <HiOutlineBuildingLibrary className="w-6 h-6" />
      case 'CASH': return <HiOutlineBanknotes className="w-6 h-6" />
      case 'EWALLET': return <HiOutlineCreditCard className="w-6 h-6" />
      default: return <HiOutlineBanknotes className="w-6 h-6" />
    }
  }

  const getColor = (type: string) => {
    switch (type) {
      case 'BANK': return 'bg-blue-100 text-blue-600 dark:bg-blue-900/40 dark:text-blue-300'
      case 'CASH': return 'bg-green-100 text-green-600 dark:bg-green-900/40 dark:text-green-300'
      case 'EWALLET': return 'bg-purple-100 text-purple-600 dark:bg-purple-900/40 dark:text-purple-300'
  ...
  return (
    <div className="max-w-7xl mx-auto">
      ...
      <div className="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Kas & Bank (Treasury)</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div className="bg-linear-to-br from-indigo-600 to-blue-700 rounded-2xl p-6 text-white shadow-lg relative overflow-hidden">
            ...
            <p className="text-indigo-100 font-medium mb-1 relative z-10">Total Saldo Likuid</p>
            <h2 className="text-4xl font-bold mb-4 relative z-10">{formatCurrency(totalBalance)}</h2>
            <div className="text-sm text-indigo-100 relative z-10 flex items-center gap-2">
                <span className="bg-white/20 px-2 py-1 rounded-lg text-xs font-semibold">{accounts.length} Akun Aktif</span>
            </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {accounts.map((acc) => (
            <div key={acc.id} className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden shadow-sm">
                <div className="p-6">
                    ...
                    <h4 className="text-lg font-bold text-gray-900 dark:text-white mb-1 group-hover:text-blue-600 transition-colors">{acc.name}</h4>
                    <p className="text-sm text-gray-500 dark:text-gray-400 mb-4 font-mono">{acc.accountNumber || '-'}</p>
        ...
        {/* Add Account Placeholder */}
         {canManageFinance && (
           <Button variant="ghost" onClick={() => setAddAccountModalOpen(true)} className="border-2 border-dashed border-gray-200 dark:border-gray-700 p-6 flex flex-col items-center justify-center text-center min-h-[240px] w-full">
