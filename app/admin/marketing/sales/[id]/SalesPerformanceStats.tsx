            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-sm border border-gray-200 dark:border-gray-700 relative overflow-hidden">
                ...
                <div className="flex items-center justify-between mb-4 relative z-10">
                    <div>
                        ...
                        <p className="text-sm text-gray-500 dark:text-gray-400">Pencapaian {periodLabels[period]?.toLowerCase() || 'bulan ini'}</p>
                    </div>
                    <div className="text-right">
                        <span className="text-3xl font-bold text-indigo-600 dark:text-indigo-400">{canvasing.approved}</span>
                ...
                <div className="flex justify-between text-xs font-medium">
                    <span className="text-indigo-600 dark:text-indigo-400">{canvasing.progress}% Tercapai</span>
                    <span className="text-gray-500">{Math.max(0, target - canvasing.approved)} lagi untuk mencapai target</span>
                </div>
            </div>
                ...
                <div className="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-sm border border-gray-200 dark:border-gray-700">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                        <div className="text-center p-4 bg-yellow-50 dark:bg-yellow-900/20 rounded-xl">
                            <p className="text-3xl font-bold text-yellow-600 dark:text-yellow-400">{points.approvedValue}</p>
                            <p className="text-xs text-yellow-600/70 dark:text-yellow-400/60 font-medium">Poin Disetujui</p>
                        </div>
                        <div className="text-center p-4 bg-gray-50 dark:bg-gray-700/50 rounded-xl">
                            <p className="text-3xl font-bold text-gray-700 dark:text-gray-200">{points.pendingValue}</p>
                            <p className="text-xs text-gray-500 font-medium">Poin Pending</p>
                        </div>
                        <div className="text-center p-4 bg-gray-50 dark:bg-gray-700/50 rounded-xl">
                            <p className="text-3xl font-bold text-gray-700 dark:text-gray-200">{points.total}</p>
                        ...
                        <div className="text-center p-4 bg-linear-to-br from-emerald-500 to-teal-600 rounded-xl text-white">
                            ...
                            <p className="text-xs font-medium text-emerald-100">Total Poin (Semua)</p>
                        </div>
                    </div>
                </div>
            )}
                                    ...
                                    <div className="text-right flex items-center gap-2">
                                        {activity.pointClaim && (
                                            <span className={`text-xs px-2 py-0.5 rounded-full ${
                                                ...
                                                ⭐ {activity.pointClaim.pointValue}
                                            </span>
                                        )}
