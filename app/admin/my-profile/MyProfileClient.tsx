...
export default function MyProfileClient() {
  ...
  const handlePhotoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const response = await fetchWithHandling<{ image: string }>(
          ...
          {editMode ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-4">
              ...
              <InfoItem
                ...
                value={
                  profile?.workingHourMode === "FLEXIBLE"
                    ? "Fleksibel"
                    : profile?.workingHourMode === "SHIFT"
                      ? "Shift Kerja"
...
function InfoItem({
  ...
  return (
    <div className="flex items-start gap-4 py-3 group">
      <div
        className={`w-10 h-10 ${bgColor} dark:bg-gray-700 rounded-xl flex items-center justify-center shrink-0 shadow-xs group-hover:scale-110 transition-transform`}
      >
        {icon}
      </div>
      <div className="min-w-0">
        ...
        <p className="font-semibold text-gray-800 dark:text-gray-200 wrap-break-word">
          {value || "-"}
        </p>
      </div>
    </div>
