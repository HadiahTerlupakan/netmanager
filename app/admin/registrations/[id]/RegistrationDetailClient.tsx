...
export function ClientComponent({ params }: { params: Promise<{ id: string }> }) {
    ...
    const getStatusConfig = (status: string) => {
        switch (status) {
            case 'PENDING':
                return { color: 'bg-yellow-100 text-yellow-800 border-yellow-300', icon: MdAccessTime, label: 'Menunggu Verifikasi' }
            case 'VERIFIED':
                return { color: 'bg-blue-100 text-blue-800 border-blue-300', icon: MdVerified, label: 'Terverifikasi' }
            case 'REJECTED':
                return { color: 'bg-red-100 text-red-800 border-red-300', icon: MdBlock, label: 'Ditolak' }
            case 'SURVEYED':
                return { color: 'bg-purple-100 text-purple-800 border-purple-300', icon: MdConstruction, label: 'Sudah Disurvei' }
            case 'INSTALLED':
                return { color: 'bg-green-100 text-green-800 border-green-300', icon: MdInstallDesktop, label: 'Terinstal' }
            case 'CANCELLED':
                return { color: 'bg-gray-100 text-gray-800 border-gray-300', icon: MdCancel, label: 'Dibatalkan' }
            default:
                return { color: 'bg-gray-100 text-gray-800 border-gray-300', icon: MdAccessTime, label: status }
        }
    }
    ...
    const getAvailableActions = (status: string) => {
        switch (status) {
            case 'PENDING':
                return [
                    { label: 'Verifikasi', status: 'VERIFIED', color: 'bg-blue-600 hover:bg-blue-700', icon: MdCheckCircle },
                    { label: 'Tolak', status: 'REJECTED', color: 'bg-red-600 hover:bg-red-700', icon: MdBlock, needsReason: true },
                    { label: 'Batalkan', status: 'CANCELLED', color: 'bg-gray-600 hover:bg-gray-700', icon: MdCancel }
                ]
            case 'VERIFIED':
                return [
                    { label: 'Tandai Sudah Survei', status: 'SURVEYED', color: 'bg-purple-600 hover:bg-purple-700', icon: MdConstruction },
                    { label: 'Batalkan', status: 'CANCELLED', color: 'bg-gray-600 hover:bg-gray-700', icon: MdCancel }
                ]
            case 'SURVEYED':
                return [
                    { label: 'Tandai Terinstal', status: 'INSTALLED', color: 'bg-green-600 hover:bg-green-700', icon: MdInstallDesktop },
                    { label: 'Batalkan', status: 'CANCELLED', color: 'bg-gray-600 hover:bg-gray-700', icon: MdCancel }
                ]
            default:
                return []
    ...
    if (error && !registration) {
        return (
            <div className="p-6">
                ...
                <Link href="/admin/registrations" className="mt-4 inline-flex items-center text-blue-600 hover:underline">
        ...
        <div className="p-6">
            ...
            <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-4">
                    <Link href="/admin/registrations" className="p-2 hover:bg-slate-100 rounded-lg transition-colors">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2 space-y-6">
                    ...
                    <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                        <h2 className="text-lg font-semibold text-slate-800 mb-4">Informasi Pelanggan</h2>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="flex items-start gap-3">
                                <MdPerson className="text-xl text-slate-400 mt-0.5" />
                                <div>
                                    <p className="text-sm text-slate-500">Nama</p>
                                    <p className="font-medium text-slate-800">{registration.name}</p>
                                </div>
                            </div>
                            <div className="flex items-start gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="font-medium text-slate-800">{registration.phone}</p>
                                </div>
                            </div>
                            <div className="flex items-start gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="font-medium text-slate-800">{registration.email}</p>
                                </div>
                            </div>
                            <div className="flex items-start gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="font-medium text-slate-800">{registration.packageName || '-'}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    {/* Location Card */}
                    <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                        <h2 className="text-lg font-semibold text-slate-800 mb-4">Lokasi</h2>
                        <div className="space-y-4">
                            <div className="flex items-start gap-3">
                                <MdLocationOn className="text-xl text-slate-400 mt-0.5" />
                                <div>
                                    <p className="text-sm text-slate-500">Area</p>
                                    <p className="font-medium text-slate-800">{registration.location || '-'}</p>
                                </div>
                            </div>
                            <div className="flex items-start gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="font-medium text-slate-800">{registration.address}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div className="space-y-6">
                    ...
                    {actions.length > 0 && (
                        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                            <h2 className="text-lg font-semibold text-slate-800 mb-4">Aksi</h2>
                    ...
                    {/* Meta Info Card */}
                    <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                        <h2 className="text-lg font-semibold text-slate-800 mb-4">Info Teknis</h2>
                            ...
                            {registration.verifiedAt && (
                                <div>
                                    ...
                                    <p className="font-medium text-slate-800">
                                        ...
                                        {registration.verifiedBy && ` oleh ${registration.verifiedBy}`}
                                    </p>
                                </div>
                            )}
                            <div>
                                ...
                                <p className="font-mono text-slate-800">{registration.ipAddress || '-'}</p>
                                {ipInfo && (
