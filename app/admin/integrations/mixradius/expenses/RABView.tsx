...
import type { RABRevisionProfitLossSummary, RABRevisionRecord } from './rabRevisionTypes'
...
export default function RABView({ isOpen, data, onRefresh, onOpenRevision, onClose }: RABViewProps) {
    ...
    const [revisionSummary, setRevisionSummary] = useState<RABRevisionProfitLossSummary | null>(null)
    const [isLoadingRevisionAnalytics, setIsLoadingRevisionAnalytics] = useState(false)
    const [isSubmittingRevisionId, setIsSubmittingRevisionId] = useState<string | null>(null)
    const [isRejectModalOpen, setIsRejectModalOpen] = useState(false)
    const [rejectingRevision, setRejectingRevision] = useState<RABRevisionRecord | null>(null)
    ...
    const loadRevisionAnalytics = useCallback(async (projectId: string) => {
        ...
        try {
            ...
            toast.error(error instanceof Error ? error.message : 'Gagal memuat data revisi RAB')
        } finally {
    ...
    const handleApproveRevision = async (revision: RABRevisionRecord) => {
        ...
        try {
            ...
            toast.error(error instanceof Error ? error.message : 'Gagal menyetujui revisi')
        } finally {
    ...
    const handleConfirmRejectRevision = async () => {
        ...
        try {
            ...
            toast.error(error instanceof Error ? error.message : 'Gagal menolak revisi')
        } finally {
            setIsSubmittingRevisionId(null)
        }
    }
    ...
    return (
        <>
                        ...
                        <div className="flex flex-col gap-3 rounded-xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-700 dark:bg-gray-800 md:flex-row md:items-center md:justify-between">
                            <div>
                                ...
                                <p className="mt-1 text-sm text-gray-600 dark:text-gray-300">
                                    Fokuskan review pada 3 hal: status revisi terbaru, dampak biaya final terhadap realisasi, dan keputusan approval.
                                </p>
                            </div>
                        ...
                        {isLoadingRevisionAnalytics ? (
                            ...
                            <>
                                {revisionSummary?.finalRevisionSummary ? (
                                    ...
                                    <div className="rounded-xl border border-dashed border-gray-300 bg-gray-50 p-6 text-sm text-gray-600 dark:border-gray-700 dark:bg-gray-900/30 dark:text-gray-300">
                                        Belum ada revisi final yang disetujui. Buka form revisi, ajukan approval, lalu setujui agar baseline final bisa dipakai untuk analisa dampak.
                                    </div>
                                )}

                                <div className="grid gap-4 xl:grid-cols-[1.05fr,1.4fr]">
                                    <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-700 dark:bg-gray-800">
                                        <h4 className="text-sm font-semibold text-gray-900 dark:text-white">Timeline Revisi</h4>
                                        <p className="mt-1 text-sm text-gray-600 dark:text-gray-300">
                                            Histori status draf, pengajuan, approval, dan penetapan baseline final.
                                        </p>
                                        <div className="mt-4">
                                            <RABRevisionTimeline
                                                revisions={revisions}
                                                finalApprovedRevisionId={revisionSummary?.finalRevisionSummary?.id ?? data.finalApprovedRevisionId}
                                                canReview={canManage}
                                    ...
                                    <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-700 dark:bg-gray-800">
                                        <h4 className="text-sm font-semibold text-gray-900 dark:text-white">Variance per Item</h4>
                                        <p className="mt-1 text-sm text-gray-600 dark:text-gray-300">
                                            Bandingkan budget final yang disetujui dengan realisasi aktual di level item.
                                        </p>
                                        <div className="mt-4">
                                            {revisionSummary?.finalRevisionSummary ? (
                                                <RABVarianceTable items={revisionSummary.itemVariances} />
                                            ) : (
                                                <div className="rounded-xl border border-dashed border-gray-300 bg-gray-50 p-4 text-sm text-gray-600 dark:border-gray-700 dark:bg-gray-900/30 dark:text-gray-300">
                                                    Variance per item akan tampil setelah ada revisi yang sudah disetujui sebagai baseline final.
                                                </div>
                                            )}
                                        </div>
                                    </div>
            ...
            <Modal
                ...
                title="Tolak Revisi"
                description={rejectingRevision ? `Revisi ${rejectingRevision.revisionNumber}` : undefined}
                size="lg"
            >
                <ModalBody>
                    <div className="space-y-3">
                        <p className="text-sm text-slate-600 dark:text-slate-300">
                            Tambahkan catatan agar pembuat revisi tahu apa yang harus diperbaiki sebelum mengajukan ulang.
                        </p>
                        <textarea
                            value={rejectNotes}
