...
export default function CategoryList() {
    ...
    const buildTree = (
      ...
      return cats
        ...
        .map((cat) => {
          ...
          const totalRecursive =
            (cat.totalDirect || 0) +
            children.reduce((sum, child) => sum + child.totalRecursive, 0);
  ...
  const handleDelete = async (id: string) => {
    if (
      !confirm("Hapus kategori ini? Sub-kategori juga akan terhapus jika ada.")
    )
      return;
  ...
  const CategoryItem = ({
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm mb-4 overflow-hidden transition-all hover:shadow-md">
          ...
          <div className="relative flex items-center justify-between p-4 group">
            <div className="flex items-center gap-4 flex-1 min-w-0">
              ...
              <div
                className={`
                            ...
                            `}
              >
                <span className="absolute -bottom-2 -right-1 text-4xl opacity-10 font-black">
                  {parentIndexString}
                </span>
            ...
            <div className="flex items-center gap-4 sm:gap-6">
              <div
                className={`text-right flex flex-col items-end ${hasAmount ? "opacity-100" : "opacity-40"}`}
              >
                <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">
                  Total
                </span>
                <span
                  className={`text-lg font-black font-mono ${selectedType === "OPEX" ? "text-orange-600 dark:text-orange-400" : "text-purple-600 dark:text-purple-400"}`}
                >
                ...
                {hasChildren && item.totalDirect > 0 && (
                  <span
                    ...
                    title="Langsung di kategori ini"
                  >
              ...
              <div className="hidden sm:flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    handleOpenModal(undefined, item.id);
                  }}
    ...
    return (
      <div className="relative pl-4 mt-1">
        ...
        <div
          className={`absolute left-0 top-0 w-[2px] bg-gray-200 dark:bg-gray-700/50 rounded-full my-1 ${isLast ? "h-[20px] bottom-auto" : "bottom-0"}`}
        ></div>
        ...
        <div className="pr-2 py-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700/50 transition-colors group flex flex-col sm:flex-row sm:items-center justify-between gap-2 sm:gap-0">
          <div className="flex items-center gap-3 flex-1 min-w-0 pl-1">
            ...
            {hasChildren && (
              <button
                ...
                className="flex items-center gap-1 text-[10px] px-1.5 py-0.5 bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600 rounded text-gray-500 font-medium transition-colors cursor-pointer"
              >
          ...
          <div className="flex items-center gap-4 pl-8 sm:pl-0">
            <span
              className={`font-mono text-sm font-bold ${hasAmount ? "text-gray-700 dark:text-gray-300" : "text-gray-300 dark:text-gray-600"}`}
            >
              {formatCurrency(item.totalRecursive)}
            </span>
      ...
      <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-4 bg-white dark:bg-gray-800 p-4 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm">
        ...
        <div className="flex items-center gap-4 w-full xl:w-auto justify-between xl:justify-end">
          ...
          <div className="text-right px-4">
            ...
            <p
              className={`text-xl font-black ${selectedType === "OPEX" ? "text-orange-600" : "text-purple-600"}`}
            >
      ...
      <div className="min-h-[400px]">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center h-80 text-gray-400 border-2 border-dashed border-gray-200 dark:border-gray-800 rounded-2xl bg-gray-50/50 dark:bg-gray-800/30">
            ...
            <p className="text-sm">
              Mulai dengan menambahkan induk kategori baru untuk {selectedType}
            </p>
          </div>
        ) : (
          <div className="space-y-2 pb-10">
            {categoryTree.map((cat, index) => (
              <CategoryItem
                key={cat.id}
                item={cat}
                parentIndexString={(index + 1).toString()}
              />
            ))}
          </div>
        )}
