...
export default function MixRadiusAccountsClient() {
  ...
  const fetchData = async () => {
    try {
      ...
      const mapped = rawConfigs.map(
        (c: Partial<MixRadiusConfig> & { apiUrl?: string }) => ({
          ...c,
  ...
  const handleDelete = async (id: string, name: string) => {
    if (
      !confirm(
        `Hapus akun "${name}"? Data yang sudah ada mungkin tidak bisa diakses jika akun dihapus.`,
      )
    )
      return;
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400 mt-1">
            Kelola multiple akun/server MixRadius. Pilih satu akun sebagai yang{" "}
            <strong>Aktif</strong>.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 mt-1">
              Sertakan protokol (http/https) dan port.
            </p>
          </div>
