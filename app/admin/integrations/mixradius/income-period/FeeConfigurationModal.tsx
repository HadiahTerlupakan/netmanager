...
export default function FeeConfigurationModal({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      clientLogger.error("Gagal menyimpan konfigurasi fee", error);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title="Konfigurasi Fee Payment Gateway"
    >
      <div className="space-y-4">
        <div className="bg-blue-50 dark:bg-blue-900/20 text-blue-800 dark:text-blue-200 p-3 rounded-lg text-sm">
          Tentukan biaya potongan untuk setiap metode pembayaran agar Pendapatan
          Bersih dapat dihitung.
        </div>
        ...
        <div className="space-y-3 max-h-[60vh] overflow-y-auto pr-2">
          {Object.entries(fees).map(([method, config]) => (
            <div
              key={method}
              className="flex items-center gap-2 bg-gray-50 dark:bg-gray-700/50 p-2 rounded-lg border border-gray-200 dark:border-gray-600"
            >
              ...
              <input
                ...
                value={config.value}
                onChange={(e) =>
                  updateFee(method, "value", parseFloat(e.target.value))
                }
