...
export default function MixRadiusClient({
  ...
  useEffect(() => {
    if (Object.keys(invoiceCounts).length > 0) {
      localStorage.setItem(
        "mixradius_invoice_counts",
        JSON.stringify(invoiceCounts),
      );
    }
  }, [invoiceCounts]);
      ...
      for (let i = 0; i < ids.length; i += INVOICE_FETCH_CHUNK_SIZE) {
        ...
        try {
          const res = await fetch(
            "/api/integrations/mixradius/invoice-counts",
            {
              method: "POST",
  ...
  const handleDismantle = async () => {
    ...
    if (
      !confirm(
        `Apakah Anda yakin ingin mengajukan bongkar untuk pelanggan ${selectedCustomer.username}?`,
      )
    ) {
      return;
    }
  ...
  const fetchData = useCallback(
    async (forceRefresh = false) => {
      ...
      try {
        ...
        const response = await fetch(
          `/api/integrations/mixradius/customers?${params}`,
        );
        ...
        const errorMsg =
          err instanceof Error
            ? err.message
            : "Gagal mengambil data dari MixRadius";
        setError(errorMsg);
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <button
          ...
          disabled={loading}
          className="inline-flex items-center gap-2 px-4 py-2 bg-blue-600 dark:bg-blue-500 text-white rounded-lg hover:bg-blue-700 dark:hover:bg-blue-600 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
        >
          <HiOutlineArrowPath
      ...
      <div className="flex gap-2">
        ...
        <div className="relative flex-1">
          ...
          <input
            ...
            placeholder={
              searchType === "all"
                ? "Cari nama, username, ID..."
                : `Cari berdasarkan ${searchType === "member_id" ? "ID" : searchType === "fullname" ? "nama" : searchType}...`
            }
            value={search}
        ...
        {detailLoading ? (
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2 text-xs text-green-700 dark:text-green-400">
                        ...
                        {selectedCustomer.quota_usage && (
                          <span>| Quota: {selectedCustomer.quota_usage}</span>
                        )}
                      </div>
                    </div>
                  </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-y-4 gap-x-6">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-medium text-gray-900 dark:text-white">
                        {selectedCustomer.service_type || "PPPOE"}
                      </p>
                    </div>
                    <div>
                      <label className="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider block mb-1">
                      ...
                      <p className="text-sm text-gray-900 dark:text-white uppercase">
                        {selectedCustomer.ip_type?.replace(
                          "automatic",
                          "DYNAMIC",
                        ) || "-"}
                      </p>
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-900 dark:text-white uppercase">
                        {selectedCustomer.subscription_type?.replace("_", " ")}
                      </p>
                    </div>
                    <div>
                      <label className="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider block mb-1">
                            ...
                            <td className="px-3 py-3 text-sm text-gray-500 dark:text-gray-400">
                              <div className="flex flex-col gap-1">
                                <span className="text-xs">
                                  Aktif: {inv.activation_date}
                                </span>
                                <span className="text-xs text-red-500">
                                  Exp: {inv.deadline_date}
                                </span>
                              </div>
                            </td>
          ...
          <div className="py-12 text-center">
            ...
            <p className="text-gray-500 dark:text-gray-400">
              Tidak ada data pelanggan yang dipilih
            </p>
          </div>
        )}
