...
import { HiArrowPathRoundedSquare, HiOutlineEye, HiPencil, HiPrinter, HiTrash, HiArrowPath, HiNoSymbol, HiXMark } from 'react-icons/hi2'
...
export function createPppListColumns(disableDuration: number): Column<PelangganPPP>[] {
  return [
    {
      key: 'nama',
...
export function renderPppListActions(item: PelangganPPP, allowed: boolean, disableDuration: number, onDelete: (id: string) => void, onStatusUpdate: (id: string, newStatus: string, actionName: string) => void) {
  return (
    <div className="flex items-center justify-end gap-1.5">
      {item.status !== 'ISOLIR' && (
        <button onClick={() => onStatusUpdate(item.id, 'ISOLIR', 'ISOLIR')} title="Isolir (Blokir Akses)" className="p-2 text-orange-600 bg-orange-50 hover:bg-orange-100 dark:bg-orange-500/10 dark:hover:bg-orange-500/20 dark:text-orange-400 rounded-lg transition-colors">
          <HiNoSymbol className="w-4 h-4" />
        </button>
      )}
      {item.status !== 'DISMANTLE' && (
        <button onClick={() => onStatusUpdate(item.id, 'DISMANTLE', 'DISMANTLE')} title="Dismantle (Berhenti Langganan)" className="p-2 text-rose-600 bg-rose-50 hover:bg-rose-100 dark:bg-rose-500/10 dark:hover:bg-rose-500/20 dark:text-rose-400 rounded-lg transition-colors">
          <HiXMark className="w-4 h-4" />
        </button>
      )}
      {['ISOLIR', 'DISMANTLE', 'NONAKTIF'].includes(item.status) && (
        <button onClick={() => onStatusUpdate(item.id, 'AKTIF', 'AKTIF')} title="Aktifkan Kembali" className="p-2 text-emerald-600 bg-emerald-50 hover:bg-emerald-100 dark:bg-emerald-500/10 dark:hover:bg-emerald-500/20 dark:text-emerald-400 rounded-lg transition-colors">
      ...
      <button onClick={() => onDelete(item.id)} title="Hapus Pelanggan" className="p-2 text-rose-600 bg-rose-50 hover:bg-rose-100 dark:bg-rose-500/10 dark:hover:bg-rose-500/20 dark:text-rose-400 rounded-lg transition-colors">
