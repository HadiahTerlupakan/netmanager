...
import {
  isSupportedKtpImage,
  mergeKtpDataIntoPppForm,
  scanKtpOcr,
} from "@/app/admin/pelanggan/ppp/shared/ktpScan";
...
export function usePppDocumentUploads<T extends CommonPppFormData>({
  setFormData,
  setActiveTab,
}: UsePppDocumentUploadsOptions<T>) {
  ...
  const handleScanKTP = async (file?: File) => {
    ...
    try {
      ...
      clientLogger.error("Error scanning KTP:", err);
      const errorMessage =
    ...
    if (file && file.type.startsWith("image/")) {
      ...
      setTimeout(async () => {
        try {
          ...
          clientLogger.error("Error in onChange handler:", error);
  ...
  return {
    ...
    handleSidebarRumahSekitarFileChange: (file: File | null) =>
      setFileRumahSekitar(file),
