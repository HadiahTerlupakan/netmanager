...
export function validatePppClientForm(
  ...
  if (!/^\d{8}$/.test(formData.idPelanggan.trim())) return 'ID Pelanggan harus 8 digit angka'
  ...
  if (!formData.passwordLogin.trim()) return 'Password Login Portal harus diisi'
  if (options.mode === 'create' && !formData.siteId) return 'Site harus dipilih'
...
export function buildPppClientFormData<T extends Record<string, unknown>>(
  ...
  if (files.fileRumahSekitar) formData.append('fileRumahSekitar', files.fileRumahSekitar)
...
export async function checkPppIdExists(idPelanggan: string): Promise<boolean> {
  ...
  const res = await fetch(`/api/pelanggan-ppp/check-id?idPelanggan=${encodeURIComponent(idPelanggan)}`)
  if (!res.ok) {
    return false
  }
...
export function applyPppClientFieldChange<T extends PppClientFormCoreFields>(params: {
  prev: T
  name: string
  value: string
  checked: boolean
  type: string
  calculateJatuhTempo: (tanggalAktif: string | undefined, hargaPaketId: string | undefined) => string
}): {
  updated: T
