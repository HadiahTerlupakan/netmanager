...
const FormField = ({
  ...
  <div
    className={`flex flex-col sm:flex-row sm:items-baseline border-b border-gray-100 dark:border-gray-800 py-3 last:border-0 ${className}`}
  >
    <dt className="w-full sm:w-1/3 text-sm font-medium text-gray-500 dark:text-gray-400">
      {label}
    </dt>
...
export async function PppClientDetailView({
  params,
}: {
    ...
    return (
      <div className="max-w-4xl mx-auto p-6">
        <div className="bg-red-50 text-red-600 p-4 rounded-lg">
          ...
          <Link href="/admin/pelanggan/ppp" className="underline font-bold">
            Kembali
          </Link>
        </div>
      </div>
            ...
            <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-6">
              ...
              <div className="flex flex-col items-end gap-2">
                <div className="flex items-center gap-2 bg-gray-100 dark:bg-gray-800 px-3 py-1.5 rounded-md border border-gray-200 dark:border-gray-700">
                  ...
                  <StatusBadge
                    status={
                      pelanggan.status as React.ComponentProps<
                        typeof StatusBadge
                      >["status"]
                    }
                  />
              ...
              <section>
                ...
                <div className="bg-indigo-50 dark:bg-indigo-950/20 rounded-lg px-4 border border-indigo-100 dark:border-indigo-900/30">
                  ...
                  <FormField
                    ...
                    value={pelanggan.hargaPaket?.bandwidth?.name || "-"}
                  />
                  <FormField
                    label="Jenis Layanan"
                    value={
                    ...
                    value={
                      pelanggan.jatuhTempo ? (
                        <div className="flex items-center gap-2">
                          <span>
                            Setiap tanggal{" "}
                            {new Date(pelanggan.jatuhTempo).getDate()}
                          </span>
              ...
              <section>
                ...
                <div className="bg-gray-50 dark:bg-gray-800/30 rounded-lg px-4 border border-gray-100 dark:border-gray-800">
                  <FormField
                    ...
                    value={
                      <code className="bg-gray-100 dark:bg-gray-800 px-2 py-0.5 rounded border border-gray-200 dark:border-gray-700 font-mono text-indigo-600 dark:text-indigo-400">
                        {pelanggan.username}
                      </code>
                    }
                  />
                  <FormField
                    label="Profile Plan"
                    value={pelanggan.hargaPaket?.profilePPP?.name || "-"}
                  />
                  <FormField
                    label="IP Address (Static)"
                    value={staticIpAddress}
                    source={staticIpSource}
                  />
                  <FormField
                    label="Server / Router"
                    value={serverRouterName}
                    source={serverRouterSource}
                  />
                  <FormField
                    label="ODP / Port"
                    value={odpPortValue}
                    source={odpPortSource}
                  />
                  <FormField
                    label="SN Perangkat (ONT)"
                    value={"-"} /* Field placeholder if needed */
                  />
                </div>
              </section>
                  ...
                  <div className="grid grid-cols-2 gap-4 pt-2">
                    {pelanggan.fileKTP && (
                      <a
                        ...
                        className="flex items-center gap-2 p-3 rounded-lg border border-dashed border-gray-300 dark:border-gray-700 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors group"
                      >
                        <HiOutlinePhoto className="w-5 h-5 text-gray-400 group-hover:text-indigo-500" />
                        <span className="text-sm font-medium text-gray-600 dark:text-gray-300 group-hover:text-indigo-600 dark:group-hover:text-indigo-400">
                          Foto Identitas/KTP
                        </span>
                      </a>
                    )}
