...
} from "./customerInvoiceHistoryApi";
...
const isCustomerInvoiceHistoryItem = (
  value: unknown,
): value is CustomerInvoiceHistoryItem => {
  if (!isRecord(value)) return false;
  ...
  return (
    ...
    (payment === undefined ||
      payment === null ||
      (Array.isArray(payment) && payment.every(isInvoicePayment)))
  );
};

const parseCustomerInvoices = (
  response: CustomerInvoicesResponse,
    ...
    try {
      ...
      if (response.success) {
        ...
        if (
          ...
          response.data.length > parsedInvoices.length
        ) {
                ...
                <div className="bg-gray-50 dark:bg-gray-800/50 p-4 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                  <div>
                    ...
                    <div className="text-xs text-gray-500">
                      Dibuat: {formatDate(invoice.createdAt)} | Jatuh Tempo:{" "}
                ...
                {invoice.payment && invoice.payment.length > 0 && (
                  <div className="divide-y divide-gray-100 dark:divide-gray-800">
                    {invoice.payment.map((payment) => (
                      <div
                        key={payment.id}
                        className="p-4 flex flex-col sm:flex-row justify-between items-center sm:items-start gap-3 bg-white dark:bg-gray-900"
                      >
                        <div>
                          <div className="text-sm font-medium text-gray-900 dark:text-white flex items-center gap-2">
                            <span>
                              Pembayaran ({payment.paymentMethod || "Manual"})
                            </span>
                            <span
        ...
        <div className="space-y-4">
          ...
          <div className="bg-red-50 dark:bg-red-900/20 p-4 border border-red-200 dark:border-red-800 rounded-lg">
            <ul className="list-disc pl-5 text-sm text-red-700 dark:text-red-400 space-y-1">
              <li>
                Tagihan ini akan kembali berstatus{" "}
                <strong>Belum Lunas (Unpaid)</strong>.
              </li>
              <li>
                Status pelanggan akan <strong>otomatis menjadi ISOLIR</strong>.
              </li>
