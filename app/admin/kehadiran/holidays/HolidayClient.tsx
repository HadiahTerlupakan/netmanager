...
export function HolidayClient() {
  ...
  const fetchHolidays = useCallback(async () => {
    ...
    try {
      const res = await getWithAuth(
        `/api/admin/holidays?year=${currentDate.getFullYear()}`,
      );
      if (res.ok) {
      ...
      clientLogger.error("Failed to fetch holidays", err);
    } finally {
      setLoading(false);
    }
  }, [currentDate]);
  ...
  const handleDateClick = (dateStr: string) => {
    ...
    if (holiday) {
      alert(
        "Tanggal ini sudah ada event: " +
          holiday.description +
          ". Hapus dulu jika ingin mengganti.",
      );
      return;
    }
    setDescription("");
  ...
  const handleDelete = async (id: string, e: React.MouseEvent) => {
    ...
    try {
      ...
      clientLogger.error("Gagal menghapus hari libur", err);
    } finally {
      setDeletingHolidayId((currentId) =>
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      clientLogger.error("Gagal menyimpan hari libur", err);
    }
  };

  if (!currentDate) {
    return (
      <div className="p-8 text-center text-gray-500">Memuat kalender...</div>
    );
  }
  ...
  const blankKeys = Array.from(
    { length: firstDay },
    (_, blankOffset) => `blank-${year}-${month}-${blankOffset + 1}`,
  );
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
        <div className="grid grid-cols-7 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/50">
          {DAYS.map((day) => (
            <div
              key={day}
              className={`p-4 text-center font-semibold text-sm ${day === "Minggu" ? "text-red-500 dark:text-red-400" : "text-gray-600 dark:text-gray-300"}`}
            >
              {day}
            </div>
          ))}
            ...
            const dayCellProps = canCreate
              ? {
                  ...
                  onKeyDown: (event: React.KeyboardEvent<HTMLDivElement>) =>
                    handleDayKeyDown(event, canCreate, () =>
      ...
      <div className="flex items-center gap-4 text-sm text-gray-500 dark:text-gray-400">
        ...
        {canCreate && (
          <span className="ml-auto">
            Klik tanggal untuk menambah/edit libur
          </span>
        )}
      </div>
