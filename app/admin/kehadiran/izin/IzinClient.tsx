...
export function IzinClient() {
  ...
  const handleManualSubmit = async (e: React.FormEvent) => {
    ...
    if (manualForm.type === "TUKAR_LIBUR" && !manualForm.replacementDate) {
      toast.error("Tanggal pengganti wajib diisi untuk Tukar Libur");
      return;
    }
    ...
    try {
      ...
      const payload = {
        ...manualForm,
        replacementDate:
          manualForm.type === "TUKAR_LIBUR"
            ? manualForm.replacementDate
            : undefined,
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      default:
        return (
          <span className="px-2 py-1 rounded-full text-xs font-bold bg-yellow-100 text-yellow-700 flex items-center gap-1">
            <MdPending /> Menunggu
          </span>
        );
    }
    ...
    },
    {
      ...
      render: (req) => (
        <span className="text-gray-500">
          {format(new Date(req.startDate), "dd MMM yyyy", { locale: id })} -{" "}
          {format(new Date(req.endDate), "dd MMM yyyy", { locale: id })}
        </span>
      ),
    },
    {
  ...
  return (
    <div className="p-6">
      <div className="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        {canCreate && (
          <Button onClick={handleOpenManualModal} variant="default">
          ...
          <>
            <div className="space-y-6">
              <div className="flex items-center gap-4 p-4 bg-gray-50 dark:bg-gray-700/50 rounded-xl border border-gray-100 dark:border-gray-700">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500 dark:text-gray-400">
                    ...
                    {selectedRequest.user.site?.name}
                  </p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                {selectedRequest.replacementDate && (
                  <div className="md:col-span-2">
                    <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider block mb-1.5">
                      Tanggal Pengganti
                    </label>
                    <div className="font-medium text-gray-900 dark:text-white bg-gray-50 dark:bg-gray-800 px-3 py-2 rounded-lg border border-gray-100 dark:border-gray-700">
                      {format(
                        new Date(selectedRequest.replacementDate),
                        "dd MMMM yyyy",
                        { locale: id },
                      )}
                    </div>
                ...
                <div className="flex gap-2 w-full justify-end">
                  {canDelete && (
                    <Button
                      ...
                      disabled={actionLoading}
                      variant="ghost"
                      className="text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/50 mr-auto"
                    >
            ...
            <div>
              ...
              <div className="flex items-center gap-2">
                ...
                {manualForm.attachmentUrl && (
                  <a
                    href={manualForm.attachmentUrl}
                    target="_blank"
                    rel="noreferrer"
                    className="p-2 bg-blue-50 text-blue-600 rounded-lg text-xs font-bold hover:bg-blue-100"
                  >
                    Lihat
                  </a>
                )}
          ...
          {manualForm.type === "TUKAR_LIBUR" && (
            <div>
              ...
              <input
                type="date"
                className="w-full p-2.5 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none"
                value={manualForm.replacementDate}
                onChange={(e) =>
                  setManualForm({
                    ...manualForm,
