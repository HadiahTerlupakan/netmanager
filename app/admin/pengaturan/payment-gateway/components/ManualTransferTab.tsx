...
export default function ManualTransferTab() {
  ...
  const handleSave = async () => {
    ...
    if (result.success) {
      ...
      alert(
        `Gagal menyimpan: ${result.message ?? "Tidak ada detail tambahan."}`,
      );
    }
  };
  ...
  const handleDelete = async (id: string) => {
    ...
    if (result.success) {
      ...
      alert(
        `Gagal menghapus rekening: ${result.message ?? "Tidak ada detail tambahan."}`,
      );
    }
  };
  ...
  const handleToggleActive = async (account: CompanyBankAccount) => {
    ...
    if (!result.success) {
      clientLogger.error("Error toggling status:", result.message);
    }
  };
