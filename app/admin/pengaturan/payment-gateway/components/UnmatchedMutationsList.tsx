...
export default function UnmatchedMutationsList() {
  ...
  const handleAction = async (id: string, action: "RESOLVE" | "IGNORE") => {
    ...
    if (
      action === "IGNORE" &&
      !confirm("Yakin ingin mengabaikan mutasi ini?")
    ) {
      return;
    }
    ...
    try {
      ...
      clientLogger.error("Action error:", error);
    ...
    <div className="mt-8 pt-8 border-t border-gray-200 dark:border-gray-700">
      <div className="mb-6">
        ...
        <p className="text-gray-600 dark:text-gray-400 mt-1">
          Daftar transfer masuk dari Payment Gateway (seperti Moota) yang
          nominalnya tidak cocok dengan tagihan manapun.
        </p>
      </div>
      ...
      {mutations.length === 0 ? (
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-8 text-center border border-gray-200 dark:border-gray-700">
          ...
          <p className="text-gray-500 dark:text-gray-500 mt-2">
            Tidak ada mutasi menggantung yang memerlukan tindakan manual saat
            ini.
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 gap-6">
          {mutations.map((mutation) => (
            <div
              key={mutation.id}
              className="bg-white dark:bg-gray-800 rounded-xl shadow-md border-l-4 border-amber-500 overflow-hidden"
            >
                ...
                <div className="flex-1 space-y-4">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      {mutation.type && (
                        <span
                          className={`px-3 py-1 rounded-lg text-sm font-medium ${mutation.type === "CR" ? "bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400" : "bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400"}`}
                        >
                  ...
                  <div>
                    ...
                    <div className="flex gap-2">
                      <input
                        ...
                        onChange={(e) =>
                          setResolveInvoiceId((prev) => ({
                            ...prev,
                            [mutation.id]: e.target.value,
                          }))
                        }
                      />
                      <button
