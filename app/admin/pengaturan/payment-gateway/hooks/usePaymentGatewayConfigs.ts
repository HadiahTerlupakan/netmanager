...
export function usePaymentGatewayConfigs() {
    ...
    const fetchConfigs = useCallback(async () => {
        try {
            ...
            if (response.ok) {
                ...
                console.error('Failed to fetch payment gateway configs:', response.statusText)
            }
        } catch (error) {
    ...
    const toggleProvider = useCallback(
        async (providerId: string, isEnabled: boolean): Promise<PaymentActionResult> => {
            try {
                ...
                return {
                    success: false,
                    message: getPayloadMessage(payload) ?? 'Gagal mengupdate status provider',
                }
            } catch (error) {
                console.error('Error toggling provider:', error)
                return { success: false, message: 'Kesalahan saat mengupdate provider' }
            }
        },
        [fetchConfigs]
    )
    ...
    const saveConfig = useCallback(
        async (providerId: string, payload: PaymentGatewayFormPayload): Promise<PaymentActionResult> => {
            try {
                setSaving(true)
                const response = await fetch(`${CONFIGS_URL}/${providerId}`, {
                    method: 'PUT',
                ...
                return { success: false, message: 'Kesalahan saat menyimpan konfigurasi' }
            } finally {
                setSaving(false)
            }
        },
    ...
    const testConnection = useCallback(
        async (payload: PaymentGatewayTestPayload): Promise<PaymentActionResult> => {
            try {
                setTesting(true)
                ...
                const body = await safeParseJson(response)
                const successFlag = body && typeof (body as Record<string, unknown>).success === 'boolean'
                    ? (body as Record<string, unknown>).success as boolean
                    : response.ok

                return {
                    success: successFlag,
                    message: getPayloadMessage(body) ?? (successFlag ? 'Koneksi berhasil' : 'Koneksi gagal'),
                }
            } catch (error) {
