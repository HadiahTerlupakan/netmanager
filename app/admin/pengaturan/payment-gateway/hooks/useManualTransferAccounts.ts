...
export function useManualTransferAccounts() {
  ...
  const upsertAccount = useCallback(
    async (
      formData: ManualTransferFormValues,
      accountId?: string,
    ): Promise<ManualTransferActionResult> => {
      try {
        ...
        clientLogger.error("Error saving bank account:", error);
    ...
    async (
      ...
      try {
        ...
        return {
          success: false,
          message:
            getPayloadMessage(payload) ?? "Gagal memperbarui status rekening",
        };
      } catch (error) {
        clientLogger.error("Error toggling bank account status:", error);
        return {
          success: false,
          message: "Kesalahan saat memperbarui status rekening",
        };
      }
    },
    [fetchAccounts],
