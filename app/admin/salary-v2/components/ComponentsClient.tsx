...
export default function ComponentsClient() {
  ...
  const {
    ...
    isLoading: loading,
    mutate: refetch,
  } = useApi<ComponentsResponse>("/api/admin/salary-v2/components");
  ...
  const handleSave = async () => {
    ...
    try {
      const url = editingId
        ? `/api/admin/salary-v2/components/${editingId}`
        : "/api/admin/salary-v2/components";
  ...
  const handleToggleActive = async (component: PayrollComponent) => {
    try {
      const res = await fetch(
        `/api/admin/salary-v2/components/${component.id}`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
