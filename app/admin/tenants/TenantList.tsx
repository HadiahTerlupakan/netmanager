...
import { HiOutlinePencilSquare, HiOutlineTrash, HiOutlinePlus, HiOutlineBuildingOffice, HiOutlineGlobeAlt } from 'react-icons/hi2'
...
export default function TenantList({ onViewAdmins }: { onViewAdmins: (tenantId: string) => void }) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setIsSaving(true)
        try {
            const url = isEdit ? `/api/admin/tenants/${formData.id}` : '/api/admin/tenants'
            ...
            const data = await res.json()
            if (res.ok) {
                toast.success(isEdit ? 'Tenant berhasil diperbarui' : 'Tenant berhasil dibuat')
                setIsFormOpen(false)
                fetchTenants()
            } else {
    ...
    const handleDelete = async () => {
        ...
        setIsDeleting(true)
        try {
            const res = await fetch(`/api/admin/tenants/${deleteId}`, { method: 'DELETE' })
            const data = await res.json()
            if (res.ok) {
        ...
        },
        {
            ...
            render: (tenant) => (
                <div className="text-sm text-gray-600 dark:text-gray-400 flex items-center gap-1.5">
                    ...
                    {tenant.domain || <span className="text-gray-400 italic">No Domain</span>}
                </div>
            )
        },
        {
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
                {loading ? (
                    ...
                    <div className="text-center py-16">
                        ...
                        <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-1">Tidak ada Tenant</h3>
                        <p className="text-gray-500 dark:text-gray-400 mb-6">Mulai dengan menambahkan tenant baru untuk sistem SaaS Anda.</p>
                        <button
                            onClick={handleOpenCreate}
            ...
            >
                <div className="text-center">
                    ...
                    <p className="text-gray-600 dark:text-gray-400 mb-6 font-medium">
                        Tindakan ini tidak dapat dibatalkan. Menghapus tenant dapat menyebabkan error apabila masih ada data (User, Transaksi, dll) yang terkait dengan tenant ini!
                    </p>
                </div>
                <ModalFooter>
                    <button
