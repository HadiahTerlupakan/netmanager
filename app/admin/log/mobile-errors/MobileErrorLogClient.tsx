...
export default function MobileErrorLogClient() {
    ...
    const fetchLogs = useCallback(async (targetPage = 1, isRefresh = false) => {
        if (isRefresh) {
            setRefreshing(true)
        } else {
            setLoading(true)
        ...
        try {
            ...
            console.error('Failed to fetch mobile error logs:', error)
        } finally {
            setLoading(false)
            setRefreshing(false)
        }
        ...
        <div className="space-y-4">
                ...
                columns={[
                    {
                        ...
                        render: (item) => (
                            <div className="space-y-1">
                                <p className="font-medium">{item.parsedDetails.message || item.subject}</p>
                                <p className="text-xs text-muted-foreground">{item.parsedDetails.screen || '-'} • {item.parsedDetails.route || '-'}</p>
                            </div>
                        ),
                    },
                    {
                        ...
                        render: (item) => {
                            ...
                            return (
                                <span className={`inline-flex rounded-full px-2 py-1 text-xs font-medium ${severityStyles[severity] || severityStyles.error}`}>
                                    {severity}
                                </span>
                            )
                        },
                    ...
                    },
                    {
                        ...
                        header: 'Waktu',
                        priority: 'secondary',
                        render: (item) => format(new Date(item.createdAt), 'dd/MM/yyyy HH:mm:ss'),
                    },
                ]}
            />
            ...
            <div className="flex items-center justify-between text-sm text-muted-foreground">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <span>Halaman {pagination.page} / {Math.max(pagination.totalPages, 1)}</span>
                                ...
                                <div className="space-y-4 text-sm">
                                    ...
                                    <div className="grid gap-4 md:grid-cols-2">
                                        <div>
                                            ...
                                            <p className="mt-1 text-muted-foreground">{details.screen || '-'}</p>
                                        </div>
                                        <div>
                                            <h4 className="font-semibold">Route</h4>
                                            <p className="mt-1 text-muted-foreground">{details.route || '-'}</p>
                                        </div>
                                        <div>
                                            <h4 className="font-semibold">Source</h4>
                                            <p className="mt-1 text-muted-foreground">{details.source || '-'}</p>
                                        </div>
                                        <div>
                                            <h4 className="font-semibold">Severity</h4>
                                            <p className="mt-1 text-muted-foreground">{details.severity || '-'}</p>
                                        </div>
                                        <div>
                                            <h4 className="font-semibold">App Version</h4>
                                            <p className="mt-1 text-muted-foreground">{details.appVersion || '-'}</p>
                                        </div>
                                        <div>
                                            <h4 className="font-semibold">Platform</h4>
                                            <p className="mt-1 text-muted-foreground">{details.platform || '-'}</p>
                                        </div>
                                    </div>
