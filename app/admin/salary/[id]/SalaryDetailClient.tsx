...
export default function SalaryDetailClient({
  ...
  const handleAction = async (
    action: "audit" | "approve" | "paid" | "revise" | "recalculate",
    notes?: string,
  ) => {
    setLoading(true);
  ...
  const earnings = salary.details.filter(
    (d: SalaryDetailItem) => d.type === "EARNING",
  );
  const deductions = salary.details.filter(
    (d: SalaryDetailItem) => d.type === "DEDUCTION",
  );
    ...
    <div className="p-6 space-y-6 max-w-6xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="flex items-center gap-4">
                  <div
                    className={`w-12 h-12 rounded-full flex items-center justify-center text-lg font-bold ${
                      salary.user.employeeType === "KARYAWAN"
            ...
            <CardContent className="p-0">
              ...
              <div className="p-6 border-b border-gray-100 dark:border-gray-800">
                ...
                <div className="space-y-3">
                  ...
                  {earnings
                    .filter(
                      (item: SalaryDetailItem) => item.name !== "Gaji Pokok",
                    )
                    .map((item: SalaryDetailItem) => (
                      <div
                        key={item.id}
              ...
              <div className="p-6 bg-gray-50/50 dark:bg-gray-800/30">
                ...
                <div className="space-y-3">
                  {deductions.length > 0 ? (
                    deductions.map((item: SalaryDetailItem) => (
                      <div
                        key={item.id}
                        className="flex justify-between items-center text-sm"
                      >
                ...
                {salary.revisions.map((rev: SalaryRevision, idx: number) => {
                  ...
                  const getActionLabel = (field: string) => {
                    ...
                    if (field.startsWith("detail:"))
                      return `Ubah Komponen: ${field.replace("detail:", "")}`;
                    return field;
                  };
                  ...
                  const displayReason = isUUID(rev.reason)
                    ? rev.field === "status"
                      ? "Status updated via system/audit"
                      : "Manual adjustment"
                    : rev.reason;
                  ...
                  return (
                    <div
                      key={idx}
                      className="flex gap-4 p-3 bg-gray-50 dark:bg-gray-800 rounded-lg text-sm border border-gray-100 dark:border-gray-700"
                    >
                      <div className="flex-1">
                        ...
                        {rev.field === "status" && (
                          <div className="mt-2 text-xs">
                            <span
                              className={`px-2 py-0.5 rounded ${getStatusColor(rev.oldValue || "")} border border-black/5`}
                            >
                              {rev.oldValue || "-"}
                            </span>
                            <span className="text-gray-400 mx-2">→</span>
                            <span
                              className={`px-2 py-0.5 rounded ${getStatusColor(rev.newValue || "")} border border-black/5`}
                            >
                              {rev.newValue}
                            </span>
                          </div>
        ...
        <div className="space-y-6">
          <Card>
            <CardHeader>
              <CardTitle className="text-sm font-medium uppercase text-gray-500 tracking-wider">
                Aksi & Approval
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
                  ...
                  <>
                    ...
                    {salary.status === "REVISED" && (
                      <div className="p-3 bg-amber-50 border border-amber-100 rounded text-sm text-amber-800 dark:bg-amber-900/20 dark:border-amber-800 dark:text-amber-300 mb-2">
                        Tips: Gunakan tombol{" "}
                        <strong>+ Tambah Pendapatan/Potongan Manual</strong> di
                        bagian rincian gaji untuk menambahkan komponen revisi.
                      </div>
                    )}
              ...
              {/* AUDITED -> Approve */}
              {salary.status === "AUDITED" &&
                hasPermission("salary:approve") && (
                  <div className="space-y-3">
              ...
              {salary.status === "APPROVED" &&
                hasPermission("salary:mark_paid") && (
                  <div className="space-y-3">
              ...
              {salary.status === "PAID" && (
                <div className="bg-emerald-50 border border-emerald-100 p-4 rounded-lg text-center">
                  ...
                  <p className="text-sm text-emerald-700 mt-1">
                    Pada {new Date(salary.paidAt).toLocaleDateString("id-ID")}
                  </p>
                </div>
              )}
              ...
              <div className="pt-4 border-t border-gray-100 dark:border-gray-700 space-y-2">
                ...
                <div className="text-center text-xs text-gray-400">
                  * Gunakan opsi &quot;Save as PDF&quot; saat print untuk
                  download
                </div>
              </div>
            </CardContent>
            ...
            <CardContent className="p-4 text-xs text-gray-500 space-y-2">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-mono">
                  {salary.id.substring(0, 8)}...
                </span>
              </div>
              <div className="flex justify-between">
                <span>Dibuat</span>
                <span className="font-mono">
                  {new Date(salary.createdAt).toLocaleDateString("id-ID")}
                </span>
              </div>
            </CardContent>
          </Card>
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              placeholder={
                adjustData.type === "EARNING"
                  ? "Contoh: Bonus Project, Susulan Lembur"
                  : "Contoh: Potongan Kasbon, Denda"
              }
