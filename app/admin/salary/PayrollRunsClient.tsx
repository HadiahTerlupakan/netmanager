...
export default function PayrollRunsClient() {
  ...
  const apiUrl = useMemo(() => {
    ...
    return `/api/admin/salary/runs?${params}`;
  ...
  const handleCreateRun = async () => {
    setCreating(true);
    try {
      const res = await fetch("/api/admin/salary/runs", {
        method: "POST",
      ...
      if (data.success && data.data?.id) {
        router.push(`/admin/salary/${data.data.id}`);
      } else {
    ...
    <div className="p-6 space-y-6 bg-gray-50 dark:bg-gray-900 min-h-screen">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <Link href="/admin/salary/components">
            <Button variant="outline">
              <HiOutlineCog6Tooth className="w-4 h-4 mr-1" />
              Komponen
            </Button>
          </Link>
          <Link href="/admin/salary/profiles">
            <Button variant="outline">
