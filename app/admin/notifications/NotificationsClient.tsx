...
export function ClientComponent() {
  ...
  const markAllAsRead = async () => {
    try {
      const body: { markAllRead: boolean; type?: FilterType } = {
        markAllRead: true,
      };
      if (activeFilter !== "ALL") {
  ...
  const getTypeIcon = (type: string) => {
    switch (type) {
      ...
      case "ALERT":
        return (
          <div className="p-2 bg-red-100 text-red-600 rounded-lg dark:bg-red-900/20 dark:text-red-400">
            <HiOutlineExclamationTriangle className="w-6 h-6" />
          </div>
        );
      case "TICKET":
        return (
          <div className="p-2 bg-purple-100 text-purple-600 rounded-lg dark:bg-purple-900/20 dark:text-purple-400">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        {notifications.some((n) => !n.isRead) && (
          <Button variant="outline" onClick={markAllAsRead}>
            ...
            {activeFilter === "ALL"
              ? "Tandai Semua Dibaca"
              : `Tandai Semua ${getFilterLabel(activeFilter)} Dibaca`}
          </Button>
        )}
      </div>
          ...
          <div className="p-16 text-center text-gray-500 dark:text-gray-400 flex flex-col items-center">
            ...
            <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-1">
              {activeFilter === "ALL"
                ...
                : `Tidak ada notifikasi ${getFilterLabel(activeFilter)}`}
            </h3>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-4">
                      ...
                      <div className="flex flex-col items-end gap-2 text-right shrink-0">
                        <span
                          className="text-xs text-gray-500 dark:text-gray-500 whitespace-nowrap"
                          title={format(
        ...
        {total > limit && (
          <div className="px-6 py-4 bg-gray-50 dark:bg-gray-800/50 border-t border-gray-100 dark:border-gray-800 flex items-center justify-between">
            <p className="text-sm text-gray-500 dark:text-gray-400">
              ...
              dari <span className="font-medium">{total}</span> notifikasi
            </p>
