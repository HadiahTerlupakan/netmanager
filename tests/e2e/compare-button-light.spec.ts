...
test("Test compare button in light mode", async ({ page }) => {
  await page.emulateMedia({ colorScheme: "light" });

  await page.goto("http://localhost:3000/admin/login");
  await page.waitForLoadState("networkidle");
  await page.screenshot({
    path: "/tmp/light-01-admin-login-page.png",
    fullPage: true,
  });
  ...
  await page.screenshot({ path: "/tmp/light-02-admin-filled-form.png" });
  ...
  await page.screenshot({
    path: "/tmp/light-03-after-admin-login.png",
    fullPage: true,
  });
  ...
  await page.screenshot({
    path: "/tmp/light-04-users-page.png",
    fullPage: true,
  });
  ...
  if (checkboxCount >= 2) {
    ...
    await page.screenshot({
      path: "/tmp/light-05-after-selection.png",
      fullPage: true,
    });
    await page.evaluate(() => window.scrollTo(0, document.body.scrollHeight));
    await page.waitForTimeout(500);
    await page.screenshot({
      path: "/tmp/light-06-compare-bar.png",
      fullPage: true,
    });
  } else {
    console.log(
