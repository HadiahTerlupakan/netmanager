...
describe('POST /api/admin/app-version/upload-url', () => {
  ...
  it('rejects non-apk filenames', async () => {
    const request = new NextRequest('http://localhost/api/admin/app-version/upload-url', {
      method: 'POST',
      body: JSON.stringify({
  ...
  it('passes content disposition when generating presigned upload URL', async () => {
    const request = new NextRequest('http://localhost/api/admin/app-version/upload-url', {
      method: 'POST',
      body: JSON.stringify({
    ...
    expect(mockGenerateR2Key).toHaveBeenCalledWith('app-version', 'release.apk')
