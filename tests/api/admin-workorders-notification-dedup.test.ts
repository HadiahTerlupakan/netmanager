...
vi.mock('@/lib/api', () => ({
  ...
  apiSuccess: (data: unknown, init?: { message?: string }) => NextResponse.json({ success: true, data, message: init?.message }),
  apiError: (message: string, _code?: string, init?: { status?: number }) => NextResponse.json({ success: false, error: message }, { status: init?.status || 500 }),
  ApiErrors: {
...
import { PATCH as workOrderPatch, DELETE as workOrderDelete } from '@/app/api/admin/workorders/[id]/route'
...
describe('admin workorder notification dedup', () => {
  ...
  beforeEach(() => {
    ...
    mockFns.userFindUnique.mockImplementation(async ({ where }: { where: { id: string } }) => {
      if (where.id === 'tech-1') {
        return { id: 'tech-1', siteId: 'site-1', departmentId: 'dept-1', isActive: true }
      }
      return { id: 'admin-1', siteId: 'site-1', departmentId: 'dept-1', isActive: true }
    })
