...
describe('admin attendance export parity', () => {
    ...
    it('exports current labels for the shared attendance fixtures', async () => {
        const noCheckoutFixture = getCrossSurfaceAttendanceFixture('no-checkout-system-closure')
        const warnFixture = getCrossSurfaceAttendanceFixture('outside-geofence-warn-accepted')
        ...
        prismaMock.attendance.findMany.mockResolvedValue([
            {
                ...
                checkOut: noCheckoutFixture!.attendance.checkOut ? new Date(noCheckoutFixture!.attendance.checkOut) : null,
            ...
            },
            {
                ...
                checkOut: warnFixture!.attendance.checkOut ? new Date(warnFixture!.attendance.checkOut) : null,
        ...
        const response = await GET(
            new NextRequest('http://localhost/api/admin/attendance?export=true&page=1&limit=20'),
            {
                session: {
                    user: {
                        id: 'admin-1',
