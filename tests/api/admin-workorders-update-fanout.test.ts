...
vi.mock('@/lib/api', () => ({
  ...
  apiSuccess: (data: unknown, init?: { status?: number; message?: string }) =>
    NextResponse.json({ success: true, data, message: init?.message }, { status: init?.status || 200 }),
  apiError: (message: string, _code?: string, init?: { status?: number }) =>
    NextResponse.json({ success: false, error: message }, { status: init?.status || 500 }),
  ApiErrors: {
...
import { POST as taskPost } from '@/app/api/admin/workorders/[id]/tasks/route'
...
describe('admin workorder update fanout', () => {
  ...
  beforeEach(() => {
    ...
    mockFns.userFindUnique.mockResolvedValue({ siteId: 'site-1', departmentId: 'dept-1', role: 'ADMIN' })
  ...
  it('comments route notifies assignee even without push token and fans out update to observers', async () => {
    ...
    expect(mockFns.onWorkOrderUpdated).toHaveBeenCalledWith(expect.objectContaining({
      id: 'wo-1',
      assignedToId: 'tech-1',
    }), 'Admin One: Teknisi tolong cek ulang', 'Admin One', 'admin-1', ['tech-1'])
  })
  ...
  it('tasks route notifies assignee even without push token and fans out update to observers', async () => {
    const response = await taskPost(
      new NextRequest('http://localhost/api/admin/workorders/wo-1/tasks', {
        method: 'POST',
        body: JSON.stringify({ title: 'Pasang modem', description: 'Segera kerjakan' }),
        headers: { 'content-type': 'application/json' },
      }),
      { session, params: { id: 'wo-1' }, permissions: ['*'] } as never,
    )
    ...
    expect(mockFns.onWorkOrderUpdated).toHaveBeenCalledWith(expect.objectContaining({
      id: 'wo-1',
      assignedToId: 'tech-1',
    }), 'Admin menambahkan tugas: "Pasang modem"', 'Admin One', 'admin-1', ['tech-1'])
  })
})
