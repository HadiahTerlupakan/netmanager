...
describe('auto-approve leave cron route', () => {
  beforeEach(() => {
    ...
    mockFns.headers.mockResolvedValue({
      get: (name: string) => (name === 'authorization' ? 'Bearer cron-secret' : null),
    })
  })
  ...
  it('routes auto-approval through LeaveService.approveLeave for each pending request', async () => {
    ...
    mockFns.approveLeave.mockResolvedValue({ success: true, data: { id: 'leave-1' } })

    const response = await POST(new Request('http://localhost/api/cron/auto-approve-leave', {
      method: 'POST',
