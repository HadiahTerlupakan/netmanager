...
const RUN_HTTP_INTEGRATION_TESTS =
  process.env.RUN_HTTP_INTEGRATION_TESTS === "true" ||
...
describe.skipIf(!RUN_HTTP_INTEGRATION_TESTS)(
...
describe("Response Validators", () => {
  it("validateSuccessResponse works correctly", () => {
    ...
    expect(
      validateSuccessResponse({ success: true, data: [], message: "ok" }),
    ).toBe(true);
  ...
  it("validateErrorResponse works correctly", () => {
    ...
    expect(
      validateErrorResponse({ success: true, error: "test", code: "X" }),
    ).toBe(false);
