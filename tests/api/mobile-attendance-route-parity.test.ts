...
vi.mock('@/lib/api', () => ({
  ...
  apiSuccess: (data: unknown, options?: { headers?: HeadersInit; message?: string }) =>
    NextResponse.json(
      {
        success: true,
        data,
...
describe('mobile attendance route parity', () => {
  ...
  it('passes offlineTime through check-in multipart when _offline_meta is signed', async () => {
    const { POST } = await import('@/app/api/mobile/attendance/check-in/route')
    const formData = new FormData()
    formData.set('location', 'HQ')
    formData.set('_offline_meta', JSON.stringify({
      capturedAt: '2026-03-08T01:00:00.000Z',
      signature: 'valid-signature',
    }))

    const response = await POST(new NextRequest('http://localhost/api/mobile/attendance/check-in', { method: 'POST', body: formData }), {
    ...
    expect(checkInSpy.mock.calls[0]?.[0].offlineTime?.toISOString()).toBe('2026-03-08T01:00:00.000Z')
  })

  it('passes offlineTime through check-out multipart when _offline_meta is signed', async () => {
    const { POST } = await import('@/app/api/mobile/attendance/check-out/route')
    const formData = new FormData()
    formData.set('location', 'HQ')
    formData.set('_offline_meta', JSON.stringify({
      capturedAt: '2026-03-08T01:00:00.000Z',
    ...
    const response = await POST(new NextRequest('http://localhost/api/mobile/attendance/check-out', { method: 'POST', body: formData }), {
      session: { user: { id: 'user-1', tenantId: 'tenant-1' } },
    } as never)
    ...
    expect(checkOutSpy.mock.calls[0]?.[0].offlineTime?.toISOString()).toBe('2026-03-08T01:00:00.000Z')
  })

  it('rejects multipart check-out offline metadata when signature is missing', async () => {
    const { POST } = await import('@/app/api/mobile/attendance/check-out/route')
    const formData = new FormData()
    formData.set('location', 'HQ')
    formData.set('_offline_meta', JSON.stringify({
    ...
    expect(body.error.message).toBe('Data offline harus ditandatangani')
    expect(vi.mocked(AttendanceService.prototype.checkOut)).not.toHaveBeenCalled()
  })
})
