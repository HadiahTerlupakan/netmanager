...
describe('rab project revisions route', () => {
  ...
  it('creates revision 1 by snapshotting the current project and items', async () => {
    ...
    const response = await POST(new NextRequest('http://localhost/api/finance/rab-projects/rab-1/revisions', { method: 'POST' }), {
  ...
  it('lists revisions with bigint-safe serialization', async () => {
    ...
    const response = await GET(new NextRequest('http://localhost/api/finance/rab-projects/rab-1/revisions'), {
      params: Promise.resolve({ id: 'rab-1' }),
    })
