...
describe("restock check route", () => {
  beforeEach(() => {
    ...
    Object.assign(prismaMock, {
      restockSettings: prismaMock.restockSettings,
      barangGudang: prismaMock.barangGudang,
      restockAlerts: prismaMock.restockAlerts,
      user: prismaMock.user,
      notifications: prismaMock.notifications,
    });
  ...
  it("creates inventory alert notifications for concrete recipients instead of orphan records", async () => {
    const response = await POST(
      new NextRequest("http://localhost/api/scheduler/restock-check", {
        method: "POST",
