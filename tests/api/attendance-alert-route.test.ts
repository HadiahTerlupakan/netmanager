...
describe("attendance-alert cron route", () => {
  ...
  it("returns a skip response when the attendance-alert lock is already held", async () => {
    ...
    const response = await GET(
      new NextRequest("http://localhost/api/cron/attendance-alert?type=auto", {
  ...
  it("uses the process-specific lock configuration", async () => {
    ...
    const response = await GET(
      new NextRequest(
        "http://localhost/api/cron/attendance-alert?type=process",
        {
          headers: { authorization: "Bearer cron-secret" },
        },
      ),
    ...
    expect(mockFns.acquireCronLock).toHaveBeenCalledWith(
      "route:attendanceAlert:process",
      60 * 60,
    );
    expect(mockFns.processIncompleteAttendance).not.toHaveBeenCalled();
  });

  it("returns 503 when attendance-alert lock storage is unavailable", async () => {
