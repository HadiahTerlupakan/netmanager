...
describe("firebase admin env contract", () => {
  ...
  it("staging app deployment wires firebase admin credentials from netmanager-secrets", () => {
  ...
  it("staging app deployment keeps firebase realtime database env optional", () => {
    const yaml = readManifest("k8s/staging/app-deployment.yaml");
  ...
  it("staging secret template reserves firebase admin keys", () => {
    const yaml = readManifest("k8s/staging/secrets.yaml");
