...
async function renderStatisticsBar() {
  const { MapStatisticsBar } = await import('@/components/map/MapStatisticsBar')
...
describe('map presentational components', () => {
  it('renders toolbar tabs, search, and active mode banner', async () => {
    ...
    await expect(renderToolbar()).resolves.toContain('Click on map to place ODC, then click Save')
