...
describe("RoleService", () => {
  ...
  describe("createRole", () => {
    ...
    it("should deduplicate permissions", async () => {
      ...
      prismaMock.permission.findMany
        ...
        .mockResolvedValueOnce([{ id: "perm-1" }] as unknown as Permission[]);
      prismaMock.role.create.mockResolvedValueOnce({
        id: "new-role",
        name: "Test Role",
        createdAt: new Date("2026-04-01T00:00:00.000Z"),
  ...
  describe("updateRole", () => {
    it("should throw error if role not found", async () => {
      ...
      await expect(
        service.updateRole("non-existent", {
          name: "New Name",
          permissions: [],
        }),
    ...
    it("should not allow renaming SUPER_ADMIN role", async () => {
      ...
      await expect(
        service.updateRole("super-admin-id", {
  ...
  describe("deleteRole", () => {
    ...
    it("should not allow deleting SUPER_ADMIN role", async () => {
      ...
      await expect(service.deleteRole("super-admin-id")).rejects.toThrow(
        "Tidak dapat menghapus role SUPER_ADMIN",
      );
    });
    ...
    it("should delete role with no users", async () => {
      const mockRole: import("@/modules/roles/domain/entities/RoleEntity").RoleEntity =
        {
          id: "role-1",
          name: "Empty Role",
          createdAt: new Date("2026-04-01T00:00:00.000Z"),
