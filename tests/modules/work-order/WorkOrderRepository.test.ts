...
import { syncWoStatusToTicket } from "@/modules/work-order/services/WorkOrderSyncService";
import {
  WorkOrderStatus,
  WorkOrderType,
  type WorkOrders,
...
describe("WorkOrderRepository", () => {
  ...
  describe("create", () => {
    it("should create work order with PENDING status", async () => {
      ...
      prismaMock.workOrders.create.mockResolvedValueOnce(
        mockWo as unknown as WorkOrders,
      );
  ...
  describe("updateStatus", () => {
    it("should set startedAt when transitioning to IN_PROGRESS", async () => {
      ...
      prismaMock.workOrderUpdates.create.mockResolvedValue(
        createUpdateMock() as unknown as WorkOrderUpdates,
      );
      ...
      expect(syncWoStatusToTicket).not.toHaveBeenCalled();
    });
  ...
  describe("cancel", () => {
    it("should add cancellation note and set status to CANCELLED", async () => {
      prismaMock.workOrderUpdates.create.mockResolvedValue(
        createUpdateMock({ updateType: "NOTE" }) as unknown as WorkOrderUpdates,
      );
      prismaMock.workOrders.findFirst.mockResolvedValue(
        createWoMock() as unknown as WorkOrders,
      );
