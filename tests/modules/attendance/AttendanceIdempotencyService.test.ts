...
describe('AttendanceIdempotencyService', () => {
  ...
  it('marks request as in-progress and blocks duplicates while processing', async () => {
    const payloadHash = service.buildPayloadHash({ latitude: -6.2, longitude: 106.8 })
    const first = await service.begin('user-1', 'check-in', 'req-1', payloadHash)
    const second = await service.begin('user-1', 'check-in', 'req-1', payloadHash)
  ...
  it('returns hash-mismatch when same request id is reused with different payload', async () => {
    const firstHash = service.buildPayloadHash({ location: 'Site A', latitude: 1 })
    const secondHash = service.buildPayloadHash({ location: 'Site B', latitude: 2 })

    const first = await service.begin('user-1', 'check-in', 'req-3', firstHash)
    const second = await service.begin('user-1', 'check-in', 'req-3', secondHash)
  ...
  it('allows retry after in-progress lock is released', async () => {
    ...
    const result = await service.begin('user-1', 'check-in', 'req-4', payloadHash)
    expect(result).toBe('started')
  })

  it('uses deterministic payload hash regardless object key order', () => {
    const hashA = service.buildPayloadHash({ latitude: -6.2, longitude: 106.8, location: 'HQ' })
    const hashB = service.buildPayloadHash({ location: 'HQ', longitude: 106.8, latitude: -6.2 })

    expect(hashA).toBe(hashB)
  })

  it('returns completed when key is already completed with same payload hash', async () => {
    const payloadHash = service.buildPayloadHash({ location: 'Completed Site' })
    await service.begin('user-1', 'check-out', 'req-5', payloadHash)
    await service.complete('user-1', 'check-out', 'req-5', payloadHash, { success: true, data: { id: 'a1' } })

    const beginAgain = await service.begin('user-1', 'check-out', 'req-5', payloadHash)
    expect(beginAgain).toBe('completed')
  })

  it('falls back to memory cache when redis errors', async () => {
    const redisModule = await import('@/lib/redis')
    vi.mocked(redisModule.redis.set).mockRejectedValueOnce(new Error('redis unavailable'))
    vi.mocked(redisModule.redis.get).mockRejectedValueOnce(new Error('redis unavailable'))

    const payloadHash = service.buildPayloadHash({ location: 'Fallback Site' })
    const first = await service.begin('user-fallback', 'check-in', 'req-6', payloadHash)
    expect(first).toBe('started')

    await service.complete('user-fallback', 'check-in', 'req-6', payloadHash, { success: true, data: { id: 'fallback' } })

    vi.mocked(redisModule.redis.get).mockRejectedValueOnce(new Error('redis unavailable'))
    const replay = await service.getReplay('user-fallback', 'check-in', 'req-6')
    expect(replay).toEqual({ success: true, data: { id: 'fallback' } })
  })
})
