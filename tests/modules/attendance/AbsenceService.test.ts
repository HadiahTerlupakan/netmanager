...
describe('AbsenceService canonical absence status', () => {
  ...
  beforeEach(() => {
    ...
    vi.spyOn(HolidayRepository.prototype, 'findMany').mockResolvedValue([] as never)
  ...
  it('creates ABSENT placeholder records instead of ALPHA for new daily absences', async () => {
    const result = await service.processDailyAbsence(new Date('2026-03-02T00:00:00.000Z'), 'tenant-1')
  ...
  it('creates DAY_OFF placeholder records for holidays so they can appear in Data Absensi from DB', async () => {
    ...
    const result = await service.processDailyAbsence(new Date('2026-03-29T00:00:00.000Z'), 'tenant-1')
    ...
    expect(result).toEqual({ processed: 1, absent: 0, dayOff: 1, message: 'Holiday' })
  })
