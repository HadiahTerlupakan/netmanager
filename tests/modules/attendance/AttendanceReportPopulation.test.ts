...
describe('AttendanceService report population parity', () => {
  ...
  beforeEach(() => {
    ...
    mockFns.getUserAttendanceStats.mockResolvedValue([{ userId: 'user-att', _count: { _all: 10 } }])
    mockFns.getUserOvertimeStats.mockResolvedValue([])
    mockFns.getTopAbsentees.mockResolvedValue([{ user: { id: 'user-abs', name: 'Abs Only' }, count: 3 }])
    mockFns.getUserTotalDuration.mockResolvedValue(new Map([['user-att', 600]]))
    mockFns.getUserAbsenceStats.mockResolvedValue([{ userId: 'user-abs', _count: { _all: 3 } }])
  ...
  it('combines ALPHA and ABSENT into summary alphaCount', async () => {
    const result = await service.getReportData(new Date('2026-03-01'), new Date('2026-03-31'))

    expect(result.summary.alphaCount).toBe(4)
  })

  it('keeps absence-only users in employeeSummary when absence stats include them', async () => {
    const result = await service.getReportData(new Date('2026-03-01'), new Date('2026-03-31'))

    expect(result.employeeSummary.map((item) => item.userId)).toContain('user-abs')
    expect(result.employeeSummary.find((item) => item.userId === 'user-abs')).toMatchObject({
      hadir: 0,
      alpha: 3,
    })
  })
