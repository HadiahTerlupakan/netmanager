...
export async function startBaileysSession(
  ...
  try {
    ...
    // Delivery / read receipts → update WhatsAppMessage by key.id
    sock.ev.on("messages.update", (updates) => {
      void handleBaileysMessageStatusUpdates(sessionId, updates);
    });
...
async function handleBaileysMessageStatusUpdates(
  sessionId: string,
