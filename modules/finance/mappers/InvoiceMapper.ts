...
import type { Invoice, InvoiceItem, Payment } from "@prisma/client-billing";
import type {
  InvoiceListItemDTO,
  InvoiceDetailDTO,
  InvoicePortalDTO,
...
export class InvoiceMapper {
  /**
   * Map Prisma invoice aggregate to finance domain entity.
   */
  static toDomain(entity: InvoiceWithRelations): InvoiceEntity {
    return {
      ...entity,
      pelanggan: entity.pelanggan ?? null,
      invoiceItem: entity.items ?? [],
      items: entity.items ?? [],
  ...
  /**
   * Map domain invoice aggregate to detail DTO.
   */
  static toDTO(entity: InvoiceEntity): InvoiceDetailDTO {
    return this.toDetail(entity as InvoiceWithRelations);
  }
  ...
  static toDetail(entity: InvoiceWithRelations): InvoiceDetailDTO {
    ...
    return {
      ...
      invoiceNumber: entity.invoiceNumber,
      issueDate:
        entity.issueDate?.toISOString() ?? entity.createdAt.toISOString(),
      dueDate: entity.dueDate.toISOString(),
      subtotal,
      taxAmount,
      discountAmount,
  ...
  private static mapPayments(
    payments: (Payment & { verifiedAt?: Date | null })[],
  ): InvoicePaymentDTO[] {
