...
import { calculateInvoiceAmounts } from "../utils/invoice-calculator";
...
export class InvoiceFactory {
  ...
  static async createMonthlyInvoice(dto: {
    ...
    const { discountAmount, taxAmount, totalAmount } = calculateInvoiceAmounts({
      subtotal,
      taxRate: dto.taxRate,
  ...
  static async createOneTimeInvoice(dto: {
    ...
    // Calculate amounts
    let subtotal = 0;
    const invoiceItems: PrismaBilling.InvoiceItemCreateManyInvoiceInput[] =
      dto.items.map((item) => {
        ...
        return {
          ...
          totalPrice,
          itemType: (item.itemType ?? "SERVICE") as
            | "SERVICE"
            | "PRODUCT"
            | "SETUP_FEE"
            | "MONTHLY_FEE"
  ...
  static async createProratedInvoice(dto: {
    ...
    const proratedAmount = Math.round(
      (dto.packagePrice / daysInMonth) * remainingDays,
    );
    ...
    const { taxAmount, totalAmount } = calculateInvoiceAmounts({
