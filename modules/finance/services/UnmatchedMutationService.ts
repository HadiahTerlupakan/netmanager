import type { Payment, Prisma, UnmatchedMutation } from '@prisma/client-billing'
...
export class UnmatchedMutationService {
  ...
  async list(params: ListUnmatchedMutationParams): Promise<{
    ...
    const [total, mutations] = await Promise.all([
      ...
      this.unmatchedRepo.findMany(where, { skip, take: limit, orderBy: { date: 'desc' } }),
    ])
  ...
  async resolve(params: ResolveMutationParams): Promise<{ payment: Payment; mutation: UnmatchedMutation }> {
    if (!params.invoiceId) {
