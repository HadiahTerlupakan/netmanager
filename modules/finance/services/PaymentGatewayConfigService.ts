...
type SanitizedPaymentGatewayConfig = Omit<
...
export class PaymentGatewayConfigService {
  ...
  async listConfigs(): Promise<ServiceResult<SanitizedPaymentGatewayConfig[]>> {
    try {
      ...
      return {
        success: false,
        error: "Gagal mengambil konfigurasi payment gateway",
        code: "FETCH_ERROR",
      };
    }
  }
  ...
  async upsertConfig(params: {
    ...
    try {
      ...
      const normalizedMerchantId = normalizeOptionalText(
      ...
      const config = await this.repository.upsertByProviderAndTenant({
        ...
        create: {
          ...
          apiSecret: normalizedApiSecret
            ? encryptApiKey(normalizedApiSecret)
            : null,
        ...
        update: {
          ...(params.input.isEnabled !== undefined
            ...
            : {}),
          ...(params.input.isProduction !== undefined
            ? { isProduction: params.input.isProduction }
            : {}),
          ...(params.input.priority !== undefined
            ? { priority: params.input.priority }
            : {}),
          ...(params.input.apiKey !== undefined
            ? {
            ...
            : {}),
          ...(params.input.apiSecret !== undefined
            ? {
                apiSecret: normalizedApiSecret
                  ? encryptApiKey(normalizedApiSecret)
                  : null,
            ...
            : {}),
          ...(normalizedClientKey !== undefined
            ? { clientKey: normalizedClientKey }
            : {}),
          ...(normalizedMerchantId !== undefined
      ...
      const errorMessage =
        error instanceof Error
          ? error.message
          : "Gagal memperbarui konfigurasi payment gateway";
