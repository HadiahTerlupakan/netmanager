import type { PaymentProvider, ProviderConfig, CreatePaymentParams, PaymentResult, TransactionStatus, WebhookResult, TestResult } from '../provider-interface'
...
export class MootaProvider implements PaymentProvider {
    ...
    verifyWebhook(payload: unknown, signature?: string, rawBody?: string): boolean {
        ...
        try {
            ...
            console.error('[Moota Provider] Signature verification error:', error)
            return false
        }
    }
    ...
    async processWebhook(payload: unknown): Promise<WebhookResult> {
        try {
            ...
            const mutations = Array.isArray(payload) ? payload : (Array.isArray(payloadRecord?.data) ? payloadRecord.data : [])
            ...
            const mutation = Array.isArray(mutations) && mutations.length > 0 ? mutations[0] : payload
            ...
            console.error('[Moota Provider] Error processing webhook:', error)
            return {
                orderId: '',
                status: 'FAILED',
                raw: payload
