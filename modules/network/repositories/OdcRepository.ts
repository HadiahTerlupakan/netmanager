...
import type {
  ...
  OdcEntity,
  OdcUpdateData,
} from "../domain/entities/OdcEntity";
import type { IOdcRepository } from "../domain/ports/IOdcRepository";

export class OdcRepository implements IOdcRepository {
  constructor(private client: PrismaClient = prisma) {}

  async findAll(siteId?: string): Promise<OdcEntity[]> {
    const items = await this.client.odc.findMany({
      where: siteId ? { siteId } : {},
      orderBy: { createdAt: "desc" },
    });
    return items as unknown as OdcEntity[];
  }

  async findById(id: string): Promise<OdcEntity | null> {
    const item = await this.client.odc.findUnique({ where: { id } });
    return item as unknown as OdcEntity | null;
  }
