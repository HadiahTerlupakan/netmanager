...
class SNMPConnectionPool {
  ...
  async getSession(
    ...
    let availableSession = sessionList.find(
      (s) => !s.inUse && Date.now() - s.lastUsed < 30000,
    ); // Reuse if less than 30s old
...
export async function fetchOnuDataPaginated(
  ...
  const [
    ...
    snmpWalkOptimized(ipAddress, port, community, version, oidName, {
      useCache: true,
      timeout: dataTimeout,
    }).catch(() => ({})),
    snmpWalkOptimized(ipAddress, port, community, version, oidDesc, {
      useCache: true,
      timeout: dataTimeout,
    }).catch(() => ({})),
    snmpWalkOptimized(ipAddress, port, community, version, oidRxOltNew, {
      useCache: true,
      timeout: dataTimeout,
    }).catch(() => ({})),
    snmpWalkOptimized(ipAddress, port, community, version, oidRxOnuNew, {
      useCache: true,
      timeout: dataTimeout,
    }).catch(() => ({})),
    snmpWalkOptimized(ipAddress, port, community, version, oidSN, {
      useCache: true,
      timeout: dataTimeout,
    }).catch(() => ({})),
    snmpWalkOptimized(ipAddress, port, community, version, oidActualType, {
      useCache: true,
      timeout: dataTimeout,
    }).catch(() => ({})),
    snmpWalkOptimized(ipAddress, port, community, version, oidPppoe, {
      useCache: true,
      timeout: dataTimeout,
    }).catch(() => ({})),
  ]);
  ...
  for (const idx of indexes) {
    ...
    const actualTypeValue =
      (actualTypeData as Record<string, string>)[idx] || "";
    ...
    if (!pppoeValue) {
      ...
      if (indexParts.length >= 2) {
        ...
        const matchingKeys = Object.keys(
          pppoeData as Record<string, string>,
        ).filter((k) => k.endsWith(`.${onuId}`));
        if (matchingKeys.length > 0) {
          pppoeValue =
            (pppoeData as Record<string, string>)[matchingKeys[0]] || "";
        }
      }
    }
