...
export class WorkOrderRepository implements IWorkOrderRepository {
  ...
  async updateTask(
    ...
    return this.prisma.workOrderTasks.findUnique({
      where: { id: taskId },
    ...
    const topAssists: TopPerformer[] = Object.entries(userStats).map(
      ([name, stats]) => {
        const result: TopPerformer = {
          ...
          count: stats.count,
          avgCompletionTime:
            stats.count > 0 ? stats.totalHours / stats.count : 0,
        };
        if (stats.role) result.role = stats.role;
        if (stats.site) result.site = stats.site;
        return result;
   ...
   */
  async getDepartmentWorkload(departmentId?: string): Promise<
    Array<{
  ...
  > {
    ...
    updates.forEach((update) => {
      ...
      if (departmentId && update.workOrders.departmentId !== departmentId)
        return;
  ...
  async getAdminKPIStats(
    ...
    verifiedWOs.forEach((wo) => {
      if (wo.completedAt && wo.verifiedAt) {
        const diff =
          new Date(wo.verifiedAt).getTime() -
          new Date(wo.completedAt).getTime();
