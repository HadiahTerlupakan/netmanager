...
export class SalaryAuditService {
  ...
  async addManualAdjustment(
    ...
    const { totalEarnings, totalDeductions, netSalary } = calculateSalaryTotals(
      updatedSalary.details,
    );
  ...
  async reviseDetail(
    ...
    // Recalculate totals
    const revisedDetails = salary.details.map((d) => ({
      type: d.type,
      amount: d.name === detailName ? newAmount : d.amount,
    }));
    const { totalEarnings, totalDeductions, netSalary } =
      calculateSalaryTotals(revisedDetails);
