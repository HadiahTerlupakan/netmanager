...
export class SalaryUserRepository {
    ...
    async findManyActiveWithSalaryConfig(where?: Prisma.UserWhereInput): Promise<UserSalaryConfig[]> {
    ...
    async findWorkDaysConfig(userId: string): Promise<{ workDays: string | null } | null> {
        return this.client.user.findUnique({
            where: { id: userId },
...
export class EmployeeLoanRepository {
    ...
    async updateRemainingAmount(id: string, remainingAmount: number, status: string): Promise<EmployeeLoan> {
    ...
    async createLoanPaymentInTx(tx: PrismaClient, data: {
        loanId: string
        amount: number
        notes?: string
    }): Promise<LoanPayment> {
    ...
    async findUniqueInTx(tx: PrismaClient, id: string): Promise<EmployeeLoan | null> {
        return tx.employeeLoan.findUnique({ where: { id } })
    }

    async updateInTx(tx: PrismaClient, id: string, data: { remainingAmount?: number; status?: 'ACTIVE' | 'PAID_OFF' }): Promise<EmployeeLoan> {
        return tx.employeeLoan.update({ where: { id }, data })
    }

    async deleteLoanPaymentInTx(tx: PrismaClient, id: string): Promise<LoanPayment> {
...
export class SalaryAttendanceRepository {
    ...
    async findAttendanceByUserAndPeriod(userId: string, startDate: Date, endDate: Date): Promise<Pick<Attendance, 'status'>[]> {
    ...
    async findOvertimeByUserAndPeriod(userId: string, startDate: Date, endDate: Date): Promise<{ duration: number | null; isHolidayOvertime: boolean; isNationalHoliday: boolean }[]> {
        return this.client.overtime.findMany({
            where: {
                ...
                OR: [
                    ...
                    { AND: [{ startTime: null }, { createdAt: { gte: startDate, lte: endDate } }] }
                ]
            },
            select: { duration: true, isHolidayOvertime: true, isNationalHoliday: true }
        })
    }

    async countWorkOrdersByUserAndPeriod(userId: string, startDate: Date, endDate: Date): Promise<number> {
                ...
                AND: [
                    ...
                    },
                    {
                        OR: [
                            ...
                            { AND: [{ verifiedAt: null }, { closedAt: { gte: startDate, lte: endDate } }] }
                        ]
                    }
                ],
