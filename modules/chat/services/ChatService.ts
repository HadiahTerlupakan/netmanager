...
export class ChatService {
  ...
  async getConversations(userId: string) {
    const conversations =
      await this.repository.findConversationsForUser(userId);
    ...
    return conversations.map((conv) => {
      ...
      return {
        ...
        name: conv.isGlobal
          ? "Global Chat"
          : conv.name || otherParticipants.map((p) => p.name).join(", "),
  ...
  async createConversation(input: CreateChatInput) {
    ...
    if (allParticipantIds.length === 2) {
      const existing =
        await this.repository.findExisting1on1(allParticipantIds);
      if (existing) {
    ...
    if (otherUserIds.length > 0) {
      ...
      otherUserIds.forEach((otherUserId) => {
        this.emitChatMessage(otherUserId, {
          ...
          console.error("[Chat] Broadcast socket emit error:", err),
        );
      });
    }
