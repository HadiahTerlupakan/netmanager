...
export class UserRepository {
     ...
     */
    async findAdminsForNotification(tenantId: string | null | undefined, userSiteId: string | null | undefined) {
    ...
    async findManyWithFullDetails(userIds: string[]) {
        return prisma.user.findMany({
            where: { id: { in: userIds } },
            select: {
                id: true,
    ...
    async findByIdsWithDetails(userIds: string[]): Promise<Array<{id: string, name: string, image: string | null, role: {name: string} | null, departments: {name: string} | null, sites: {name: string} | null}>> {
        return prisma.user.findMany({
            where: { id: { in: userIds } },
            select: {
                id: true,
    ...
    async findManyByDepartmentAndSite(departmentId?: string, siteId?: string, excludeUserId?: string): Promise<Array<{id: string}>> {
        const where: Prisma.UserWhereInput = {
            isActive: true,
            role: {
                permission: {
    ...
    async findManyWithPushToken(userIds: string[]): Promise<Array<{id: string, pushToken: string | null}>> {
        return prisma.user.findMany({
            where: { id: { in: userIds } },
    ...
    async findManyByDepartmentWithPushToken(departmentId: string): Promise<Array<{id: string, pushToken: string | null}>> {
        return prisma.user.findMany({
            where: {
                isActive: true,
                departmentId,
    ...
    async findByIdWithDepartment(userId: string): Promise<{departmentId: string | null} | null> {
        return prisma.user.findUnique({
            where: { id: userId },
    ...
    async findManyActiveWithPushTokenAndSite(departmentId?: string, siteId?: string, excludeUserId?: string): Promise<Array<{id: string}>> {
        const where: Prisma.UserWhereInput = {
            isActive: true,
            pushToken: { not: null }
        }
    ...
    async findByIdWithPushToken(userId: string): Promise<{ id: string, pushToken: string | null } | null> {
        return prisma.user.findUnique({
            where: { id: userId },
            select: { id: true, pushToken: true }
        })
    ...
    async findManyWithPushTokenAndFilter(userIds: string[]): Promise<Array<{ id: string, pushToken: string | null }>> {
        return prisma.user.findMany({
            where: {
                id: { in: userIds },
                pushToken: { not: null }
    ...
    async findManyWithCustomWhere(where: Prisma.UserWhereInput): Promise<Array<{ id: string }>> {
        return prisma.user.findMany({
            where,
            select: { id: true }
        })
