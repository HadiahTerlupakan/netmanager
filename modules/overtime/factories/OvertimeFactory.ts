...
export class OvertimeFactory {
    ...
    /**
     * Create emergency/urgent overtime request (auto-approved)
     */
    static createEmergencyRequest(dto: {
        userId: string
        reason: string
        approverId: string
    }): CreateOvertimeInput & { approvedBy: string } {
        return {
            userId: dto.userId,
            reason: `[DARURAT] ${dto.reason}`,
            status: 'APPROVED',
            isHolidayOvertime: false,
            isNationalHoliday: false,
            isOffDay: false,
    ...
    /**
     * Create scheduled overtime (pre-approved for recurring events)
     */
    static createScheduledRequest(dto: {
        userId: string
        reason: string
        scheduledDate: Date
    }): CreateOvertimeInput & { scheduledFor: Date } {
        return {
            userId: dto.userId,
            reason: `[TERJADWAL] ${dto.reason}`,
            status: 'PENDING',
            isHolidayOvertime: false,
            isNationalHoliday: false,
            isOffDay: false,
            scheduledFor: dto.scheduledDate,
        }
    }
