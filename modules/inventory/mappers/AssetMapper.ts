...
export class AssetMapper {
  private static mapAssignedActor(entity: AssetWithRelations) {
    if (entity.user) {
      return {
        type: entity.assignedActorType ?? "user",
        id: entity.user.id,
  ...
  static toListItem(entity: AssetWithRelations): AssetListItemDTO {
    const assignedActor = this.mapAssignedActor(entity);
    ...
    return {
      ...
      assignedToName: assignedActor?.name ?? null,
      assignedActor,
  ...
  private static mapDepreciationLogs(
    logs: AssetDepreciationLog[],
  ): AssetDepreciationLogDTO[] {
    return logs.map((log) => ({
      id: log.id,
