...
export class PrismaPayrollRunRepository implements IPayrollRunRepository {
  async findById(id: string, tenantId: string): Promise<PayrollRun | null> {
    const record = await prisma.payrollRunV2.findFirst({
      where: { id, tenantId },
    });
  ...
  async findAll(filter: PayrollRunFilter): Promise<PayrollRun[]> {
    const records = await prisma.payrollRunV2.findMany({
      where: {
  ...
  async create(
    ...
    const record = await prisma.payrollRunV2.create({ data });
  ...
  async update(
    ...
    const record = await prisma.payrollRunV2.update({
      where: { id },
      data: updateData,
    });
    return this.toEntity(record);
  ...
  async delete(id: string, tenantId: string): Promise<void> {
    ...
    await prisma.payrollRunV2.delete({ where: { id } });
  }
