...
export class PrismaPayrollComponentRepository implements IPayrollComponentRepository {
  async findById(
    ...
    const record = await prisma.payrollComponentV2.findFirst({
      where: { id, tenantId },
    });
  ...
  async findAll(filter: ComponentFilter): Promise<PayrollComponent[]> {
    const records = await prisma.payrollComponentV2.findMany({
      where: {
  ...
  async create(
    ...
    const record = await prisma.payrollComponentV2.create({ data });
  ...
  async update(
    ...
    const record = await prisma.payrollComponentV2.update({
      where: { id },
      data: updateData,
    });
    return this.toEntity(record);
  ...
  async delete(id: string, tenantId: string): Promise<void> {
    ...
    await prisma.payrollComponentV2.delete({ where: { id } });
  }
