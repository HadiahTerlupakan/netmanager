...
function buildDefaultCheckout(checkIn: Date, scheduleEndTime?: string | null): Date {
  const checkoutAt = new Date(checkIn)
  const [hours, minutes] = (scheduleEndTime ?? `${ATTENDANCE_CONSTANTS.END_OF_DAY_HOUR}:00`).split(':').map(Number)

  checkoutAt.setHours(hours, Number.isFinite(minutes) ? minutes : 0, 0, 0)

  if (checkoutAt <= checkIn) {
    checkoutAt.setTime(checkIn.getTime() + ATTENDANCE_CONSTANTS.DEFAULT_WORK_HOURS * 60 * 60 * 1000)
  }
...
function getOvernightShiftEnd(checkIn: Date, startTime: string | null, endTime: string | null): Date | null {
...
export class AttendanceSessionPolicyService {
  resolve(input: AttendanceSessionPolicyInput): AttendanceSessionPolicyDecision {
    ...
    if (workingHourMode === 'FLEXIBLE') {
      const threshold = new Date(attendance.checkIn.getTime() + 24 * 60 * 60 * 1000)
    ...
    return {
      reason: shouldAutoCheckout ? 'eligible-for-auto-checkout' : 'same-day-open',
      isOvernightShiftActive: false,
      isStaleFlexibleSession: false,
      shouldAutoCheckout,
