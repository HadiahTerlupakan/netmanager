...
export class LeaveBalanceRepository {
     ...
     */
    async upsertQuota(userId: string, year: number, leaveType: LeaveType, quota: number, tenantId?: string) {
        const id = randomUUID()
     ...
     */
    async initializeYearlyBalance(userId: string, year: number, tenantId?: string) {
     ...
     */
    async incrementUsed(userId: string, year: number, leaveType: LeaveType, days: number, tenantId?: string) {
        // First ensure balance exists
        const balance = await this.getBalance(userId, year, leaveType, tenantId)
        if (!balance) {
            await this.upsertQuota(userId, year, leaveType, DEFAULT_LEAVE_QUOTAS[leaveType], tenantId)
            const newBalance = await this.getBalance(userId, year, leaveType, tenantId)
     ...
     */
    async decrementUsed(userId: string, year: number, leaveType: LeaveType, days: number, tenantId?: string) {
        const balance = await this.getBalance(userId, year, leaveType, tenantId)
     ...
     */
    async getRemainingDays(userId: string, year: number, leaveType: LeaveType, tenantId?: string): Promise<number> {
        const balance = await this.getBalance(userId, year, leaveType, tenantId)
        if (!balance) {
     ...
     */
    async hasEnoughDays(userId: string, year: number, leaveType: LeaveType, requiredDays: number, tenantId?: string): Promise<boolean> {
        const remaining = await this.getRemainingDays(userId, year, leaveType, tenantId)
     ...
     */
    async getAllBalances(year: number, filters?: { departmentId?: string; siteId?: string; tenantId?: string }) {
        const where: { year: number; tenantId?: string; user?: { departmentId?: string; siteId?: string } } = {
            year,
