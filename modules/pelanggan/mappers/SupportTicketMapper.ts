...
export class SupportTicketMapper {
     ...
     */
    static toPortal(entity: TicketWithRelations, hasNewReply: boolean = false): TicketPortalDTO {
        const lastReply = entity.replies?.[entity.replies.length - 1]
    ...
    private static mapMessages(replies: (TicketReplies & {
        ...
        return replies.map(reply => ({
            ...
            sender: reply.sender ? {
                ...
                type: reply.isFromAdmin ? 'admin' as const : 'customer' as const,
            } : null,
            attachments: [] as TicketAttachmentDTO[], // Attachments per message can be added if needed
        }))
    }

    private static mapAttachments(attachments: {
        id: string
        fileName: string
        fileUrl: string
        fileType: string | null
