...
export async function getApiSettings(): Promise<ApiSettingsPayload> {
  const records = await SettingsRepository.findManyByKeys(API_SETTINGS_KEYS)
  return mapApiSettingsResponse(records)
}

export async function updateApiSettings(payload: ApiSettingsPostPayload): Promise<void> {
...
async function fetchAvailableGeminiModels(apiKey: string): Promise<string | null> {
  try {
...
export async function testGoogleGeminiApiKey(apiKey: string): Promise<GeminiApiKeyTestResult> {
  ...
  try {
    ...
    try {
      ...
      console.error('[apiSettings] Unable to parse Gemini test error response', error)
      responseBody = null
    }

    const baseMessage = (responseBody as { error?: { message?: string } })?.error?.message
    ...
    return { success: false, error: 'Gagal memverifikasi API Key. Silakan coba lagi.' }
  }
}
