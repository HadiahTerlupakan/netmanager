...
function findBinary(candidates: string[], fallbackBinary: string): string {
...
export async function createBackupArchive(): Promise<BackupExportResult> {
  ...
  try {
    ...
    const fileNames = dumpFiles.map((filePath) => path.basename(filePath)).join(' ')
    await execAsync(`tar -czf "${tarFilePath}" -C "${tmpDir}" ${fileNames}`, { shell: '/bin/sh' })
    const fileBuffer = fs.readFileSync(tarFilePath)

    return {
      fileName: tarFileName,
      fileBuffer,
      databases: dumpFiles.map((filePath) => path.basename(filePath, '.sql.gz')),
      warnings: errors,
    }
  } finally {
    try {
...
export async function importBackupArchive({
  ...
  try {
    if (!fileName.endsWith('.tar.gz') && !fileName.endsWith('.tgz')) {
      throw new Error('Format file tidak valid. Gunakan file .tar.gz hasil export backup.')
    }
    ...
    const extractedFiles = fs.readdirSync(extractDir).filter((entry) => entry.endsWith('.sql.gz'))
      ...
      try {
        ...
        if (prismaConfig) {
          ...
          try {
            ...
            console.warn(`[backup:import] prisma db push warning for ${dbName}:`, String(pushError).substring(0, 300))
          }
        }

        results.push({ database: dbName, status: 'success', message: 'Berhasil di-restore. Data diganti dengan isi backup.' })

        if (tenantId) {
          try {
            const getTablesCommand = `${pgPrefix} "${psqlBin}" -d "${dbConfig.database}" -t -A -c "SELECT table_name FROM information_schema.columns WHERE column_name = 'tenantId' AND table_schema = 'public'"`
            const tablesResult = await execAsync(getTablesCommand, { shell: '/bin/sh' })
            const tables = tablesResult.stdout.trim().split('\n').filter(Boolean)

            for (const table of tables) {
              const backfillCommand = `${pgPrefix} "${psqlBin}" -d "${dbConfig.database}" -c "UPDATE \\\"${table}\\\" SET \\\"tenantId\\\" = '${tenantId}' WHERE \\\"tenantId\\\" IS NULL;"`
              await execAsync(backfillCommand, { shell: '/bin/sh' })
            }
          } catch (backfillError) {
            console.warn(`[backup:import] Auto-backfill warning for ${dbName}:`, String(backfillError).substring(0, 300))
          }
        }
      } catch (error) {
        const message = error instanceof Error ? error.message : String(error)
        results.push({ database: dbName, status: 'error', message: `Gagal restore: ${message.substring(0, 200)}` })
      }
    }
...
export async function resetDatabasesAndSchema(): Promise<BackupResetResult> {
  ...
  for (const [dbName, envVar] of Object.entries(DB_ENV_MAP)) {
    ...
    const dbConfig = parseDatabaseUrl(rawUrl)
    if (!dbConfig) {
      results.push({ database: dbName, status: 'error', message: 'Gagal parse DATABASE_URL.' })
      continue
    }
