...
export async function getMitraTokens(mitraId: string): Promise<string[]> {
    try {
        ...
        console.error('Error fetching mitra FCM tokens:', e)
        return []
    }
}
 ...
 */
export async function sendFCMNotification(tokens: string[], title: string, body: string, data?: Record<string, string>) {
