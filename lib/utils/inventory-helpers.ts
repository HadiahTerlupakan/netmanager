...
export function buildTransferDeleteConfirmMessage(transfer: {
  kodeTransfer: string;
  jumlah: number;
  barang?: {
    nama?: string | null;
  ...
  const barangNama = transfer.barang?.nama || "Unknown";
  const barangSatuan = transfer.barang?.satuan || "";
  const jumlahLabel = barangSatuan
    ? `${transfer.jumlah} ${barangSatuan}`
    : String(transfer.jumlah);

  return `Apakah Anda yakin ingin membatalkan transfer ini?\n\nKode: ${transfer.kodeTransfer}\nBarang: ${barangNama}\nJumlah: ${jumlahLabel}\n\nStok akan dikembalikan ke gudang sumber.`;
}
