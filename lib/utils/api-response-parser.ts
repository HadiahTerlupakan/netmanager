    ...
    for (const key of dataKeys) {
      if (key in obj && obj[key] !== undefined) {
        ...
        const meaningfulKeys = Object.keys(obj).filter(
          (k) => !['success', 'message', 'pagination', 'meta', 'total', 'page'].includes(k)
        )
...
export async function parsePaginatedResponse<T>(
  ...
  const pagination: PaginationMeta = {
    ...
    limit: extractNumber(obj, ['limit', 'perPage', 'per_page', 'pageSize', 'page_size']) || 10,
    total: extractNumber(obj, ['total', 'totalCount', 'total_count', 'count']) || 0,
    totalPages: extractNumber(obj, ['totalPages', 'total_pages', 'pages', 'lastPage', 'last_page']) || 1,
  }
  ...
  if (obj.pagination && typeof obj.pagination === 'object') {
    ...
    pagination.page = extractNumber(paginationObj, ['page', 'currentPage']) || pagination.page
    pagination.limit = extractNumber(paginationObj, ['limit', 'perPage', 'pageSize']) || pagination.limit
    pagination.total = extractNumber(paginationObj, ['total', 'totalCount']) || pagination.total
    pagination.totalPages = extractNumber(paginationObj, ['totalPages', 'pages']) || pagination.totalPages
  }

  // Check for meta object
  if (obj.meta && typeof obj.meta === 'object') {
    const metaObj = obj.meta as Record<string, unknown>
    pagination.page = extractNumber(metaObj, ['page', 'currentPage']) || pagination.page
    pagination.limit = extractNumber(metaObj, ['limit', 'perPage', 'pageSize']) || pagination.limit
    pagination.total = extractNumber(metaObj, ['total', 'totalCount']) || pagination.total
    pagination.totalPages = extractNumber(metaObj, ['totalPages', 'pages']) || pagination.totalPages
  }
