...
export const customerCreateSchema = z.object({
  ...
  nomorKk: z.string().regex(/^\d{16}$/, 'KK number must be 16 digits').optional(),
})
...
export const employeeCreateSchema = z.object({
  ...
  salary: z.number().positive('Salary must be positive').max(999999999.99, 'Salary is too large'),
  status: z.enum(['PROBATION', 'PERMANENT', 'CONTRACT', 'TERMINATED']).default('PROBATION'),
})
...
export const tagihanCreateSchema = z.object({
  pelangganId: z.string().uuid('Invalid customer ID format'),
  periode: z.string().min(1, 'Period is required').max(20),
  deskripsi: z.string().min(1, 'Description is required').max(500),
  jumlah: z.number().positive('Amount must be positive').max(999999999.99, 'Amount is too large'),
  jatuhTempo: z.string().datetime('Invalid due date format'),
})
...
export const kmzUploadSchema = z.object({
  filename: z.string().min(1, 'Filename is required').max(255),
  mimetype: z.enum(['application/vnd.google-earth.kmz', 'application/octet-stream']),
...
export const paymentCreateSchema = z.object({
  ...
  customerPhone: z.string().regex(/^[+]?[\d\s\-()]{10,20}$/, 'Invalid phone number format').optional(),
  expiryHours: z.number().int().min(1).max(168).default(24), // Max 7 days
})

export const paymentWebhookSchema = z.object({
  orderId: z.string().min(1, 'Order ID is required').max(255),
  status: z.enum(['SUCCESS', 'PAID', 'PENDING', 'EXPIRED', 'CANCELLED', 'FAILED']),
  amount: z.number().positive('Amount must be positive').max(999999999.99),
  paidTime: z.string().datetime().optional(),
