...
const ALLOWED_MIME_TYPES_BY_FOLDER: Record<string, Set<string>> = {
  invoices: new Set(['application/pdf', 'image/jpeg', 'image/png', 'image/webp']),
  'mitra-document': new Set(['application/pdf', 'image/jpeg', 'image/png', 'image/webp']),
  'work-orders': new Set(['application/pdf', 'image/jpeg', 'image/png', 'image/webp']),
...
export function sanitizeUploadFolder(rawFolder: string | null | undefined): FolderResult {
...
export function validateUploadFile(input: FileValidationInput): FileValidationResult {
  ...
  const allowedMimeTypes = ALLOWED_MIME_TYPES_BY_FOLDER[input.folder] ?? ALLOWED_MIME_TYPES_BY_FOLDER.default
