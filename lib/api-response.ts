...
export function serializeData<T>(data: T): T {
  ...
  if (typeof data === "bigint") {
    ...
    data >= BigInt(Number.MIN_SAFE_INTEGER)
      ? Number(data)
...
export const ErrorCodes = {
  ...
  // Idempotency (409): klien membedakan "ulangi nanti" dari "kunci salah pakai"
 ...
 */
export interface PaginatedWithSummaryResponse<
  T,
  S = unknown,
...
export function apiPaginatedWithSummary<T, S = unknown>(
  ...
  const serializedData = serializeData(data) as T[];
  const serializedSummary = options.summary
    ? (serializeData(options.summary) as S)
    : undefined;
...
export const ApiErrors = {
  ...
  badRequest: (message: string, details?: Record<string, unknown>) =>
    apiError(message, ErrorCodes.VALIDATION_ERROR, {
      status: 400,
      ...(details && { details }),
    }),
