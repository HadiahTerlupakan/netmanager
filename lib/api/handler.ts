    ...
    try {
      ...
      if (options.auth) {
        ...
        if (session?.user) {
          ...
          const userPermissions = (session.user as { permissions?: string[] })
            .permissions;
          ...
          if (authHeader?.startsWith("Bearer ")) {
            ...
            if (token && token !== "null") {
              ...
              const { getMobileRequestVersionCode } =
                await import("@/lib/mobile-api-auth");
        ...
        if (!ctx.session?.user) {
          return ApiErrors.unauthorized(
            "Session tidak valid atau Token kedaluwarsa",
          );
        }
      }
      ...
      if (options.permissions && options.permissions.length > 0) {
        ...
        const hasPermission = expandedPerms.some(
          (perm) =>
            ctx.permissions.includes(perm) || ctx.permissions.includes("*"),
        );
      ...
      // 4. Execute handler
      const response = await runWithRequestTenantContext(
        {
  ...
  if (typeof message === "string") {
    ...
    if (message.includes("Unique constraint")) {
      ...
      return ApiErrors.conflict(
        `Data sudah ada${fieldName}. Silakan gunakan nilai lain.`,
      );
    }
    if (
