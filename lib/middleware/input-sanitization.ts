  ...
  try {
    ...
    if (req.headers.get('content-type')?.includes('application/json')) {
      try {
        ...
        console.error('Error parsing JSON for sanitization:', error)
        return req
      }
    }
    ...
    if (req.headers.get('content-type')?.includes('multipart/form-data')) {
      try {
        ...
        console.error('Error parsing form data for sanitization:', error)
        return req
      }
    }
    ...
    console.error('Error in request sanitization:', error)
    return req
  }
}
...
export function sanitizeQueryParams(req: NextRequest): NextRequest {
  try {
    ...
    console.error('Error in query sanitization:', error)
    return req
  }
}
...
export function sanitizePathParams(params: Record<string, string>): Record<string, string> {
  try {
    ...
    console.error('Error in path params sanitization:', error)
    return params
  }
}
