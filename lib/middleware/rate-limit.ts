  ...
  return async (context: any, routeContext?: any): Promise<NextResponse> => {
    try {
      ...
      if (isAuthContext) {
        ...
        const req =
          ((context as { request?: unknown }).request as {
            headers?: { get: (name: string) => string | null };
          }) ||
          (context as { headers?: { get: (name: string) => string | null } });
...
export function withAuthRateLimit<T = unknown>(
  ...
  return async (
    ...
    try {
      ...
      logger.error("[Auth Rate Limit] Error checking rate limit:", error);
      // Fail open - allow request if rate limiting fails
      return handler(context, routeContext);
    }
  };
