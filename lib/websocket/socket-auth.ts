...
export async function resolveSocketAuth(
  ...
  if (bearerToken) {
    logger.info("[WS] Attempting mobile token auth...");
    const { verifyMobileToken } = await import("../mobile-auth");
    const mobileUser = await verifyMobileToken(bearerToken);
    if (mobileUser?.userId) {
      logger.info(`[WS] Mobile auth success for user: ${mobileUser.userId}`);
      return {
        userId: mobileUser.userId,
        userRole: String(mobileUser.role || "USER"),
        departmentId:
          typeof mobileUser.departmentId === "string"
            ? mobileUser.departmentId
            : undefined,
        accessAdminPanel: Array.isArray(mobileUser.permissions)
          ? mobileUser.permissions.includes("*") ||
            mobileUser.permissions.includes("admin:read")
          : false,
      };
    }
    logger.warn("[WS] Mobile token verification failed");
  }
  ...
  try {
    ...
    logger.warn("[WS] Failed to decode session token manually:", e);
  }
  ...
  if (!tokenSub) {
    ...
    try {
      ...
      logger.warn("[WS] Fallback getToken failed:", e);
    }
  }

  if (!tokenSub) {
    logger.error(
      "[WS] Auth failed: no session token found in cookies (Names available:",
      cookieHeader
        ? cookieHeader
            .split(";")
...
export async function canJoinRoom(
  ...
  if (room.startsWith("workorder:")) {
    ...
    return Boolean(
      ...
      (socketData.departmentId &&
        workOrder.departmentId === socketData.departmentId),
    );
  }
  ...
  if (room.startsWith("chat:")) {
    ...
    return (
      ...
      Boolean(socketData.accessAdminPanel)
    );
  }
