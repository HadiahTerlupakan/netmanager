...
declare global {
  ...
  var socketRoomCleanupInterval: ReturnType<typeof setInterval> | undefined;
}

const initializedSocketServers = new WeakSet<SocketIOServer>();
...
export function initializeSocketServer(socketServer: SocketIOServer) {
  ...
  if (initializedSocketServers.has(socketServer)) {
    return globalThis.socketIOServer;
  }

  initializedSocketServers.add(socketServer);
  ...
  globalThis.socketIOServer.on("connection", async (socket: Socket) => {
    const { userId, userRole, departmentId, accessAdminPanel } =
      socket.data as SocketData;

    console.log(
      `[WS] User connected: ${userId} (${userRole}) [Admin: ${!!accessAdminPanel}]`,
    );
    ...
    socket.on(
      ...
      async (data: { room?: unknown } | string) => {
        const room =
          typeof data === "string"
            ? data
        ...
        if (!room) {
          console.warn(`[WS] ${userId} sent invalid join room payload:`, data);
          return;
        }

        // Validate room name to prevent unauthorized access
        if (await canJoinRoom(socket.data as SocketData, room)) {
          socket.join(room);
    ...
    // Handle room leaving
    socket.on(SOCKET_EVENTS.LEAVE_ROOM, (data: { room?: unknown } | string) => {
      const room =
        typeof data === "string"
          ? data
          : typeof data?.room === "string"
      ...
      if (!room) {
        console.warn(`[WS] ${userId} sent invalid leave room payload:`, data);
        return;
      }
...
if (!globalThis.socketRoomCleanupInterval) {
  globalThis.socketRoomCleanupInterval = setInterval(() => {
...
export function resetSocketServerForTests() {
  initializedSocketServers.delete(globalThis.socketIOServer as SocketIOServer);
  globalThis.socketIOServer = undefined;
}

export function clearSocketRoomCleanupIntervalForTests() {
  if (globalThis.socketRoomCleanupInterval) {
    clearInterval(globalThis.socketRoomCleanupInterval);
    globalThis.socketRoomCleanupInterval = undefined;
  }
}
